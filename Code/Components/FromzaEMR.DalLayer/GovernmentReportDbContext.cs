...
namespace FromzaEMR.DalLayer
{
    public class GovernmentReportDbContext : DbContext
    {
        ...
        public DynamicReport GovReportSummary(DateTime FromDate, DateTime ToDate)
        {
            ...
            if (dsCtrUsrs != null && dsCtrUsrs.Tables.Count > 0)
            {
                ...
                var reports = new
                {
                    ...
                    OutpatientReferredOut = dsCtrUsrs.Tables[8]
                };
        ...
        public InpatientServiceReportModel GetInpatientOutcome(DateTime FromDate, DateTime ToDate)
        {
            ...
            obj.MedicoLegalCases = JsonConvert.SerializeObject(dsInpatientOutcome.Tables[7]);
        ...
        public HospitalMortalityReportModel GetHospitalMortality(DateTime FromDate, DateTime ToDate)
        {
            List<SqlParameter> paramsList = new List<SqlParameter>();
            paramsList.Add(new SqlParameter("@FromDate", FromDate));
            paramsList.Add(new SqlParameter("@ToDate", ToDate));
            DataSet dsHospitalMorbi = GetDatasetFromStoredProc("SP_Report_Gov_HospitalMortality", paramsList, this.connStr);
            HospitalMortalityReportModel obj = new HospitalMortalityReportModel();
            obj.HospitalMortality = JsonConvert.SerializeObject(dsHospitalMorbi.Tables[0]);
