...
using FromzaEMR.Jobs.Utilities;
namespace FromzaEMR.Jobs.IRDNepal
{
                        ...
                        using (var dbContextTransaction = dbContext.Database.BeginTransaction())
                        {
                            try
                            {
                                foreach (BillingTransactionModel bilTxn in unSyncedBillTxns)
                                {
                                    ...
                                    string npDate=FromzaDateConvertor.NepDateToString(FromzaDateConvertor.ConvertEngToNepDate(Convert.ToDateTime(salesInvoice.invoice_date_eng)));                                    
                ...
                try
                {
                    ...
                    var retBillsToSync = (from ret in dbContext.BillReturnRequests
                                          ...
                                          }).ToList().Select(x => new IRD_Common_InvoiceModel
                                          {
                                              ...
                                              credit_note_date= FromzaDateConvertor.NepDateToString(FromzaDateConvertor.ConvertEngToNepDate(Convert.ToDateTime(x.credit_note_date))),
                    ...
                    if (retBillsToSync != null && retBillsToSync.Count > 0)
                    {
                        using (var dbContextTransaction = dbContext.Database.BeginTransaction())
                        {
                            try
                            {
                                ...
                                {
                                    string npDate = FromzaDateConvertor.NepDateToString(
                                        FromzaDateConvertor.ConvertEngToNepDate(Convert.ToDateTime(inv.return_date_eng)));
