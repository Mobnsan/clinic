...
namespace FromzaEMR.Security
{
    ...
    public class RBAC
    {
        ...
        {
            List<RbacApplication> retList = (List<RbacApplication>)FromzaCache.Get("RBAC-Apps-All");
            if (retList == null)
            {
                RbacDbContext dbContext = new RbacDbContext(connStringName);
                retList = dbContext.Applications.ToList();
                FromzaCache.Add("RBAC-Apps-All", retList, cacheExpiryMinutes);
            }
            return retList;
        }
        ...
        {
            List<RbacPermission> retList = (List<RbacPermission>)FromzaCache.Get("RBAC-Perms-All");
            if (retList == null)
            {
                RbacDbContext dbContext = new RbacDbContext(connStringName);
                retList = dbContext.Permissions.ToList();
                FromzaCache.Add("RBAC-Perms-All", retList, cacheExpiryMinutes);
            }
            return retList;
        ...
        {
            List<RbacRole> retList = (List<RbacRole>)FromzaCache.Get("RBAC-Roles-All");
            if (retList == null)
            {
                RbacDbContext dbContext = new RbacDbContext(connStringName);
                retList = dbContext.Roles.ToList();
                FromzaCache.Add("RBAC-Roles-All", retList, cacheExpiryMinutes);
            }
            return retList;
        }
        ...
        {
            List<RbacUser> retList = (List<RbacUser>)FromzaCache.Get("RBAC-Users-All");
            if (retList == null)
            {
                RbacDbContext dbContext = new RbacDbContext(connStringName);
                retList = dbContext.Users.ToList();
                FromzaCache.Add("RBAC-Users-All", retList, cacheExpiryMinutes);
            }
            return retList;
        }
        ...
        {
            List<UserRoleMap> retList = (List<UserRoleMap>)FromzaCache.Get("RBAC-UserRoleMaps-All");
            if (retList == null)
            {
                RbacDbContext dbContext = new RbacDbContext(connStringName);
                retList = dbContext.UserRoleMaps.ToList();
                FromzaCache.Add("RBAC-UserRoleMaps-All", retList, cacheExpiryMinutes);
            }
        ...
        {
            List<RolePermissionMap> retList = (List<RolePermissionMap>)FromzaCache.Get("RBAC-RolePermissionMaps-All");
            if (retList == null)
            {
                RbacDbContext dbContext = new RbacDbContext(connStringName);
                retList = dbContext.RolePermissionMaps.ToList();
                FromzaCache.Add("RBAC-RolePermissionMaps-All", retList, cacheExpiryMinutes);
            }
            return retList;
        }
        ...
        {
            List<FromzaRoute> retList = (List<FromzaRoute>)FromzaCache.Get("RBAC-Routes-All");
            if (retList == null)
            {
                RbacDbContext dbContext = new RbacDbContext(connStringName);
                retList = dbContext.Routes.ToList();
                FromzaCache.Add("RBAC-Routes-All", retList, cacheExpiryMinutes);
            }
            return retList;
        ...
        public static List<FromzaRoute> GetRoutesForUser(int userId, bool getHiearrchy)
        {
            List<FromzaRoute> allRoutes = new List<FromzaRoute>();
            ...
            {
                //don't get hidden routes if it's set to false.
                List<FromzaRoute> parentRoutes = allRoutes.Where(a => a.ParentRouteId == null && a.DefaultShow == true).ToList();
        ...
        //don't get hidden routes if it's set to false.
        private static List<FromzaRoute> GetChildRouteHierarchy(List<FromzaRoute> searchList, FromzaRoute route)
        {
            //don't get hidden routes if it's set to false.
            List<FromzaRoute> childRoutes = searchList.Where(a => a.ParentRouteId == route.RouteId).Distinct().ToList();
        ...
        {
            List<RbacPermission> retList = (List<RbacPermission>)FromzaCache.Get("RBAC-UserPermissions-UserId" + userId);
            if (retList == null)
            {
                ...
                FromzaCache.Add("RBAC-UserPermissions-UserId" + userId, retList, cacheExpiryMinutes);
            }
            return retList;
