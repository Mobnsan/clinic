...
namespace FromzaEMR.ServerModel
{
    public class FixedAssetStockModel
    {
        ...
        //navigation property
        public List<AssetLocationHistoryModel> AssetMovements { get; set; } = new List<AssetLocationHistoryModel>();

        public void Dispatch(int ToSubStoreId, int currentUser, DateTime currentDate)
        {
            var lastAssetMovement = AssetMovements.FirstOrDefault(a => a.EndDate == null);
            lastAssetMovement.EndDate = currentDate;
            AssetMovements.Add(new AssetLocationHistoryModel()
            {
                FixedAssetStockId = FixedAssetStockId,
                CreatedBy = currentUser,
                StartDate = currentDate,
                OldStoreId = SubStoreId ?? StoreId,
                // reset ownership
                OldAssetHolderId = null,
                OldLocation = null
            });
            SubStoreId = ToSubStoreId;
        }

        public void Return(int TargetStoreId, int curretUser, DateTime currentDate)
        {
            var lastAssetMovement = AssetMovements.FirstOrDefault(a => a.EndDate == null);
            lastAssetMovement.EndDate = currentDate;
            AssetMovements.Add(new AssetLocationHistoryModel()
