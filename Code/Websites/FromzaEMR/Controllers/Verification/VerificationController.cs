...
using FromzaEMR.Services.Verification.DTOs;
using FromzaEMR.Services.Verification.DTOs.Pharmacy;
...
namespace FromzaEMR.Controllers
{
    ...
    public class VerificationController : CommonController
    {
        ...
        private readonly InventoryDbContext _invenotryDbContext;
        ...
        public VerificationController(IOptions<MyConfiguration> _config, IVerificationService verificationService, IInventoryGoodReceiptService goodReceiptService, IInventoryReceiptNumberService inventoryReceiptNumberService) : base(_config)
        {
            ...
            _invenotryDbContext = new InventoryDbContext(connString);
        }
        [HttpGet]
        ...
        [HttpGet]
        [Route("GetInventoryRequisitionDetails")]
        public IActionResult GetInventoryRequisitionDetails(int requisitionId)
        {
            RbacUser currentUser = HttpContext.Session.Get<RbacUser>(ENUM_SessionVariables.CurrentUser);
            Func<InventoryRequisitionViewModel> func = () => VerificationBL.GetInventoryRequisitionDetails(requisitionId, currentUser, _invenotryDbContext);
        ...
        public IActionResult ApproveRequisition(string VerificationRemarks = "")
        {
            ...
            RequisitionModel requisition = FromzaJSONConvert.DeserializeObject<RequisitionModel>(data);
            ...
            try
            {
                ...
                var CurrentVerificationLevelCount = requisition.CurrentVerificationLevelCount + 1;
                ...
                VerificationBL.UpdateRequisitionAfterApproved(dbContext, requisition, VerificationId, currentUser, CurrentVerificationLevelCount);
        ...
        public IActionResult GetInventoryPurchaseRequestDetails([FromRoute] int PurchaseRequestId)
        {
            try
            {
                var inventoryDbContext = new InventoryDbContext(connString);
                RbacUser currentUser = HttpContext.Session.Get<RbacUser>("currentuser");
                InventoryPurchaseRequestViewModel PurchaseRequestDetails = VerificationBL.GetInventoryPurchaseRequestDetails(PurchaseRequestId, inventoryDbContext, null);
        ...
        public IActionResult ApprovePurchaseRequest(string VerificationRemarks = "")
        {
            ...
            var currentUser = HttpContext.Session.Get<RbacUser>("currentuser");
            var data = this.ReadPostData();
            PurchaseRequestModel PurchaseRequest = FromzaJSONConvert.DeserializeObject<PurchaseRequestModel>(data);
            var dbContext = new InventoryDbContext(connString);
            ...
            try
            {
                ...
                var CurrentVerificationLevelCount = PurchaseRequest.CurrentVerificationLevelCount + 1;
        ...
        public IActionResult ApprovePurchaseOrder(string VerificationRemarks = "")
        {
            ...
            var currentUser = HttpContext.Session.Get<RbacUser>("currentuser");
            var data = this.ReadPostData();
            PurchaseOrderModel PurchaseOrder = FromzaJSONConvert.DeserializeObject<PurchaseOrderModel>(data);
            var dbContext = new InventoryDbContext(connString);
            ...
            try
            {
                ...
                var CurrentVerificationLevelCount = PurchaseOrder.CurrentVerificationLevelCount + 1;
        ...
        public IActionResult ApproveGoodsReceipt(string VerificationRemarks = "")
        {
            ...
            GoodsReceiptModel GoodsReceipt = FromzaJSONConvert.DeserializeObject<GoodsReceiptModel>(data);
            var dbContext = new InventoryDbContext(connString);
            ...
            try
            {
                ...
                var CurrentVerificationLevelCount = GoodsReceipt.CurrentVerificationLevelCount + 1;
        ...
        #endregion Inventory Purchase Order APIs


        #region Pharmacy Verification
        [HttpGet]
        [Route("PharmacyPurchaseOrdersBasedOnUser")]
        public IActionResult GetPharmacyPurchaseOrdersBasedOnUser(DateTime fromDate, DateTime toDate)
        {
            RbacUser currentUser = HttpContext.Session.Get<RbacUser>(ENUM_SessionVariables.CurrentUser);

            Func<object> func = () => VerificationBL.GetPharmacyPurchaseOrdersBasedOnUser(fromDate, toDate, _pharmacyDbContext, currentUser);
            return InvokeHttpGetFunction(func);
        }

        [HttpGet]
        [Route("PharmacyPurchaseOrderInfo")]
        public IActionResult GetPharmacyPurchaseOrderInfo(int purchaseOrderId)
        {
            Func<object> func = () => VerificationBL.GetOrderInfo(purchaseOrderId, _pharmacyDbContext);
            return InvokeHttpGetFunction(func);
        }


        [HttpPost]
        [Route("ApprovePharmacyPurchaseOrder")]
        public IActionResult ApprovePharmacyPurchaseOrder([FromBody] PharmacyPurchaseOrder_DTO purchaseOrderDTO)
        {
            RbacUser currentUser = HttpContext.Session.Get<RbacUser>(ENUM_SessionVariables.CurrentUser);
            Func<object> func = () => VerificationBL.SaveVerification(purchaseOrderDTO, currentUser, _pharmacyDbContext);
            return InvokeHttpPostFunction(func);
        }


        [HttpPost]
        [Route("RejectPharmacyPurchaseOrder")]
        public IActionResult RejectPharmacyPurchaseOrder(int purchaseOrderId, int currentVerificationlevel, int currentVerificationLevelCount, int maxVerificationLevel, string VerificationRemarks)
        {
            RbacUser currentUser = HttpContext.Session.Get<RbacUser>(ENUM_SessionVariables.CurrentUser);
            Func<object> func = () => VerificationBL.SaveRejectedPurchaseOrder(purchaseOrderId, currentVerificationlevel, currentVerificationLevelCount, maxVerificationLevel, VerificationRemarks, currentUser, _pharmacyDbContext);
            return InvokeHttpPostFunction(func);
        }

        [HttpGet]
        [Route("PharmacyRequisitionsBasedOnUser")]
        public IActionResult GetPharmacyRequisitionsBasedOnUser(DateTime fromDate, DateTime toDate)
        {
            RbacUser currentUser = HttpContext.Session.Get<RbacUser>(ENUM_SessionVariables.CurrentUser);

            Func<object> func = () => VerificationBL.GetPharmacyRequisitionsBasedOnUser(fromDate, toDate, _pharmacyDbContext, currentUser);
            return InvokeHttpGetFunction(func);
        }

        [HttpGet]
        [Route("PharmacyRequisitionInfo")]
        public IActionResult GetPharmacyRequisitionInfo(int requisitionId)
        {
            Func<object> func = () => VerificationBL.GetRequisitionInfo(requisitionId, _pharmacyDbContext);
            return InvokeHttpGetFunction(func);
        }
        ...
        [HttpPost]
        [Route("ApprovePharmacyRequisition")]
        public IActionResult ApprovePharmacyRequisition([FromBody] PharmacySubStoreRequisitionVerification_DTO requisitionDTO)
        {
            RbacUser currentUser = HttpContext.Session.Get<RbacUser>(ENUM_SessionVariables.CurrentUser);
            Func<object> func = () => VerificationBL.SavePharmacyRequisitionVerification(requisitionDTO, currentUser, _pharmacyDbContext);
            return InvokeHttpPostFunction(func);
        }


        [HttpPost]
        [Route("RejectPharmacyRequisition")]
        public IActionResult RejectPharmacyRequisition(int requisitionId, int currentVerificationlevel, int currentVerificationLevelCount, int maxVerificationLevel, string VerificationRemarks)
        {
            RbacUser currentUser = HttpContext.Session.Get<RbacUser>(ENUM_SessionVariables.CurrentUser);
            Func<object> func = () => VerificationBL.SaveRejectedRequisition(requisitionId, currentVerificationlevel, currentVerificationLevelCount, maxVerificationLevel, VerificationRemarks, currentUser, _pharmacyDbContext);
            return InvokeHttpPostFunction(func);
        }
        #endregion
    }
