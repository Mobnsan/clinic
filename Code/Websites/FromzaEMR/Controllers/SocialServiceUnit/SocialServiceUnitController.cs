    ...
    public class SocialServiceUnitController : CommonController
    {
        ...
        public IActionResult PostSsuPatientInformation()
        {
            ...
            try
            {
                ...
                RbacUser currentUser = HttpContext.Session.Get<RbacUser>("currentuser");
                PatientModel patDetails = FromzaJSONConvert.DeserializeObject<PatientModel>(dataString);
                ...
                //{
                //    var membership = dbContext.Schemes.Where(i => i.SchemeName == "General").FirstOrDefault();
                //    patDetails.MembershipTypeId = membership.SchemeId;
                //}
                ...
                using (var dbContextTransaction = dbContext.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        patDetails = CreatePatientWithUniquePatientNum(dbContext, patDetails, connString);
        ...
        private PatientModel CreatePatientWithUniquePatientNum(SocialServiceUnitDbContext dbContext, PatientModel patDetails, string connString)
        {
            try
            {
                NewPatientUniqueNumbersVM newPatientNumber = SsuPatientBL.GetPatNumberNCodeForNewPatient(connString);
            ...
            catch (Exception ex)
            {
                if (ex is DbUpdateException dbUpdateEx)
                {
                    if (dbUpdateEx.InnerException?.InnerException is SqlException sqlException)
                    {
                        ...
                        {
                            CreatePatientWithUniquePatientNum(dbContext, patDetails, connString);
                        }
                        else
                        {
                            throw;
