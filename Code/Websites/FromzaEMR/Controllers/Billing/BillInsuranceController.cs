    ...
    public class BillInsuranceController : CommonController
    {
            ...
            DateTime fromDate)
        {
            ...
            try
            {
                ...
                {
                    var itemList = (from item in billingDbContext.BillServiceItems
                                    join srv in billingDbContext.ServiceDepartment on item.ServiceDepartmentId equals srv.ServiceDepartmentId
                                    join priceCatServItem in billingDbContext.BillItemsPriceCategoryMaps on item.ServiceItemId equals priceCatServItem.ServiceItemId
                                    where item.IsActive == true && priceCatServItem.PriceCategoryId == 1 //Krishna 13thMarch'23, 1 is for Normal and Hardocoded for now
