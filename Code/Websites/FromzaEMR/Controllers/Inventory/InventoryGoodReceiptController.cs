...
namespace FromzaEMR.Controllers
{
    ...
    public class InventoryGoodReceiptController : Controller
    {
        ...
        public IActionResult ReceiveGoodsReceipt([FromRoute] int GoodsReceiptId, string ReceivedRemarks)
        {
            ...
            try
            {
               var check = _inventoryGoodReceiptService.ReceiveGoodsReceipt(GoodsReceiptId, currentUser, ReceivedRemarks);
                if (check)
                {
                    responseData.Status = "OK";
                ...
                {
                    responseData.Status = "Failed";
                    responseData.ErrorMessage = "Error! Cannot receive item as Gr Item is Cancelled.";
        ...
        #region GET Other Charges 
        [HttpGet("getINVOtherChargesDetails")]
        public IActionResult GetINVOtherChargesDetails()
        {
            try
            {
                responseData.Results = _inventoryGoodReceiptService.GetINVOtherChargesDetails();
                responseData.Status = "OK";
            }
            catch (Exception ex)
            {
