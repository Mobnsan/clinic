...
using FromzaEMR.ServerModel.SystemAdminModels;
...
namespace FromzaEMR.Controllers
{
    ...
    public class SystemAdminController : CommonController
    {
        ...
        private readonly SystemAdminDbContext _systemAdminDbContext;
        ...
        public SystemAdminController(IOptions<MyConfiguration> _config) : base(_config)
        {
            ...
            _systemAdminDbContext = new SystemAdminDbContext(this.connStringAdmin);
            _reportingDbContext = new ReportingDbContext(this.connString);
        ...
        [HttpGet]
        [Route("DatabaseBakupLogs")]
        public IActionResult GetDatabaseBakupLogs()
        {
            //if (reqType == "getDBBakupLog")
            Func<object> func = () => (from dbBackupLog in _systemAdminDbContext.DatabaseLog
        ...
        [HttpGet]
        [Route("AuditTrialDetails")]
        public IActionResult GetAuditTrialDetails(DateTime fromDate, DateTime toDate, string table_Name, string userName, string actionName)
        {
            //if (reqtype == "get-audit-trail-details")

            Func<object> func = () => _reportingDbContext.AuditTrails(fromDate, toDate, table_Name, userName, actionName);
        ...
        {
            // if (reqType == "get-audit-list")

            Func<object> func = () => GetAuditList();
            return InvokeHttpGetFunction(func);
        }


        [HttpGet]
        [Route("LoginInformation")]
        public IActionResult GetLoginInformation(DateTime fromDate, DateTime toDate)
        {
        ...
        [HttpGet]
        [Route("IRDInvoiceDetails")]
        public IActionResult IRDInvoiceDetails(DateTime fromDate, DateTime toDate)
        {
            //if (reqType == "getIRDInvoiceDetails")
            Func<object> func = () => GetIRDInvoiceDetails(fromDate, toDate);
            return InvokeHttpGetFunction(func);
        }

        [HttpGet]
        [Route("AllIRDInvoiceDetails")]
        public IActionResult AllIRDInvoiceDetails(DateTime fromDate, DateTime toDate)
        {
            //if (reqType == "getIRDInvoiceDetails")
            Func<object> func = () => GetAllIRDInvoiceDetails(fromDate, toDate);
            return InvokeHttpGetFunction(func);
        }

        [HttpGet]
        [Route("PharmacyIRDInvoiceDetails")]
        public IActionResult PharmacyIRDInvoiceDetails(DateTime fromDate, DateTime toDate)
        {
            //if (reqType == "getPhrmIRDInvoiceDetails")
                Func<object> func = () => GetPahrmacyIRDInvoiceDetails(fromDate, toDate);  
            return InvokeHttpGetFunction(func);
        }
    
        [HttpGet]
        [Route("DatabaseActivity")]
        public IActionResult DatabaseActivity(DateTime fromDate, DateTime toDate, string logType)
        {
            //if (reqType == "getDbActivityLogDetails")
                Func<object> func = () => GetDatabaseActivity(fromDate, toDate, logType);
            return InvokeHttpGetFunction(func);
        }
        ...
        {
            //if (reqType != null && reqType == "get-system-admin")
                Func<object> func = () => (from parameters in _systemAdminDbContext.AdminParameters
        ...
        //{
        //    FromzaHTTPResponse<List<object>> responseData = new FromzaHTTPResponse<List<object>>();
        //    FromzaHTTPResponse<object> responseDataObj = new FromzaHTTPResponse<object>();
        ...
        //}
        //    #region FromzaAuditTrail Details 
        //else
        //if (reqtype == "get-audit-trail-details")
        //{
        //    reportingdbcontext dbcontext = new reportingdbcontext(this.connstringadmin);
        //    //fromzahttpresponse<datatable> resdata = new fromzahttpresponse<datatable>();
        ...
        //    return fromzajsonconvert.serializeobject(responsedataobj, true);
        //}
        ...
        //    FromzaHTTPResponse<List<InvoiceDetailsModel>> resData = new FromzaHTTPResponse<List<InvoiceDetailsModel>>();
        //    resData.Results = res;
        //    resData.Status = "OK";
        //    return FromzaJSONConvert.SerializeObject(resData, true);
        //}
        //else 
        ...
        //    FromzaHTTPResponse<List<PhrmInvoiceDetails>> resData = new FromzaHTTPResponse<List<PhrmInvoiceDetails>>();
        //    resData.Results = res;
        //    resData.Status = "OK";
        //    return FromzaJSONConvert.SerializeObject(resData, true);
        //}
        ...
        //    ReportingDbContext dbContext = new ReportingDbContext(this.connStringAdmin);
        //    FromzaHTTPResponse<List<SqlAuditModel>> resData = new FromzaHTTPResponse<List<SqlAuditModel>>();
        ...
        public IActionResult DatabaseBackup()
        {
            ...
            Func<object> func = () => PostDatabaseBackup();
        ...
        {
            //if (reqType == "databaseRestore")
            string ipDataStr = this.ReadPostData();
            Func<object> func = () => PostRestoreDatabase(ipDataStr);
            return InvokeHttpPostFunction(func);
        }


        [HttpPost]
        [Route("ExportDatabase")]
        public IActionResult ExportDatabase(string ExportType)
        {
            //if (reqType != null && reqType == "exportDBToCSVOrXMLOrPDF")
                string ipDataStr = this.ReadPostData();
            Func<object> func = () => PostExportDatabase(ExportType);
            return InvokeHttpPostFunction(func);
        }
        private object GetIRDInvoiceDetails(DateTime fromDate, DateTime toDate)
        {
            //We are calling from ReportingDbContext because of problem from admin db context
            List<InvoiceDetailsModel> res = _reportingDbContext.InvoiceDetails(fromDate, toDate);
            return res;
        }
        private object GetPahrmacyIRDInvoiceDetails(DateTime fromDate, DateTime toDate)
        {
            List<PhrmInvoiceDetails> res = _reportingDbContext.PhrmInvoiceDetails(fromDate, toDate);
            return res;

        }
        private object GetDatabaseActivity(DateTime fromDate, DateTime toDate, string logType)
        {
            //use admin-db's connection string to get db-activity log
            List<SqlAuditModel> res = _reportingDbContext.SqlAuditDetails(fromDate, toDate, logType);
            return res;

        }
        private object GetAuditList()
        {
            SystemAdminDbContext systemAdminDbContext = new SystemAdminDbContext(this.connStringAdmin);

            var userList = (from rbac in _rbacDbContext.Users
                            select new
                            {
                                UserName = rbac.UserName
                            }).ToList<object>();

            var tableNameList = _reportingDbContext.AuditTrailList().Select(s => new { Table_Name = s.Table_Name }).ToList();
        ...
        private object GetAllIRDInvoiceDetails(DateTime fromDate, DateTime toDate)
        {
            List<InvoiceDetailsModel> res = _reportingDbContext.GetAllInvoiceDetails(fromDate, toDate);
            return res;
        }

        private object PostExportDatabase(string ExportType)
        {
                ...
                try
                {
                    if (!String.IsNullOrEmpty(ExportType))
                    {
                        ...
                        {
                        throw new Exception("Failed to Export Database as " + ExportType);
                        }
                    }
                    else
                    {
                    throw new Exception("Please select export file type as  " + ExportType);
                    }
                }
                catch (Exception ex)
                {
        ...
        private object PostRestoreDatabase( string ipDataStr)
        {
            
                DatabaseLogModel dbBackupLogDataFromClient = FromzaJSONConvert.DeserializeObject<DatabaseLogModel>(ipDataStr);
                ...
                //Transaction Begin
                using (var dbContextTransaction = _systemAdminDbContext.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (System.IO.File.Exists(backupDBFilePath))
                        {
                            ...
                            if (CheckDBVersionForRestore(backupDBFileVersion))
                            {
                                ...
                                throw new Exception("Database restore failed, Please try again");

                            }
                        }
                            else
                            {
                            throw new Exception("Version is not compatible for Restore.");
                            }
                        }
                        else
                        {
                        throw new Exception("There is no backup file for restore, Please Try again.");
                        }
                    }
                    catch (Exception ex)
                    {
        ...
        private object PostDatabaseBackup()
        {
            using (var dbContextTransaction = _systemAdminDbContext.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if (CheckBackupFolderPath())
                    {
                        ...
                        {
                            throw new Exception("Today You have already taken " + todaysDBBackupFrequency + "DB backup");
                        }
                        else
                        {
                            ...
                            if (BackupDatabase(connStringAdmin))
                            {
                                ...
                                else
                                {
                                    ...
                                    dbContextTransaction.Rollback();
                                    throw new Exception("Backup Files deleting Error");
                                }
                            }
                            else
                            {

                                throw new Exception("Database Backup failed, Please try again");
                            }
                        }
                    }
                    else
                    {
                        throw new Exception("Please create Directory(folder) first for Backup.");
                    }
        ...
        //        #region Database Backup functionality
                ...
                //    DatabaseLogModel dbBackupLogDataFromClient = FromzaJSONConvert.DeserializeObject<DatabaseLogModel>(dataString);
                //    string backupDBFilePath = dbBackupLogDataFromClient.FolderPath + dbBackupLogDataFromClient.FileName;
                //    string backupDBFileVersion = dbBackupLogDataFromClient.DatabaseVersion;
                //    //Transaction Begin
                //    using (var dbContextTransaction = systemAdmindbContext.Database.BeginTransaction())
        ...
        {
            if (!System.Text.RegularExpressions.Regex.IsMatch(dbBackupLogDataFromClient.DatabaseName, @"^[a-zA-Z0-9_-]+$"))
            {
                throw new ArgumentException("Invalid Database Name.");
            }
            if (dbBackupLogDataFromClient.FolderPath.Contains("'") || dbBackupLogDataFromClient.FolderPath.Contains(";") ||
                dbBackupLogDataFromClient.FileName.Contains("'") || dbBackupLogDataFromClient.FileName.Contains(";"))
            {
            ...
            try
            {
                using (SqlConnection conAdmin = new SqlConnection(connStringAdmin))
                {
                    ...
                    using (SqlConnection conFromzaEMRLive = new SqlConnection(connString))
                    {
                        ...
                        SqlCommand sqlCmdSingleUserMode = new SqlCommand(sqlSetSingleUserModeQuery, conFromzaEMRLive);
                        ...
                        SqlCommand sqlCmdRestoreDB = new SqlCommand(sqlRestoreDBQuery, conFromzaEMRLive);
                        ...
                        SqlCommand sqlCmdMultiUserMode = new SqlCommand(sqlSetMultiUserModeQuery, conFromzaEMRLive);
