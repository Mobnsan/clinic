...
using FromzaEMR.ServerModel.AppointmentModels;
using FromzaEMR.Services.SSF;
using Microsoft.EntityFrameworkCore.Metadata.Internal;
using OfficeOpenXml.FormulaParsing.Excel.Functions.Math;
using Rank = FromzaEMR.ServerModel.AppointmentModels.Rank;
            ...
            try
            {
                if (clientTransaction.BillingTransactionItems != null && clientTransaction.BillingTransactionItems.Count > 0)
                {
                    ...
                    if (realTimeRemoteSyncEnabled)
                    {
                        ...
                        FromzaEMR.Controllers.VisitBL.SyncBillToRemoteServer(clientTransaction, "sales", visitDbContext);
                    }
                }
                else
                {
       ...
       */
        ...
        private int GetMaximumLastVisitDays()
        {
            ...
            if (parameter != null)
            {
                ...
                var data = FromzaJSONConvert.DeserializeObject<int>(paramValueStr);
                if (data > 0)
                {
