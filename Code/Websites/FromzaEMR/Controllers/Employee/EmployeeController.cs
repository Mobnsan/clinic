...
namespace FromzaEMR.Controllers
{
    ...
    public class EmployeeController : CommonController
    {
        ...
        [HttpGet]
        [Route("Profile")]
        public IActionResult GetEmployeeProfile(int empId)
        {
            //if (reqType == "employeeProfile")
            Func<object> func = () => EmployeeProfile(empId);
        ...
        public IActionResult GetActiveEmployeesInformation()
        {
            //if (reqType == "get-active-employees-info")
            Func<object> func = () => ActiveEmployeesInformation();
            return InvokeHttpGetFunction(func);
        }
        // POST api/values
        [HttpPost]
        [Route("LandingPage")]
        public IActionResult PostLandingPage()
        {
            //  if (reqType == "set-landing-page")
            string ipDataStr = this.ReadPostData();
            Func<object> func = () => LandingPage(ipDataStr);
            return InvokeHttpPostFunction(func);
        }


        private object EmployeeProfile(int empId)
        {

            EmployeeModel currEmp = (from e in _masterDbContext.Employees.Include("Department")
        ...
        private object ActiveEmployeesInformation()
        {
            var employeeList = (from e in _masterDbContext.Employees.Include("Department").Include("EmployeeRole").Include("EmployeeType")
        ...
        private object LandingPage(string ipDataStr)
        {

            RbacUser user = FromzaJSONConvert.DeserializeObject<RbacUser>(ipDataStr);
            if (user != null)
            {
                var currUser = _rbacdbContext.Users.Where(a => a.UserId == user.UserId).Select(a => a).FirstOrDefault();
                currUser.LandingPageRouteId = user.LandingPageRouteId;
                _rbacdbContext.Users.Attach(currUser);
                _rbacdbContext.Entry(currUser).Property(a => a.LandingPageRouteId).IsModified = true;
                _rbacdbContext.SaveChanges();
                return currUser.LandingPageRouteId;
            }
            else
            {
        ...
        //        //    RbacUser user = FromzaJSONConvert.DeserializeObject<RbacUser>(str);
        //        //    if (user != null)
        //        //    {
        ...
        //        {
        //            //
        //            FromzaHTTPResponse<object> uploadResponse = FileUploader.Upload(files, "UserProfile\\");
