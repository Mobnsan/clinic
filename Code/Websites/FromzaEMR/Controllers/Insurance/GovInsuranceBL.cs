...
namespace FromzaEMR.Controllers
{
    public class GovInsuranceBL
    {
             ...
             int? billingTransactionId = null)
        {
            ...
            BillingFiscalYear fiscYear = GovInsuranceBL.GetFiscalYear(connString);
            ...
            if (billingTransactionItems != null && billingTransactionItems.Count > 0)
            {
               // we are using this only for Provisional billing hence we can use first element to check billing status..
                ...
                {
                    ProvisionalReceiptNo = GovInsuranceBL.GetProvisionalReceiptNo(connString);
                }
            ...
            int? billingTransactionId = null)
        {
            ...
            insuranceDbContext.Entry(billItem).Property(a => a.PerformerId).IsModified = true;
            insuranceDbContext.Entry(billItem).Property(a => a.PerformerName).IsModified = true;
        ...
        public static void UpdateBillingTransactionItems(InsuranceDbContext insuranceDbContext, BillingTransactionItemModel txnItmFromClient)
        {
          ...
          .Where(itm => itm.BillingTransactionItemId == txnItmFromClient.BillingTransactionItemId).FirstOrDefault();
                        ...
                        insuranceDbContext.Entry(txnItmFromDb).Property(a => a.PerformerId).IsModified = true;
                        insuranceDbContext.Entry(txnItmFromDb).Property(a => a.PrescriberId).IsModified = true;
                        insuranceDbContext.Entry(txnItmFromDb).Property(a => a.PerformerName).IsModified = true;
            ...
            DateTime currentDate)
        {
                ...
                && billingTransaction.DepositReturnAmount != null && billingTransaction.DepositReturnAmount > 0)
            {
                BillingDepositModel dep = new BillingDepositModel()
                {
                    ...
                    OutAmount = (decimal)billingTransaction.DepositReturnAmount,
                    BillingTransactionId = billingTransaction.BillingTransactionId,
                    DepositBalance = (decimal)billingTransaction.DepositBalance,
        ...
        private static void GenerateInvoiceNoAndSaveInvoice(InsuranceDbContext dbContext, BillingTransactionModel billingTransaction, string connString)
        {
            try
            {
                billingTransaction.InvoiceNo = GovInsuranceBL.GetInvoiceNumber(connString);
        ...
        {
            int fiscalYearId = GetFiscalYear(connString).FiscalYearId;
            FromzaEMR.DalLayer.BillingDbContext billDbContext = new DalLayer.BillingDbContext(connString);
        ...
        //this function post IRD posting log details to Fromza IRD_Log table
        ...
        public static bool IsValidForDischarge(int patientId, int? patientVisitId, InsuranceDbContext insDbContext)
        {
            bool isValidForDischarge = true;

            //condition-1: Check if patient is admitted or not in Admission table.
            AdmissionModel admissionObj = insDbContext.Admissions.Where(adm => adm.PatientId == patientId
