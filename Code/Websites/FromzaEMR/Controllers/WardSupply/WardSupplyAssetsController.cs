...
using FromzaEMR.ViewModel.FixedAsset; //swapnil-2-april-2021
...
namespace FromzaEMR.Controllers
{
    public class WardSupplyAssetsController : CommonController
    {
        ...
        private readonly WardSupplyDbContext _wardDbContext;
        ...
        public WardSupplyAssetsController(IOptions<MyConfiguration> _config) : base(_config)
        {
            ...
            _wardDbContext = new WardSupplyDbContext(connString);
            _invDbContext = new InventoryDbContext(connString);
        ...
        //        {
        //            string Str = this.ReadPostData();
        //            WARDSupplyAssetRequisitionModel RequisitionFromClient = FromzaJSONConvert.
        ...
        public IActionResult PostStoreDispatch()
        {
            ...
            {
                string Str = this.ReadPostData();
                FixedAssetDispatchModel DispatchFromClient = FromzaJSONConvert.DeserializeObject<FixedAssetDispatchModel>(Str);
        ...
        public IActionResult AssetRequisition()
        {
            //   if (reqType == "AssetRequisition")
            string Str = this.ReadPostData();
            RbacUser currentUser = HttpContext.Session.Get<RbacUser>(ENUM_SessionVariables.CurrentUser);
            Func<object> func = () => AddAssetRequisition(Str, currentUser);
            return InvokeHttpPostFunction(func);

        }

        private object AddAssetRequisition(string Str, RbacUser currentUser)
        {

            WARDSupplyAssetRequisitionModel RequisitionFromClient = FromzaJSONConvert.DeserializeObject<WARDSupplyAssetRequisitionModel>(Str);
            List<WARDSupplyAssetRequisitionItemsModel> requisitionItems = new List<WARDSupplyAssetRequisitionItemsModel>();
            WARDSupplyAssetRequisitionModel requisition = new WARDSupplyAssetRequisitionModel();
            using (var dbContextTransaction = _wardDbContext.Database.BeginTransaction())
            {
                try
                {
                    //giving List Of RequisitionItems to requItemsFromClient because we have save the requisition and RequisitionItems One by one ..
                    ...
                    //removing the RequisitionItems from RequisitionFromClient because RequisitionItems will be saved later 
                    RequisitionFromClient.RequisitionItemsList = null;
                    var maxRequisitionList = _wardDbContext.WARDSupplyAssetRequisitionModels.ToList();
                    if (maxRequisitionList.Count() == 0)
                    {
                        RequisitionFromClient.RequisitionNo = 1;
                    }
                    ...
                    _wardDbContext.WARDSupplyAssetRequisitionModels.Add(requisition);
                    _wardDbContext.SaveChanges();
                    ...
                    requisitionItems.ForEach(item =>
                    {
                        ...
                        item.PendingQuantity = item.Quantity.Value;
                        item.IsActive = true;
                        _wardDbContext.WARDSupplyAssetRequisitionItemsModels.Add(item);

                    });
                    //this Save for requisitionItems
                    _wardDbContext.SaveChanges();
                    dbContextTransaction.Commit();
                    return RequisitionFromClient.RequisitionId;
                }
                catch (Exception ex)
                {
                    dbContextTransaction.Rollback();
