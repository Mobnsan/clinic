...
import { ENUM_FromzaHTTPResponses, ENUM_FromzaSSFSchemes, ENUM_MessageBox_Status, ENUM_SSF_EligibilityType, ENUM_Scheme_ApiIntegrationNames, ENUM_ServiceBillingContext } from "../../../shared/shared-enums";
...
export class RegistrationSchemeSelectComponent {
  ...
  GetLatestClaimCode(schemeId: number): void {
    this.visitBlService.GetLatestClaimCodeForAutoGeneratedClaimCodes(schemeId).subscribe((res: FromzaHTTPResponse) => {
  ...
  LoadMemberInformationByScheme(schemeId: number, currentPatientId: number): void {
    this.visitBlService.getMemberInfoByScheme(schemeId, currentPatientId).subscribe((res: FromzaHTTPResponse) => {
      if (res.Status === ENUM_FromzaHTTPResponses.OK) {
  ...
  LoadMedicarePatientInformation(patientId: number): void {
    this.visitBlService.getMedicareMemberDetail(patientId).subscribe((res: FromzaHTTPResponse) => {
      if (res.Status === ENUM_FromzaHTTPResponses.OK) {
      ...
      this.msgBoxService.showMessage(ENUM_FromzaHTTPResponses.Failed, ["Could not fetch the Medicare Member Detail"]);
    });
  }
  ...
  AssignSsfPatientData(ssfData: SsfDataStatus_DTO) {
    ...
    if (this.currentRegSchemeDto.SchemeName.toLowerCase() === ENUM_FromzaSSFSchemes.Medical.toLowerCase()) {
    ...
    if (eligibility && eligibility.length && this.currentRegSchemeDto.SchemeName.toLowerCase() === ENUM_FromzaSSFSchemes.Medical.toLowerCase()) {
    ...
    } else if (eligibility && eligibility.length && this.currentRegSchemeDto.SchemeName.toLowerCase() !== ENUM_FromzaSSFSchemes.Medical.toLowerCase()) {
