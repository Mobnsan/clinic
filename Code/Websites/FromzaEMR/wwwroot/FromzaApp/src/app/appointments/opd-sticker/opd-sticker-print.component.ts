...
import { ENUM_Country, ENUM_FromzaHTTPResponses, ENUM_FromzaHTTPResponseText, ENUM_MembershipTypeName, ENUM_PriceCategory } from "../../shared/shared-enums";
...
export class PrintStickerComponent {
  ...
  constructor(
    ...
    this.printerName = localStorage.getItem('Fromza_OPD_Default_PrinterName');
  ...
  CallBackStickerOnly(res) {
    if (res.Status === ENUM_FromzaHTTPResponses.OK && res.Results.length !== 0) {
      ...
      this.OpdStickerDetails = {...res.Results[0]};
      this.OpdStickerDetails.CountrySubDivisionName = res.Results[0].District;
...
Address: `+ this.OpdStickerDetails.Address;
  ...
  printStickerClient() {
   ...
   .dptdesc-left {
    ...
    documentContent += '<link rel="stylesheet" type="text/css" href="../../themes/theme-default/FromzaPrintStyle.css"/>';
  ...
  public PrintDotMatrix() {
    ...
    finalDataToPrint += "Name:" + (this.OpdStickerDetails.Rank ? this.OpdStickerDetails.Rank.substring(0,3)+ '.' : "") + this.OpdStickerDetails.PatientName + " " + this.ageSex + nline;
    ...
    finalDataToPrint += "Rank:"+(this.OpdStickerDetails.Rank ? this.OpdStickerDetails.Rank : "") + " "+ (this.OpdStickerDetails.MembershipTypeName ? "Type:"+ this.OpdStickerDetails.MembershipTypeName : "") + nline;
    if(this.OpdStickerDetails.SSFPolicyNo && this.OpdStickerDetails.MembershipTypeName === ENUM_MembershipTypeName.SSF){
        finalDataToPrint += "SSF Policy No: " + this.OpdStickerDetails.SSFPolicyNo + nline;
    }
    if(this.OpdStickerDetails.PolicyNo && this.OpdStickerDetails.MembershipTypeName === ENUM_MembershipTypeName.ECHS){
        finalDataToPrint += "ECHS No: " + this.OpdStickerDetails.PolicyNo + nline;
    }
    let address = '';
    if (this.OpdStickerDetails.CountryName === ENUM_Country.Nepal) {
      address = "Address: " + ((this.showMunicipality && this.OpdStickerDetails.MunicipalityName)? this.OpdStickerDetails.MunicipalityName + (this.OpdStickerDetails.WardNumber? "-"+ (this.OpdStickerDetails.WardNumber) : "") + ", " : "") + (this.OpdStickerDetails.CountrySubDivisionName) + nline;
    } else {
      address = "Address: " + (this.OpdStickerDetails.Address? this.OpdStickerDetails.Address + ", " : "") + (this.OpdStickerDetails.CountrySubDivisionName? this.OpdStickerDetails.CountrySubDivisionName + ", " : "") + this.OpdStickerDetails.CountryName + nline;
    }
