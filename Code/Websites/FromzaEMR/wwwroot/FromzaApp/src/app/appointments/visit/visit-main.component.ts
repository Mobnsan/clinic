...
import { ENUM_AppointmentType, ENUM_BillPaymentMode, ENUM_BillingType, ENUM_FromzaHTTPResponseText, ENUM_FromzaHTTPResponses, ENUM_InvoiceType, ENUM_MessageBox_Status, ENUM_OrderStatus, ENUM_Scheme_ApiIntegrationNames, ENUM_ServiceBillingContext, ENUM_VisitType } from '../../shared/shared-enums';
...
export class VisitMainComponent {
  ...
  constructor(public patientService: PatientService,
    ...
    this.Initialize();
    let TeleMedicineConfig = this.coreService.Parameters.find(p => p.ParameterGroupName == "TeleMedicine" && p.ParameterName == "FromzaConfigurationForTeleMedicine").ParameterValue;
  ...
  CheckExistingPatientsAndSubmit() {
    if (this.loading) {
      this.visitBLService.GetApptForDeptOnSelectedDate(this.quickVisit.Visit.DepartmentId, this.quickVisit.Visit.PerformerId, this.quickVisit.Visit.VisitDate, this.quickVisit.Patient.PatientId)
        .subscribe(res => {
          if ((res.Status === ENUM_FromzaHTTPResponseText.OK) && res.Results && this.restrictApptOnDepartmentLevel) {
  ...
  //   this.CreditLimit = 0;
  //   this.visitBLService.GetPatientCreditLimitsByScheme(schemeId, patientId, serviceBillingContext).subscribe((res: FromzaHTTPResponse) => {
  //     if (res.Status === ENUM_FromzaHTTPResponses.OK && res.Results) {
