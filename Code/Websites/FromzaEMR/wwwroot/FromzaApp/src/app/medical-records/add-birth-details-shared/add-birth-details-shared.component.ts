...
import { MasterType } from '../../shared/fromza-cache-service-utility/cache-services';
import { FromzaCache } from '../../shared/fromza-cache-service-utility/cache-services';
...
export class AddBirthDetailsSharedComponent {
    ...
    public CertificateNoBeforeEdit: number;
    constructor(
    ...
    ngOnInit() {
        ...
        this.providerList = FromzaCache.GetData(MasterType.Employee, null);
        this.GetBirthType();
    ...
    PatientListFormatter(data: any): string {
        let html: string = "";
        html = `<font size=03> ${data.PatientCode} </font>&nbsp;-&nbsp;&nbsp;<font color='blue'; size=03 ><b>${data.ShortName ? data.ShortName : ''} </b></font>&nbsp;&nbsp;(${data.Age ? data.Age : ''}/${data.Gender ? data.Gender : ''})</b></font>`;
        return html;
    }
    ...
    public SaveBirthDetail() {
        ...
        if (IsDataValid) {
            ...
            this.msgBoxServ.showMessage('Warning!', ["Invalid Inputs"]);
            this.loading = false;
        }
    }
    ...
    public BirthCertificateNumberDuplicationCheck() {
        ...
        let a = this.AllBirthCertificateNumbers.some(a => a.CertificateNumber == this.BirthDetail.CertificateNumber);
        let b = this.AllNewBirthDetails.some(b => b.CertificateNumber == this.BirthDetail.CertificateNumber);
