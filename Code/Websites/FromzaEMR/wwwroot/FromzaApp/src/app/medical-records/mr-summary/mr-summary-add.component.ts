...
import { MasterType, FromzaCache } from '../../shared/fromza-cache-service-utility/cache-services';
...
export class AddNewMedicalRecordComponent {
  ...
  public IcdVersionDisplayName: string = null;
  public allCasesMaster: any;
  public medicoLegalSubCases: any;
  showSubCase: boolean = false;
  mainCases: any;
  ...
  constructor(public MedicalRecordsBLService: MR_BLService,
    ...
    this.coreService.GetAllLookUpDetails(1).subscribe(res => {
      if (res.Status == "OK") {
        this.allCasesMaster = res.Results;
        this.mainCases = this.allCasesMaster.filter(x => x.ParentId == null);
        this.mainCases = this.mainCases.filter(x => x.Name != 'General');
      }
    });
  ...
  ngOnInit() {
    ...
    this.AllDoctors = FromzaCache.GetData(MasterType.Employee, null);
  ...
  public AssignDeliveryICD10() {
    if (this.allMasterDataForMR.AllICDCodes) {
      // Normal Deivery ICD10 code = JB20.Z
      let NICD10: ICD10 = this.allMasterDataForMR.AllICDCodes.find(di => di.ICD10Code == "JB20.Z");
      if (NICD10 && NICD10.ICD10ID > 0) {
        this.NormalDeliveryICD10 = NICD10;
      }
      // CS Deivery ICD10 code = JB22.Z
      let CsICD10 = this.allMasterDataForMR.AllICDCodes.find(cs => cs.ICD10Code == "JB22.Z");
      if (CsICD10 && CsICD10.ICD10ID > 0) {
        this.CSDeliveryICD10 = CsICD10;
      }
      // Forceps Deivery ICD10 code = JB21
      let FICD10 = this.allMasterDataForMR.AllICDCodes.find(f => f.ICD10Code == "JB21");
      if (FICD10 && FICD10.ICD10ID > 0) {
        this.ForcepsDeliveryICD10 = FICD10;
      }
      // Breech Deivery ICD10 code = JB23.1
      let BICD10 = this.allMasterDataForMR.AllICDCodes.find(b => b.ICD10Code == "JB23.1");
      if (BICD10 && BICD10.ICD10ID > 0) {
        this.BreechDeliveryICD10 = BICD10;
      }

      // Vaccum Deivery ICD10 code = JB21
      let VICD10 = this.allMasterDataForMR.AllICDCodes.find(v => v.ICD10Code == "JB21");
  ...
  public AssignCurrentDischargeType(dischargeTypeId: number) {
    ...
    } else if (this.RecordSummary.CurrentDischargeType.DischargeTypeName.toLowerCase() == 'referred' || this.RecordSummary.CurrentDischargeType.DischargeTypeName.toLowerCase() == 'referred out') {
  ...
  callBackForDeathDetails(parentData) {
    ...
    if (parentData.Status = 'Submit' || 'update' && parentData.data) {
  ...
  public AssignSubCase() {
    if (this.RecordSummary.CaseMain == "Medico-legal") {
      this.showSubCase = true;
      let subCategory = this.allCasesMaster.filter(x => x.Name == "Medico-legal" && x.DisplayName == "Medico-legal");
      this.medicoLegalSubCases = subCategory[0].ChildLookUpDetails;
    }
    else {
      this.showSubCase = false;
    }
  }
}
