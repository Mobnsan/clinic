...
export class MROutpatientListComponent {
      ...
      res => {
        if (res.Status == "OK") {
          ...
          this.ICD10DiseaseGroupList.forEach(a =>
            this.icd10DiagnosisFilteredList.push({ ICD10Code: a.ICDCode, ICD10Description: a.DiseaseGroupName }));
        }
        else {
  ...
  public OnReportingGroupChange() {
    ...
    if (this.selectedReportingGroupId == null) {
      ...
      this.ICD10DiseaseGroupList.forEach(a => {
        if (a.ReportingGroupId == this.selectedReportingGroupId) {
          this.icd10DiagnosisFilteredList.push({ ICD10Code: a.ICDCode, ICD10Description: a.DiseaseGroupName });
        }
      });
    }
  ...
  FilterList(departId: number = null, drId: number = null) {
    ...
    if (departId && drId) {
      ...
      this.filteredOutpatientList = this.OutpatientList.filter(a =>
        a.DepartmentId == departId && a.PerformerId == drId
      );
