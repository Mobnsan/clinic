...
export class GovInpatientOutcomeReportComponent {
  ...
  //   popupWindow.document.write('<html><head><link rel="stylesheet" type="text/css" href="../../../themes/theme-default/FromzaStyle.css" /></head><style type="text/css">.Selected {border-collapse: collapse; border-spacing: 5px; border: 1px solid black;padding: 5px;}</style><body onload="window.print()">' + printContents + '</body></html>');
  ...
  Success(res) {
    if (res.Status == "OK") {
      ...
      if (res.Results) {
        ...
        this.InpatientServiceSummary.InpatientOutcome.forEach(a => {
          if (a.DOR_Female || a.DOR_Male) {
            a.LAMA_Female = a.LAMA_Female + a.DOR_Female;
            a.LAMA_Male = a.LAMA_Male + a.DOR_Male;
          }
          if ((a.Not_Improved_Female || a.Not_Improved_Male) && (a.Not_Improved_Female > 0 && a.Not_Improved_Male > 0)) {
            this.oldData = true;
          }
        })
        ...
        this.InpatientServiceSummary.BroughtDead = this.InpatientServiceSummary.DeathSummary.filter((x => x.Condition == 'Brought Dead'));

        let indexOfBroughtDead = this.InpatientServiceSummary.DeathSummary.findIndex((x => x.Condition == 'Brought Dead'));
        this.InpatientServiceSummary.DeathSummary.splice(indexOfBroughtDead, 1);
        console.log(this.InpatientServiceSummary.DeathSummary);
        this.InpatientServiceSummary.SurgerySummary = JSON.parse(res.Results.SurgerySummary);
        this.InpatientServiceSummary.MedicoLegalCases = JSON.parse(res.Results.MedicoLegalCases);
  ...
  Print() {
    ...
    popupWindow.document.write(`<html><head>
      <link rel="stylesheet" type="text/css" href="../../../assets-dph/external/global/plugins/bootstrap/css/theme-default/Fromza_ui_style.css" />
      <link rel="stylesheet" type="text/css" href="../../../themes/theme-default/Fromza_ui_style.css" />
      <link rel="stylesheet" type="text/css" href="../../../themes/theme-default/FromzaPrintStyle.css" />
      </head>
      <style type="text/css">
