...
import { FromzaDateTime } from '../../shared/pipes/fromza-datetime.pipe';
...
export default class MRGridColumnSettings {
  static BirthList = [
    ...
    { headerName: "Weight(gm)", field: "WeightOfBaby", width: 75 },
    {
      ...
      field: "",
      width: 120,
      template: `<a fromza-grid-action="view-birth-certificate" class="grid-action">Certificate</a>`
    }
  ];
  static DeathList = [
    { headerName: "Certificate No.", field: "CertificateNumber", width: 150 },
    ...
    { headerName: "Death Time", field: "DeathTime", width: 120 },
    {
      ...
      field: "",
      width: 120,
      template: `<a fromza-grid-action="view-death-certificate" class="grid-action">Certificate</a>`
    }
  ];
  static datTime: FromzaDateTime = new FromzaDateTime();
  ...
  static OPListActionRenderer(params) {
    if (params.data && params.data.FinalDiagnosisCount && params.data.FinalDiagnosisCount > 0)
      return `<a fromza-grid-action="add-diagnosis" class="grid-action" style="background-color: #247e58!important;">
      Edit Final Diagnosis</a>`
    else return `<a fromza-grid-action="add-diagnosis" class="grid-action">Add Final Diagnosis</a>`
  }
  ...
  static IPListActionRenderer(params) {
    if (params.data && params.data.MedicalRecordId > 0)
      return `<a fromza-grid-action="view-mr" class="grid-action btn btn-primary actionbtn margin-7-hr" style="background-color: #247e58!important;">
      View MR</a>`
    else return `<a fromza-grid-action="add-mr" class="grid-action btn btn-primary actionbtn margin-7-hr">Add MR</a>`;
