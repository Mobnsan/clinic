...
export class BirthCertificateComponent {
  ...
  public BirthCertificateParam:any = {"ShowFooter": true, "BirthCertificateFooterContent": "[]" };
  public GetBirthCertificateParamter(){
    let params = this.coreService.Parameters.find(a => a.ParameterGroupName == "ADT" && a.ParameterName == "BirthCertificateFooter");
    if (params) {
      let data = params.ParameterValue;
      let actualParams = JSON.parse(data);
      this.BirthCertificateParam = actualParams && actualParams;
    }
  }
  public GetBirthDetailForCertificate() {
    this.medicalRecordsBLService.GetBirthDetailForCertificate(this.babyBirthDetailId).subscribe(res => {
      if (res.Status == 'OK') {
        ...
        this.amPm = Number(this.CurrentBaby.BirthTime.match(/^(\d+)/)[1]) >= 12 ? 'PM' : 'AM';
