...
import { ENUM_FromzaHTTPResponses, ENUM_DateFormats, ENUM_MessageBox_Status } from '../../shared/shared-enums';
...
export class InsuranceClaimsPreviewComponent {
    ...
    public async GetClaimDetailsForPreview(claimSubmissionId: number): Promise<void> {
        try {
            const res: FromzaHTTPResponse = await this.claimManagementBlService.ClaimDetailsForPreview(claimSubmissionId).toPromise();
