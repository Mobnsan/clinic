...
export class LabRequestsComponent implements OnInit {
  ...
  GetBillingCounterForLab() {
    ...
    allBilCntrs = FromzaCache.GetData(MasterType.BillingCounter, null);
  ...
  SetVisitDetailToTransaction() {
    if (this.visitList && this.visitList.length) {
      this.billingTransaction.BillingTransactionItems.forEach(billItem => {
        ...
        billItem.BillingTransactionItemValidator.controls['PrescriberId'].setValue(this.visitList[0].PerformerId);
        billItem.PrescriberId = this.visitList[0].PerformerId;
      ...
      for (var j = 0; j < this.billingTransaction.BillingTransactionItems.length; j++) {
        ...
        if (isFormValid) {
          for (var j = 0; j < this.billingTransaction.BillingTransactionItems.length; j++) {
            ...
            let visit = this.visitList.find(a => a.PerformerId == this.billingTransaction.BillingTransactionItems[j].PrescriberId)
            if (visit)
