...
@Component({
  selector: "fromza-lab-add-result",
...
export class LabTestsAddResultComponent {
  ...
  public set tempReport(_templateReport: LabReportVM) {
    if (_templateReport) {
      ...
      this.AutoCalculationConfigList = [];
  ...
  CheckComponentValueIsValid(test, comp: LabTestComponent, type: string, i: number, j: number, k: number) {
    comp.IsSelected = true;
    if (comp && (comp.Value == null || comp.Value == "" || comp.Value.trim() == "")) {
  ...
  public AutoCalculationTargetsArrayCalculate(autoCalculationTargets: Array<LabResult_TemplatesVM>) {
    autoCalculationTargets.forEach(template => {
      template.Tests.forEach(test => {
        test.ComponentJSON.forEach(component => {
          const currItem = component;

          if (currItem.IsAutoCalculate === true) {
            const currentFormula = currItem.CalculationFormula;
            const compIdsInFormula = this.GetComponentIdFromFormula(currentFormula);

            for (let j = 0; j < compIdsInFormula.length; j++) {
              const newAutoCalcObj: AutoCalculationConfig = new AutoCalculationConfig();

              newAutoCalcObj.LabTestId = test.LabTestId;
              newAutoCalcObj.TargetComponentId = currItem.ComponentId;
              newAutoCalcObj.SourceComponentId = +compIdsInFormula[j];
              newAutoCalcObj.AllSourcesId = compIdsInFormula;
              newAutoCalcObj.Formula = currentFormula;

              this.AutoCalculationConfigList.push(newAutoCalcObj);
            }
          }
        })
    ...
    return this.AutoCalculationConfigList;
  }
  GetComponentIdFromFormula(Formula: string): number[] {
    const regex = /(?:\{(\d+)\}|\((\d+)\))/g;
  ...
  CheckForAutoCalculation(test, component, ComponentList) {
    const matchingConfig = this.AutoCalculationConfigList.filter(
      c => c.LabTestId === test.LabTestId && c.SourceComponentId === component.ComponentId
    );
    matchingConfig.forEach(config => {
      const { TargetComponentId, AllSourcesId, Formula } = config;

      const allSourcesHaveValues = AllSourcesId.every(id => {
        let comp = ComponentList.find(c => c.ComponentId === id);
        if (comp && comp.Value !== null && comp.Value !== "") {
          return true;
        }
        else {
          return false;
      ...
      const targetComponent = ComponentList.find(c => c.ComponentId === TargetComponentId);
      if (allSourcesHaveValues) {
        let parsedFormula = Formula;
        for (const id of AllSourcesId) {
          const sourceComponent = ComponentList.find(c => c.ComponentId === id);
          let sourceValue = "";
          if (sourceComponent) {
            sourceValue = parseFloat(sourceComponent.Value).toString();
          }

          parsedFormula = parsedFormula.replace(`{${id}}`, sourceValue);
        }

        const result = eval(parsedFormula);
        if (targetComponent) {
          const formattedResult = result.toFixed(2);
          const wholeNumberPart = parseInt(formattedResult);

          if (formattedResult.endsWith('.00')) {
            targetComponent.Value = wholeNumberPart.toString();
          } else {
            targetComponent.Value = formattedResult;
          }

          targetComponent.IsValueValid = true;
        }
      }
      else {
        if (targetComponent) {
