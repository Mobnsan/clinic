...
export class LISMappingAddComponent {
        ...
        this.notMappedDataListRequest = this.labLISBlService.GetAllLISNotMappedDataByMachineId(this.selectedMachineId, this.selectedMapId).subscribe(res => {
            if (res.Status == "OK") {
                ...
                if (this.isEdit) {
                    ...
                    renderedData.FromzaComponent = this.notMappedDataList.find(d => d.ComponentId == this.existingMapping.ComponentId);
    ...
    Save() {
        ...
        //get those rows that have either Fromza Component or LIS Component or both
        this.compMappingArrayTemp = this.compMappingArrayTemp.filter(d => (d.FromzaComponent && d.FromzaComponent.ComponentId) || (d.LISComponent && d.LISComponent.LISComponentMasterId));
        ...
        this.compMappingArrayTemp.forEach(d => {
            ...
            let validComp = d.FromzaComponent && (typeof d.FromzaComponent == "object") && d.FromzaComponent.ComponentId;
        ...
        if (isDataValid) {
            let data = this.compMappingArrayTemp.filter(d => d.IsValidComponent && d.IsValidLISComponent).map(dt => {
                ...
                newObj.ComponentId = dt.FromzaComponent.ComponentId;
    ...
    public CheckDuplicationAndUpdateFlag() {
        ...
        this.compMappingArrayTemp.forEach(v => {
            if (v.FromzaComponent && (typeof (v.FromzaComponent) == 'object') && v.FromzaComponent.ComponentId) {
                if (v.FromzaComponent.ComponentId in mappedElem) {
                    mappedElem[v.FromzaComponent.ComponentId]++;
                } else { mappedElem[v.FromzaComponent.ComponentId] = 1; }
            }
        });
        ...
        this.compMappingArrayTemp.forEach(m => {
            m.IsDuplicate = false;
            if (m.FromzaComponent && allDupIds.includes(m.FromzaComponent.ComponentId)) {
                m.IsDuplicate = true;
            }
        });
    }
    ...
    public MachineChanged() {
        let allValidCompMap = this.compMappingArrayTemp.filter(c => c.FromzaComponent && c.LISComponent && c.FromzaComponent.ComponentId && c.LISComponent.LISComponentMasterId);
