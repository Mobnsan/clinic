...
export class SignatoriesComponent {
  ...
  @Output('isSignatureSelected')
  public selectedEmployee = new EventEmitter<boolean>();
  constructor(
  ...
  LoadEmployees() {
    this.http
      .get<any>(
        "/api/Master/Signatories?departmentName=" + this.departmentName,
        this.options
      )
      .map((res) => res)
  ...
  onItemSelect(item: any) {
    const isSignatureConsist = this.selEmployees.some(items => this.allEmployees.some(item => item.EmployeeId === items.id));
    if (!isSignatureConsist) {
  ...
  OnItemDeSelect(item: any) {
    const index = this.selEmployees.indexOf(item);
    if (index >= 0) {
