...
export class LabVendorAddComponent {
  ...
  ngOnInit() {
    if (this.actionName == "add") {
      ...
      //All vendors are Active=true, External=false and IsDefault=False.
      //External=False and IsDefault=true can be made only from database. not from UI.
      //Since those huge impact in the Lab workflow itself.
      this.newVendor.IsExternal = true;
      this.newVendor.IsDefault = false;
      this.newVendor.IsActive = true;
    }
    else {
