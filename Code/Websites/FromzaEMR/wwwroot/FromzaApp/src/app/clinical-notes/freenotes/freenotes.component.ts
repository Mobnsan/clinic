...
export class FreeNotesComponent implements OnInit, OnDestroy {
  ...
  constructor(public visitService: VisitService,
    ...
    this.patVisit.PerformerId = this.patVisit.PerformerId;
    this.patVisit.PerformerName = this.patVisit.PerformerName;
        ...
        .subscribe(res => {
          if (res.Status == "OK") {
            ...
            if (this.notes && this.notes.SubjectiveNote && this.notes.ClinicalPrescriptionNote) {
              this.notes.ClinicalPrescriptionNote.ChiefComplaint = JSON.parse(this.notes.SubjectiveNote.ChiefComplaint);
            ...
            if (this.DoctorList.length > 0 && this.notes.SecondaryDoctorId) {
              this.selectedSecondaryDoctor = this.DoctorList.find(doc => doc.PerformerId == this.notes.SecondaryDoctorId);
  ...
  PostTemplate() {
    ...
    this.notes.SecondaryDoctorId = this.selectedSecondaryDoctor ? this.selectedSecondaryDoctor.PerformerId : null;
    this.notes.PerformerId = this.patVisit.PerformerId ? this.patVisit.PerformerId : 0;
    ...
    if (this.notes.TemplateId == 2 || this.notes.TemplateName == "History & Physical" || this.notes.TemplateName == "Consult Note") {
    ...
    if (this.notes.TemplateName == "Prescription Note") {
      ...
      if (this.notes.IsPending) {
        ...
        if ((this.notes.SubjectiveNote.ReviewOfSystems && this.notes.SubjectiveNote.ReviewOfSystems.trim().length)
          ...
          && (this.notes.SubjectiveNote.ChiefComplaint && this.notes.SubjectiveNote.ChiefComplaint.length)) {
          this.PostPrescriptionNote();
        } else {
          this.msgBoxServ.showMessage("Warning", ["Submit can't be done with fields empty !"]);
        }
  ...
  PutTemplate() {
    ...
    // this.notes.ClinicalPrescriptionNote.PatientVisitId = this.patVisit.PatientVisitId;
    this.notes.TemplateId = this.selTemplateList.TemplateId;
    if (this.selectedNoteType == undefined) {
      this.msgBoxServ.showMessage(ENUM_MessageBox_Status.Warning, ["Please Select Note Type"]);
      return;
    }
    this.notes.NoteTypeId = this.selectedNoteType.NoteTypeId;
    //this.notes.TemplateName = this.templateList.find(a => a.TemplateId == this.TemplateId).TemplateName;
    this.notes.TemplateName = this.selTemplateList.TemplateName;
    this.notes.SecondaryDoctorId = this.selectedSecondaryDoctor.PerformerId;
    ...
    if (this.notes.TemplateId == 2 || this.notes.TemplateName == "History & Physical") {
      ...
      if (this.notes.IsPending) { // if notes are are pending then, empty fields are allowed to post 
        ...
        if (this.notes.SubjectiveNote && this.notes.ObjectiveNote) { //&& this.notes.AllIcdAndOrders.length > 0 This is removed
    ...
    if (this.notes.TemplateId == 3 || this.notes.TemplateName == "Consult Note") {
      //this.msgBoxServ.showMessage("Warning", ["Update of History and physical is work in progress !"]);
  ...
  public ManagePrescriptionData() {
    ...
    this.notes.SubjectiveNote.ChiefComplaint = JSON.stringify(this.notes.ClinicalPrescriptionNote.ChiefComplaint);
  }
    ...
    this.notetemplateBLService.PutHistoryAndPhysicalNote(this.notes)
      .subscribe(res => {
        if (res.Status == "OK") {
          ...
          this.msgBoxServ.showMessage("Failed", ["Error while Saving History and Physical Note!"]);
          console.log(res.ErrorMessage);
        }
      });
  ...
  public PutConsultNote() {

    this.notetemplateBLService.PutHistoryAndPhysicalNote(this.notes)
      .subscribe(res => {
        if (res.Status == "OK") {
          this.msgBoxServ.showMessage("Success", ["Consult Note Saved !"]);
          this.RouteToViewNotes();
        }
        else {
          this.msgBoxServ.showMessage("Failed", ["Error while Saving Consult Note !"]);
          console.log(res.ErrorMessage);
        }
      });
        ...
        if (itm.IsEditable) {
          ...
          itm.OrdersList.forEach(odr => {
            if (odr.Order.PreferenceType.toLowerCase() == 'lab') {
              ...
              currReq.PrescriberName = this.visitService.globalVisit.PerformerName;
              currReq.PrescriberId = this.visitService.globalVisit.PerformerId;
