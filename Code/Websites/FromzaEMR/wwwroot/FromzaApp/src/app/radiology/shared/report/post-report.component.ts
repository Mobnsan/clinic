...
@Component({
  selector: "fromza-post-report",
  templateUrl: "./post-report.html"
  //,
  //styleUrls: ['themes/theme-default/FromzaStyle.css']
...
export class PostReportComponent {
  ...
  public set value(val: boolean) {
    try {
      ...
      if (this.showreport) {
        ...
        this.selectedPrescriberId = this.report.PrescriberId;
        this.selectedPrescriberName = this.report.PrescriberName ? this.report.PrescriberName : "Self";

        //this.GetReportingDoctor();
        if (this.report && this.report.ProviderName) {
          this.selPrescriberDr = { EmployeeId: this.report.ReportingDoctorId, FullName: this.report.ProviderName };
          //this.setReferredByDr();
        } else {
          this.selPrescriberDr = { EmployeeId: null, FullName: 'Self' };
        }
  ...
  AddReport(orderStatus) {
    try {
      //this.AssignPrescriberDr();

      this.report.PrescriberName = this.selectedPrescriberName ? this.selectedPrescriberName : 'self';
      this.report.PrescriberId = this.selectedPrescriberId ? this.selectedPrescriberId : 0;
      this.report.PerformerId = this.report.PerformerId;
      this.report.PerformerName = this.report.PerformerName;
      ...
      if (this.report.ReportText) {
        ...
        if (this.CheckIfSignatureValid()) {          
          ...
          if (this.enableDoctorUpdateFromSignatory) {
            ...
            this.report.PerformerIdInBilling = this.report.PerformerNameInBilling = null;
            if (signData && signData.length == 1) {
              this.report.PerformerIdInBilling = signData[0].EmployeeId;
              this.report.PerformerNameInBilling = signData[0].EmployeeFullName;
            }
          }
  ...
  public AssignPrescriberDr() {
    this.report.PrescriberName = this.selectedPrescriberName;
    this.report.PrescriberId = this.selectedPrescriberId;
