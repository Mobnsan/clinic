...
@Component({
  templateUrl: './er-discharged-patient-list.html'
})

// App Component class
export class ERDischargedPatientListComponent {
  ...
  public GetERDischargedPatientList() {
    var id = this.caseIdList ? this.caseIdList : null;
    this.emergencyBLService.GetAllDischargedERPatients(id[0])
      .subscribe(res => {
        if (res.Status == "OK") {
  ...
  public SetPatDataToGlobal(data): void {
    ...
    this.globalVisit.ERTabName = "finalized-discharged";
