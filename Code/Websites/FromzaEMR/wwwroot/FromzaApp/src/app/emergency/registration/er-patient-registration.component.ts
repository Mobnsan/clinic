...
import { ENUM_FromzaHTTPResponses, ENUM_MessageBox_Status, ENUM_Scheme_ApiIntegrationNames, ENUM_ServiceBillingContext } from '../../shared/shared-enums';
...
export class ERPatientRegistrationComponent {
  ...
  RegisterNewERPatient(): void {
    ...
    if (this.loading) {
      ...
      if (this.ERPatient.IsValid(undefined, undefined) && this.ERPatient.EthnicGroup) {
        ...
        this.loading = false;
        this._messageBoxService.showMessage(ENUM_FromzaHTTPResponses.Failed, ["One or more validation error occurred"]);
      }
    }
  }
  ...
  GetCountrySubDivision(): void {
    ...
    this._emergencyBLService.GetCountrySubDivision(countryId)
      .subscribe((res: FromzaHTTPResponse) => {
        if (res.Status === ENUM_FromzaHTTPResponses.OK && res.Results.length) {
