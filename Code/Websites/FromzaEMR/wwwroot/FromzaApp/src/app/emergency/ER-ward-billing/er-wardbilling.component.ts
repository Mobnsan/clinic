...
import { ENUM_FromzaHTTPResponses, ENUM_MessageBox_Status, ENUM_ServiceBillingContext } from '../../shared/shared-enums';
...
export class ERWardBillingComponent {
  ...
  GetActionList(params): string {
    if (params.data.AllowCancellation) {
      return `<a fromza-grid-action="cancel" class="grid-action btn btn-danger">
              Cancel
           </a>`;
    } else {
  ...
  GetBillingCounterForEmergency(): void {
    ...
    allBillingCounters = FromzaCache.GetData(MasterType.BillingCounter, null);
