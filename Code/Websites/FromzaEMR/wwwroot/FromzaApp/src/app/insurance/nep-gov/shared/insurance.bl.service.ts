...
import { GovInsuranceDlService } from "./insurance.dl.service";
@Injectable({
  providedIn: "root",
})
export class GovInsuranceBlService {
  constructor(public insuranceDLService: GovInsuranceDlService,
  ...
  public UpdateInsBalance(patientId: number, insuranceProviderId: number, updatedInsBalance: number, remark: string) {
    let insHistory: GovInsuranceBalanceAmountHistory = new GovInsuranceBalanceAmountHistory();
  ...
  public UpdateAppointmentStatus(appointmentId: number, status: string, performerId: number, performerName: string) {
    return this.insuranceDLService.PutAppointmentStatus(appointmentId, status, performerId, performerName)
      .map((responseData) => {
        return responseData;
      });
  }
  ...
  public GetVisitItemsMapped(billItems: Array<BillingTransactionItem>, orderStatus: string, billStatus: string): string {
    ...
    if (billItems && billItems.length) {
      ...
      billItems.forEach(bill => {
        ...
        let doctorList = FromzaCache.GetData(MasterType.Employee, null);
