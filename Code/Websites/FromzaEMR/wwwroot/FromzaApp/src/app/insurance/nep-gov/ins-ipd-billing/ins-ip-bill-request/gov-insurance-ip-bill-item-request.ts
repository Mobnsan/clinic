...
@Component({
  selector: 'ins-ip-bill-item-request',
  templateUrl: './gov-insurance-ip-bill-item-request.html'
})
export class GovInsuranceIpBillItemRequest implements OnInit {
  ...
  constructor(
    ...
    this.LoadDefaultDiscountScheme();//sud:10-Oct'21
  ...
  ResetVariables() {
    ...
    this.taxDetail.taxId = this.insuranceService.taxId;
    this.taxDetail.taxPercent = this.insuranceService.taxPercent;
  ...
  printInvoice() {
    ...
    txnReceipt.Patient = this.patientInfo; //Object.create(this.patientInfo);
  ...
  SetBillingTxnDetails() {
    ...
    this.billingTransaction.BillingTransactionItems.forEach(txnItem => {
      ...
      txnItem.BillingType = this.insuranceService.BillingType;
      txnItem.VisitType = this.insuranceService.BillingType.toLowerCase();
      ...
      txnItem.DiscountSchemeId = this.defDiscountSchemeId;//sud:10-Oct'21--setting default (General) scheme id.
  ...
  PostToDepartmentRequisition() {
    ...
    this.insuranceBlService.PostDepartmentOrders(this.billingTransaction.BillingTransactionItems, "active", "provisional", true, this.currPatVisitContext)
      .subscribe(res => {
        if (res.Status == "OK") {
  ...
  PostToBillingTransaction() {
    ...
    this.insuranceBlService.PostBillingTransactionItems(this.billingTransaction.BillingTransactionItems)
      .subscribe(
        res => {
          if (res.Status == "OK") {
  ...
  public SetBillingItemsNPrices() {
    this.billItemsComplete = this.insuranceService.allBillItemsPriceList;
  ...
  public AssignSelectedItem(index) {
    ...
    if (this.selectedItems[index]) {
      ...
      if (item) {
        ...
        this.billingTransaction.BillingTransactionItems[index].UpdateValidator("off", "PrescriberId", null);
      }
      else
  ...
  AddNewBillTxnItemRow(index = null) {    //method to add the row
    ...
    if (index != null) {
      ...
      this.billingTransaction.BillingTransactionItems[new_index].UpdateValidator("on", "Quantity", "invalidNumber");
    }
  }
  NewBillingTransactionItem(index = null): BillingTransactionItem {
    let billItem = new BillingTransactionItem();
    billItem.DiscountSchemeId = this.defDiscountSchemeId;//sud:10-Oct'21: Using default discount schemeid 
    billItem.Quantity = 1;
    billItem.ItemList = this.billItems;
    billItem.UpdateValidator("on", "Quantity", "invalidNumber");
    return billItem;
  }
  ...
  CheckItemProviderValidation(index: number) {
    ...
    // }
    if (this.selectedItems[index] && this.selectedItems[index].IsDoctorMandatory) {
      this.billingTransaction.BillingTransactionItems[index].UpdateValidator("on", "PrescriberId", "required");
    }
    else {
      this.billingTransaction.BillingTransactionItems[index].UpdateValidator("off", "PrescriberId", null);
    }
