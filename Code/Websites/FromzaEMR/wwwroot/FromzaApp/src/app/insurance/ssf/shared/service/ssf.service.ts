...
import { ENUM_FromzaHTTPResponses, ENUM_DateFormats } from "../../../../shared/shared-enums";
...
export class SsfService {
  ...
  GetSsfPatientDetailAndEligibilityFromSsfServer(policyNo: string, loadFromSSFServer: boolean) {
    if (policyNo !== null) {
      ...
      this.visitBlService.getSSFPatientDetailAndCheckSSFEligibilityFromSsfServer(policyNo, currentDate).subscribe((res: Array<FromzaHTTPResponse>) => {
        if (res) {
          console.log(res);
          //* res will have an Array of FromzaHttpResponse where 0 index is for SSF Patient Detail and 1 index is for Eligibility
  ...
  GetSsfPatientDetailAndEligibilityLocally(patientId: number, schemeId: number) {
    this.visitBlService.getSSFPatientDetailLocally(patientId, schemeId).subscribe((res: FromzaHTTPResponse) => {
  ...
  LoadSSFEmployer(policyHolderUid: string) {
    this.visitBlService.GetSSFEmployerDetail(policyHolderUid).subscribe((res: FromzaHTTPResponse) => {
      if (res.Status === ENUM_FromzaHTTPResponses.OK && res.Results) {
...
export class SSFBackupClass {
  ...
  //     this.SSFEligibility = [];
  //     this.visitBLService.getSSFPatientDetailLocally(this.patient.PatientId).subscribe((res: FromzaHTTPResponse) => {
  //       if (res.Status === ENUM_FromzaHTTPResponses.OK) {
