...
@Component({
  ...
  templateUrl: "./gov-insurance-ip-patient-summary.html"
})
export class GovInsurancePatientIpSummaryComponent {
  ...
  constructor(public dlService: DLService,
    ...
    this.IPBillItemGridCol = (new GovINSGridColumnSettings(this.CoreService)).IPBillItemGridCol;
    ...
    this.insuranceBlService.PostIpBillingTransaction(this.billingTransaction)
      .subscribe((res: FromzaHTTPResponse) => {
        if (res.Status == "OK") {
          ...
          this.BackToPatientListGrid();//redirect back to patient list if transaction failed. happens when patient is already discharged from other window/other user.
          console.log(res.ErrorMessage);
          this.loading = false;
        }
      });
