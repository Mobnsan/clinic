...
import { ENUM_BillingType, ENUM_VisitType } from "../../../../shared/shared-enums";
...
export class GovInsuranceVisitBillingInfoComponent implements OnInit {
        ...
        if (this.insuranceService.appointmentType.toLowerCase() == "followup") {
          ...
          if (newBill.ChangeType == "Country") {
            ...
            if (this.opdBillTxnItem)
              this.opdBillTxnItem.PrescriberId = newBill.ReferredBy;
          }
  ...
  ngOnInit() {
    ...
    if (this.routeFromService && this.routeFromService.RouteFrom && this.routeFromService.RouteFrom == "appointment") {
      ...
      let apptDocId: number = this.insuranceService.insGlobalAppointment.PerformerId;
      if (apptDocId) {
        this.insuranceService.TriggerBillChangedEvent({ ChangeType: "Doctor", SelectedDoctor: { PerformerId: apptDocId } });
      }
    }
  }
  ...
  AssignVisitBillItemToTxn(billItem) {
    ...
    this.opdBillTxnItem.PerformerId = billItem.PerformerId;
    this.opdBillTxnItem.PerformerName = billItem.PerformerName;
  ...
  GetPrevVisitBillDetails() {
    ...
    if (prevVisId) {
      this.insuranceBLService.GetOpdTicketInvoiceInfo(patId, prevVisId)
        .subscribe(res => {
  ...
  CalculateAmountForReturnCase() {
    ...
    let itemId = this.insuranceService.ParentVisitInfo.PerformerId;
  ...
  HandleBillChangedForFollowUp(newBill) {
    ...
    let oldDoctorId = this.insuranceService.ParentVisitInfo.PerformerId;
    let newDeptId = this.insuranceService.globalVisit.DepartmentId;
    let newDoctId = this.insuranceService.globalVisit.PerformerId;
  ...
  Followup_DoctorChanged(newDoctId) {
    if (this.billingTransaction && this.billingTransaction.BillingTransactionItems.length) {
      this.visitBillItem = this.docOpdPriceItems.find(d => d.PerformerId == newDoctId);
      this.ResetOpdBillTxnItem();
      if (this.visitBillItem) {
        this.AssignVisitBillItemToTxn(this.visitBillItem);
      }
  ...
  public AssignAdditionalBillItemToBillTxn(row: AdditionalBillItemVMModel) {
    if (row.ItemId && row.ItemName && (this.billingTransaction.BillingTransactionItems.some(a => a.ItemId == row.ItemId && a.ItemName == row.ItemName) ? false : true)) {
      ...
      billItem.PerformerId = this.opdBillTxnItem.PerformerId;
      billItem.PerformerName = this.opdBillTxnItem.PerformerName;
