...
import { ENUM_FromzaHTTPResponses, ENUM_InvoiceType, ENUM_MessageBox_Status, ENUM_OrderStatus, ENUM_VisitType } from '../../../../shared/shared-enums';
...
export class GovInsuranceVisitMainComponent {
  ...
  //sud:22Jan'23--Commented since it's not used in either this component or its html.
  ...
  CheckValidations(): { isValid: boolean, message: Array<string> } {
    ...
    if (this.quickVisit.BillingTransaction.PaymentMode == "credit") {
      ...
      // } 
      if (this.quickVisit.BillingTransaction.OrganizationId == 0) {
        validationSummary.isValid = false;
  ...
  CheckAppointmentTypeAndCreateVisit() {
    if (this.quickVisit.Visit.AppointmentType.toLowerCase() == "transfer" && this.insuranceService.ParentVisitInvoiceDetail) {
      ...
      this.quickVisit.Visit.PrescriberId = this.insuranceService.ParentVisitInfo.PerformerId;//this is provider id of last visit.

      this.ReturnPreviousVisitBillingTxnAndCreateVisit();
    }
    else if (this.quickVisit.Visit.AppointmentType.toLowerCase() == "referral") {
      this.quickVisit.Visit.ParentVisitId = this.insuranceService.ParentVisitInfo.PatientVisitId;
      this.quickVisit.Visit.ReferredById = this.insuranceService.ParentVisitInfo.PerformerId;//this is provider id of last visit.
      this.CreateVisit();
    }
    else {
      this.CreateVisit();
  ...
  public AssignValuesToBillTxn() {
    ...
    if (this.previousVisitBillingTxn) {
      ...
      if (this.previousVisitBillingTxn.BillingTransactionItems.length > 0) {
        ...
        let parentVisitDocId = this.insuranceService.ParentVisitInfo.PerformerId;
      ...
      .subscribe(
        (res: FromzaHTTPResponse) => {
          if (res.Status === ENUM_FromzaHTTPResponses.OK) {
            ...
            let bilTxn = result.Results.BillingTransaction;
            ...
            this.selectedVisit = result.Results.Visit;
        ...
        err => {
          this.loading = false;
          this.msgBoxServ.showMessage(ENUM_MessageBox_Status.Failed, ["Unable to create visit"]);
  ...
  UpdateAppointmentStatus() {
    ...
    if (appointment.AppointmentId) {
      try {
        this.insuranceBLService.UpdateAppointmentStatus(appointment.AppointmentId, "checkedin", performerId, performerName)
          .subscribe(res => {
            if (res.Status == "OK") {
