...
@Component({
  ...
  templateUrl: './gov-ins-patient-registration.html'
})

// App Component class
export class GovINSPatientRegistrationComponent {
  ...
  Initialize() {
    ...
    this.Country_All = FromzaCache.GetData(MasterType.Country, null);
    this.districts_All = FromzaCache.GetData(MasterType.SubDivision, null);
  ...
  RegisterInsurancePatient() {
    ...
    if (!this.insPatient.EthnicGroup) {
      this.msgBoxServ.showMessage(ENUM_FromzaHTTPResponses.Failed, ["Ethnic group is mandatory"]);
      this.validCheck = false;
      this.disableButton = false;
      return;
    }
  ...
  public updateMunicipality(event) {
    if (event) {
      this.insPatient.MunicipalityId = event.data.MunicipalityId;
    }
  }
  ...
  onEthnicGroupChangeCallBack(ethnicGroup) {
    if (ethnicGroup) {
      this.insPatient.EthnicGroup = ethnicGroup.ethnicGroup;
    }
  }
}
