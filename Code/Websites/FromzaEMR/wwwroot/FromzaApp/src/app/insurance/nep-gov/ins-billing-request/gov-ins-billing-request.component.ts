...
export class GovInsBillingRequestComponent {
  ...
  NewBillingTransactionItem(): BillingTransactionItem {
    ...
    billTxnItem.UpdateValidator("off", "PrescriberId", "required");
    return billTxnItem;
  }
      ...
      .subscribe(res => {
        if (res.Status == 'OK') {
          if (res.Results.length) {
            ...
            if (this.model.BillingTransactionItems.length) {
              this.currentRequestedByDoctor = this.visitList[0].PerformerName == "Duty Doctor" ? "SELF" : this.visitList[0].PerformerName;
      ...
      this.model.BillingTransactionItems.forEach(element => {
        if (element.IsZeroPriceAllowed == false || element.IsZeroPriceAllowed == null) {
          if (!(element.Price != null && element.Price > 0)) {
            this.messageBoxService.showMessage("error", ["Price of Item cannot be zero (0)"]);
          }
        }
      });
      if (this.model.BillingTransactionItems) {
  ...
  AssignBillTxnItemsValuesForSubmit() {
    for (var j = 0; j < this.model.BillingTransactionItems.length; j++) {
      ...
      if (integrationName == "LAB") {
        ...
        this.model.BillingTransactionItems[j].OrderStatus = ENUM_OrderStatus.Active;//sud:25Aug'22--this value was missing from long time.
      }
      ...
      if (doctor) {
        ...
        if (this.model.BillingTransactionItems) {
          this.model.BillingTransactionItems.forEach(billTxnItem => {
            billTxnItem.PrescriberId = this.currentRequestedByDoctor.EmployeeId;
            billTxnItem.PrescriberName = this.currentRequestedByDoctor.FullName;
    ...
    if (this.insuranceService.BillingFlow == "insurance") {
      ...
      if (this.currBillingContext.Insurance.Ins_InsuranceBalance) {
        if (this.currBillingContext.Insurance.Ins_InsuranceBalance - (this.currBillingContext.Insurance.InsuranceProvisionalAmount + this.model.TotalAmount) < 0) {
          this.messageBoxService.showMessage("failed", ["Insufficient Insurance Balance, please remove some items and proceed"]);
          return false;
        }
      }
      else {
  ...
  public CloseInvoicePrint() {
    ...
    this.router.navigate(["/GovInsurance/Patient"]);
  }
