...
export class ConsultationRequestsComponent implements OnInit {
    ...
    public async GetDepartmentList() {
        try {
            const res: FromzaHTTPResponse = await this._nursingBLService.GetAllApptDepartment().toPromise();
    ...
    public async GetDoctorList() {
        try {
            const res: FromzaHTTPResponse = await this._nursingBLService.GetAllAppointmentApplicableDoctor().toPromise();
            if (res.Status === ENUM_FromzaHTTPResponses.OK) {
