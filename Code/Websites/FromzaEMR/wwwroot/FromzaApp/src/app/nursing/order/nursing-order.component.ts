...
export class NursingOrderComponent {
  ...
  AddBillRequest(index) {    //method to add the row
    ...
    this.CurrentNursingOrderItem.Quantity = 1;
    this.CurrentNursingOrderItem.PrescriberId = this.BillingTransactionItems[index].PrescriberId;
    ...
    if (this.CheckSelectedItems()) {
      ...
      if (this.BillingTransactionItems.length > 0) {
        for (var t = 0; t < this.BillingTransactionItems.length; t++) {
          this.BillingTransactionItems[t].RemoveValidators(["PerformerId", "DiscountPercent", "PrescriberId", "Price"]);
      ...
      if (isFormValid) {
        if (this.CheckDuplicateItem()) {
          ...
          for (var j = 0; j < this.BillingTransactionItems.length; j++) {
            ...
            this.BillingTransactionItems[j].PrescriberId = this.patientVisitService.globalVisit.PerformerId;
          }
  ...
  public AssignSelectedDoctor(index) {
    if (this.selectedAssignedToDr.length) {
      if (typeof (this.selectedAssignedToDr[index]) == 'object') {
        this.BillingTransactionItems[index].PerformerId = this.selectedAssignedToDr[index].EmployeeId;
        this.BillingTransactionItems[index].PerformerName = this.selectedAssignedToDr[index].FullName;
      }
    }
  }
}
