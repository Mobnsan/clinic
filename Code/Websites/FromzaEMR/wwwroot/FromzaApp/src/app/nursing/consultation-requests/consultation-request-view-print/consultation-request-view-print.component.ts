...
export class ConsultationRequestViewPrintComponent implements OnInit {
    ...
    public async GetAllApptDepartment() {
        try {
            const res: FromzaHTTPResponse = await this.nursingBLService.GetAllApptDepartment().toPromise();
    ...
    public async GetAllAppointmentApplicableDoctor() {
        try {
            const res: FromzaHTTPResponse = await this.nursingBLService.GetAllAppointmentApplicableDoctor().toPromise();
            if (res.Status === ENUM_FromzaHTTPResponses.OK) {
