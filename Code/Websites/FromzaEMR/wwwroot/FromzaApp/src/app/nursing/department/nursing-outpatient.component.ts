...
export class NursingOutPatientComponent {
  ...
  GetButtonsBasedOnPermissions(params) {
    ...
    if (currPat.IsTriaged == 1) {
      template = '<a fromza-grid-action="opd-triage" class="grid-action">Edit Triage</a>';
    } else {
      template = '<a fromza-grid-action="opd-triage" class="grid-action">Add Triage</a>';
    }
    ...
    if (NursingOutPatientComponent.serv.HasPermission("nursing-op-clinical-view")) {
      template += `<a fromza-grid-action="clinical" class="grid-action">Clinical</a>`;
    }

    if (NursingOutPatientComponent.serv.HasPermission("nursing-op-fileupload-view")) {
      template += ' <i fromza-grid-action="upload-files" class="fa fa-upload grid-action" style="padding: 3px;" title="upload files"> </i>';
    }
    return template;
  }
