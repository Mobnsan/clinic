...
export class NursingInPatientComponent {
  ...
  GetConsumptionsOfPatient(PatientId: number, PatientVisitId: number, StoreIds: string) {
    this.pharmacyBLService.GetPatientConsumptionsFromNursingWard(PatientId, PatientVisitId, StoreIds).subscribe((res: FromzaHTTPResponse) => {
      if (res.Status === ENUM_FromzaHTTPResponses.OK) {
        this.PatientConsumptionList = res.Results;
      }
      else {
        this.msgBoxServ.showMessage(ENUM_FromzaHTTPResponses.Failed, ['Failed to get patient consumption list']);
      }
    },
      err => {
        this.msgBoxServ.showMessage(ENUM_FromzaHTTPResponses.Failed, ['Failed to get patient consumption list']);
  ...
  GetPatientConsumptionList() {
    if (this.WardSubStoreMapList && this.WardSubStoreMapList.length) {
      ...
      this.pharmacyBLService.GetPatientConsumptionsOfNursingWard(StoreIds).subscribe((res: FromzaHTTPResponse) => {
        if (res.Status === ENUM_FromzaHTTPResponses.OK) {
  ...
  GetStoreAssociatedWithWard(WardId: number) {
    this.pharmacyBLService.GetWardSubStoreMapDetails(WardId).subscribe((res: FromzaHTTPResponse) => {
      if (res.Status === ENUM_FromzaHTTPResponses.OK) {
...
export class NursingGridColSetting {
  ...
  GetNursingActionsByPermission(params) {
    ...
    template += `<a fromza-grid-action="consumption" class="grid-action" title="Add patient consumption">
    ...
    if (NursingGridColSetting.serv.HasPermission("nursing-ip-wardbilling-view")) {
      template += ` <a fromza-grid-action="orders" class="grid-action" title="Click to add Orders">
                        Ward Request
                    </a>`;
    }

    if (NursingGridColSetting.serv.HasPermission("nursing-transfer-view")) {
      template += ` <a fromza-grid-action="transfer" class="grid-action" title="Click to transfer">
                        Transfer
                    </a>`;
    }

    template += `<a fromza-grid-action="vitals" class="grid-action" title="Add patient vitals">
                        Vitals
                    </a>`;
    //for favourite patients
    if (currPatient.IsFavorite) {
      template +=
        '<a fromza-grid-action="removefavorite" class="fa fa-heart" style="font-size:17px;top:-1px;padding: 5px;" title="remove favorite" ></a>';
    } else {
      template +=
        '<a fromza-grid-action="addfavorite" class="fa fa-heart-o" style="font-size:17px;top:-1px;padding: 5px" title="add favorite"></a>';
    }
    ...
    if (NursingGridColSetting.isReceiveEnabled) {
      if (
        ...
        if (
          ...
          template = ` <a fromza-grid-action="receive" class="animated-btn blinking-btn-warning grid-action" title="Receive Transferred Patient">
                        Receive
                    </a>`;
        }
      }
