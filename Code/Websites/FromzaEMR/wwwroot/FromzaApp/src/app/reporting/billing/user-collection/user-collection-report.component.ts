...
export class RPT_BIL_UserCollectionReportComponent {
  ...
  LoadCounter(): void {
    this.counterlist = FromzaCache.GetData(MasterType.BillingCounter, null);
  }
  Success(res) {
    if (res.Status == "OK") {
      ...
      this.CollectionSegregation = data.CollectionSegregation;
  ...
  CalculateTotalAmountsForSummaryView(collnSummaryList: Array<any>) {
    ...
    this.total_SummaryView.CashSales_Amount = this.total_SummaryView.CashSales_ReturnAmount = this.total_SummaryView.CollectionFromReceivables
      ...
      = this.total_SummaryView.Sett_CashDiscount = this.total_SummaryView.NetCollection = this.total_SummaryView.OtherPaymentsGiven = this.total_SummaryView.CoPaymentCashAmount = 0;
    if (collnSummaryList && collnSummaryList.length > 0) {
      collnSummaryList.forEach(row => {
        ...
        this.total_SummaryView.CoPaymentCashAmount += row.CoPaymentCashAmount;
