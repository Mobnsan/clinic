...
export class RPT_BIL_PatientCensusReportComponent {
  ...
  MapReportData(rprtData: Array<any>) {
    this.ReportData = [];
    let docNameList = rprtData.map(itm => itm.Performer).filter((value, index, self) => self.indexOf(value) === index);
    if (docNameList) {
      docNameList.forEach(i => {
        let docData = rprtData.filter(a => a.Performer == i);
        this.ReportData.push({ Performer: i, Data: docData });
      });
    }
  }
