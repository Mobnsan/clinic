...
export class RPT_BIL_ReferralSummaryMainComponent {
  ...
  Load() {
    ...
    if (this.curRefReportMain.fromDate != null && this.curRefReportMain.toDate != null) {
      if (this.curRefReportMain.prescriberId >= 0 && this.curRefReportMain.prescriberId != null)
        this.selRefReportMain = this.curRefReportMain;
      else {
        this.curRefReportMain.prescriberId = null;
        this.selRefReportMain = this.curRefReportMain;
      }
      // check if providerId is defined by user or not,
      //if yes, showDeptRefDept=true;  else showRefSummary=true;
      if (this.selRefReportMain.prescriberId == null) {
        this.showRefSummary = true;
        this.showItemSummary = false;
      }
      else if (this.selRefReportMain.prescriberId != null) {
  ...
  referrerChanged() {
    this.curRefReportMain.prescriberId = this.selReferrer ? this.selReferrer.EmployeeId : null;
  }
