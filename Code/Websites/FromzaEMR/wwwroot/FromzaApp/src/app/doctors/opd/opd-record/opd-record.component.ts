...
@Component({
  selector: 'app-opd-record',
  templateUrl: './opd-record.component.html',
  styleUrls: ['./opd-record.component.css']
})
export class OpdRecordComponent implements OnDestroy {
  ...
  GetICDList() {
    var icd = FromzaCache.GetData(MasterType.ICD, null);
  }
        ...
        .subscribe(res => {
          if (res.Status == "OK") {
            if (res.Results && res.Results.length > 0) {
              ...
              this.OPDPatientGridData = this.OPDPatientGridData.filter(x => x.VisitType == "outpatient" || x.VisitType == "OUTPATIENT");
  ...
  LoadPreviousVisitList() {
    ...
    if (fromDate <= toDate) {
      if (fromDate <= this.today && toDate <= this.today) {
        this.validDate = true;
          ...
          .subscribe(res => {
            if (res.Status == "OK") {
              if (res.Results && res.Results.length > 0) {
                ...
                this.OPDPatientGridData = new Array<Visit>();
                this.OPDPatientGridData = res.Results;
                this.OPDPatientGridData = this.OPDPatientGridData.filter(x => x.VisitType == "outpatient" || x.VisitType == "OUTPATIENT");
                this.OPDPatientGridData = this.GetFormattedVisits(this.OPDPatientGridData);
                this.FilterList(this.SelectedDepartmentId);
                this.GetFavoritesList();
