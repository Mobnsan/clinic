...
export class VisitSummaryCreateComponent {
    ...
    public LoadInQnairLevel() {
        ...
        if (prevQnairAns.length) {
            ...
            this.msgBoxServ.showMessage(ENUM_MessageBox_Status.Failed, ["No previous data for this section."]);
        }
    }
    ...
    public CreateDataSets() {
        ...
        this.dlService.Read(url).map(res => res).subscribe(res => {
            ...
            if (res.Status == ENUM_FromzaHTTPResponseText.OK) {
                ...
                this.msgBoxServ.showMessage(ENUM_MessageBox_Status.Failed, ["failed to load DSM-Codes for this request, please contact the administrator."]);
            }
    ...
    public SaveTemplateData(addType: string) {
        if (!this.loading) {
            ...
            if (newAnswers.length) {
                ...
                this.msgBoxServ.showMessage(ENUM_MessageBox_Status.Failed, ["Please enter some data."]);
            }
        }
        this.loading = false;
    ...
    public ShowVisitList(qnairId: number) {
        ...
        if (this.visitList.length > 0)
            ...
            this.msgBoxServ.showMessage(ENUM_MessageBox_Status.Failed, ["No previous data"]);
