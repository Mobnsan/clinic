...
import { ENUM_BillPaymentMode, ENUM_BillingStatus, ENUM_BillingType, ENUM_FromzaHTTPResponseText, ENUM_FromzaHTTPResponses, ENUM_IntegrationNames, ENUM_MessageBox_Status, ENUM_PriceCategory, ENUM_RegistrationSubCases, ENUM_SSF_EligibilityType, ENUM_ServiceBillingContext, ENUM_ValidatorTypes, ENUM_VisitType } from "../../../shared/shared-enums";
...
@Component({
  ...
  styles: [`.inl-blk, fromza-date-picker{display: inline-block;} .flx{display: flex;}`],
...
export class AdmissionCreateComponent {
  ...
  GetPatientLastVisitContext(PatientId: number): void {
    this.billingBlService.GetPatientLatestVisitContext(this.patientId).subscribe((res: FromzaHTTPResponse) => {
  ...
  GetNewClaimCode() {
    this.admissionBLService.GetNewClaimCode()
      .subscribe(res => {
        ...
        else if (res.Status === ENUM_FromzaHTTPResponses.Failed) {
          this.CurrentAdmission.ClaimCode = res.Results;
  ...
  getSSFPatientDetailLocally() {
    this.SSFEligibility = [];
    this.visitBLService.getSSFPatientDetailLocally(this.CurrentAdmission.PatientId).subscribe((res: FromzaHTTPResponse) => {
      if (res.Status === ENUM_FromzaHTTPResponses.OK) {
      ...
      this.msgBoxServ.showMessage(ENUM_FromzaHTTPResponses.Failed, ["Unable to get SSF Patient Detail Locally"]);
    });
  }

  LoadSSFEmployer() {
    this.visitBLService.GetSSFEmployerDetail(this.CurrentAdmission.PatientSchemesMap.PolicyHolderUID).subscribe((res: FromzaHTTPResponse) => {
      if (res.Status === ENUM_FromzaHTTPResponses.OK && res.Results) {
  ...
  isClaimed(LatestClaimCode: number, PatientId: number): void {
    this.visitBLService.IsClaimed(LatestClaimCode, PatientId)
      .subscribe((res: FromzaHTTPResponse) => {
        ...
        (err: FromzaHTTPResponse) => {
          this.msgBoxServ.showMessage(ENUM_FromzaHTTPResponses.Failed, ["Unable to check for pending claims"]);
        }
      );
  }
  ...
  getSSFPatientDetail() {
    if (this.CurrentAdmission.PatientSchemesMap.PolicyNo != null) {
      ...
      this.visitBLService.GetSSFPatientDetail(this.CurrentAdmission.PatientSchemesMap.PolicyNo).subscribe(res => {
        if (res.Status === ENUM_FromzaHTTPResponses.OK && res.Results.UUID !== null) {
  ...
  GetBedFeatureSchemePriceCategoryMap(schemeId: number): void {
    this._admissionMasterBlService.GetBedFeatureSchemePriceCategoryMap(schemeId).subscribe((res: FromzaHTTPResponse) => {
      if (res.Status === ENUM_FromzaHTTPResponses.OK && res.Results) {
