 ...
 * Separate grid-column settings for ADT module with option to use parameter value from Core-Services.
 ...
 Remarks: Most of the Properties/Methods are moved from grid-column-settings.constant.ts file (app/shared/fromza-grid)
 */
...
export class ADTGridColumnSettings {
    ...
    if (params.data.IsAdmitted == true) {
      ...
      tmplate =
        `<label style="font-weight: bold;background-color:#e4de1d;color: #151515;line-height: 18px;padding: 0 5px;">Reserved</label>
              <a fromza-grid-action="view-reserved-patient" class="grid-action">View</a>
      ...
      tmplate = `<a fromza-grid-action="admit" class="grid-action">Admit</a>`;
    }
  ...
  GetAdmittedListTemplateWithPermission(params) {
    ...
    if (ADTGridColumnSettings.isReceiveEnabled
      ...
      tmplate = '<a fromza-grid-action="transfer-hold" class="blinking-btn-danger grid-action" title="Cannot do further action until Received by Nursing">Transfer</a>';
    } else {
      if (ADTGridColumnSettings.securityServ.HasPermission("transfer-button")) {
        tmplate = `<a fromza-grid-action="transfer" class="grid-action">Transfer</a>`
      }
    }


    if (ADTGridColumnSettings.securityServ.HasPermission("sticker-button")) {
      tmplate += `<a fromza-grid-action="show-sticker"  title="Print Sticker" class="grid-action">
    ...
    if (ADTGridColumnSettings.securityServ.HasPermission("print-wristband-button")) {
      tmplateInDropDown += `<li><a fromza-grid-action="ip-wrist-band">Print WristBand</a></li>`;
    }

    if (ADTGridColumnSettings.securityServ.HasPermission("bill-history-button")) {
      tmplateInDropDown += `<li><a fromza-grid-action="billdetail">&nbsp;Bill History</a></li>`
    }

    if (ADTGridColumnSettings.securityServ.HasPermission("change-doctor-button")) {
      tmplateInDropDown += `<li><a fromza-grid-action="changedr">&nbsp;Change Doctor</a></li>`
    }
    ...
    if (ADTGridColumnSettings.securityServ.HasPermission("generic-sticker-button")) {
      tmplateInDropDown += `<li><a fromza-grid-action="generic-sticker">&nbsp;Print Generic Sticker</a></li>`;
    }

    if (ADTGridColumnSettings.securityServ.HasPermission("change-bed-feature-button")) {
      tmplateInDropDown += `<li><a fromza-grid-action="upgrade">&nbsp;Change Bed Feature</a></li>`
    }

    if (ADTGridColumnSettings.securityServ.HasPermission("cancel-admission-button")) {
      tmplateInDropDown += `<li><a fromza-grid-action="cancel">&nbsp;Cancel Admission</a></li>`
    }

    if (ADTGridColumnSettings.securityServ.HasPermission("btn-adt-admission-slip-view")) {
      tmplateInDropDown += `<li><a fromza-grid-action="admission-slip">&nbsp;Admission Slip</a></li>`
    }
  ...
  //      tmplate += ` <i fromza-grid-action="changedr" class="fa-user-md grid-action" style="padding: 3px;" title="Change Doctor"></i>`
  //  }
  //  if (params.data.ChangeBedButton == "change-bed-feature-button") {
  //      tmplate += `<i fromza-grid-action="upgrade" class="fa fa-exchange grid-action" style="padding: 3px;" title="Change Bed Feature"></i> `
  //  }
  //  if (params.data.BillHistoryButton == "bill-history-button") {
  //      tmplate += `<i fromza-grid-action="billdetail" class="fa fa-money grid-action" style="padding: 3px;" title="Bill History"></i> `
  //  }
  //  if (params.data.CancelAdmButton == "cancel-admission-button") {
  //      tmplate += `<i fromza-grid-action="cancel" class="fa fa-close grid-action" style="padding: 3px;" title="Cancel Admission"></i>  `
  //  }
  //  //if generic sticker to be shown then add below html, else it won't add.
  //  if (params.data.GenericStickerButton == "generic-sticker-button") {
  //      tmplate += `  <i fromza-grid-action="generic-sticker" class="glyphicon glyphicon-print grid-action" style="padding: 3px;" title="Print Generic Sticker"></i> `;
  //  }

  //  //read ShowWristBandFeature from parameter, if show then add below html, else don't add.
  //  if (params.data.PrintWristButton == "print-wristband-button") {
  //      tmplate += `<i fromza-grid-action="ip-wrist-band" class="glyphicon glyphicon-print grid-action" style="padding: 3px;" title="Print WristBand"></i>`;
  //  }
  //  return tmplate;
  //}
  ...
  ShowActionsforAdmittedPatientList(showWristBand, showGenSticker) {
    ...
    let tmplate = `<a fromza-grid-action="transfer" class="grid-action">Transfer</a>
                         <a fromza-grid-action="show-sticker"  title="Print Sticker" class="grid-action">
                              <i class="glyphicon glyphicon-print"></i>&nbsp;sticker </a>

                <i fromza-grid-action="changedr" class="fa-user-md grid-action" style="padding: 3px;" title="Change Doctor"></i>
                <i fromza-grid-action="upgrade" class="fa fa-exchange grid-action" style="padding: 3px;" title="Change Bed Feature"></i>
                <i fromza-grid-action="billdetail" class="fa fa-money grid-action" style="padding: 3px;" title="Bill History"></i>
                <i fromza-grid-action="cancel" class="fa fa-close grid-action" style="padding: 3px;" title="Cancel Admission"></i>
                    `;
  ...
  DischargeListActionRenderer(params) {
    ...
    if (currDischarge.DischargeSummaryId > 0 && currDischarge.IsSubmitted == 1 && !ADTGridColumnSettings.hasEditSummaryReportPermission) {
      template = `<a fromza-grid-action="dischargeSummary" class="grid-action">
                            View Summary</a>`;
    }
      ...
      template = `<a fromza-grid-action="dischargeSummary" class="grid-action">
      View Summary</a>
      <a fromza-grid-action="dischargeSummary" class="grid-action">
                            Edit Summary</a>`;
    }
    ...
    if (currDischarge.BillStatusOnDischarge == "unpaid") {
      template += `<a fromza-grid-action="clear-due" class="grid-action">
                             Clear Due</a>`;
    }

    //sud:3May'21--Hiding Discharge Cancel Functionality since Credit Note is introduced in Billing.
    //!Krishna:30thJune'23, Opening below commented code, we need this now in Manipal
    ...
    //   template += `<a fromza-grid-action="discharge-cancel" class="grid-action">
    //         Cancel Discharge</a>`;
    // }
    ...
    if (ADTGridColumnSettings.securityServ.HasPermission("btn-adt-discharge-slip-view")) {
      templateInDropDown += `<li><a fromza-grid-action="discharge-slip">&nbsp;Discharge Slip</a></li>`
    }


    if (templateInDropDown && templateInDropDown.trim().length) {
      template = template + `<div class="dropdown" style = "display:inline-block;">
            <button class="dropdown-toggle grid-btnCstm" type="button" data-toggle="dropdown">...
              <span class="caret" > </span></button>
                <ul class="dropdown-menu grid-ddlCstm" >`+ templateInDropDown + `</ul></div>`
    }
  ...
  NursingDischargeListActionRenderer(params) {
    ...
    //if ((((checkHrs / 1000) / 60)) / 60 < 120) {
    //  templateHtml += `<a fromza-grid-action="discharge-cancel" class="grid-action">
    //        Cancel Discharge</a>`;
    //}
    return templateHtml;
  }
