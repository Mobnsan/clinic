    ...
    this.admissionBLService.CheckAdmissionCancelled(this.admissionCancel)
      .subscribe(res => {
        if (res.Status == 'OK') {
          ...
          if (this.returnDepositInfo && this.returnDepositInfo.OutAmount)
            this.msgBoxServ.showMessage("success", ["Cancelled and Amount " + this.coreService.currencyUnit + this.returnDepositInfo.OutAmount + " returned successfully."]);
          else
