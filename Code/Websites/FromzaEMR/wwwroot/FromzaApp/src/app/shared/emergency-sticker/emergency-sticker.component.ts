...
import { ENUM_Country, ENUM_FromzaHTTPResponseText, ENUM_MembershipTypeName, ENUM_PriceCategory } from "../shared-enums";
...
export class EmergencyStickerComponent {
  ...
  public showMunicipality: boolean = null;
  ...
  constructor(
    ...
    this.printerName = localStorage.getItem('Fromza_ER_Default_PrinterName');
  ...
  CallBackStickerOnly(res) {
    if (res.Status === ENUM_FromzaHTTPResponseText.OK && res.Results.length !== 0) {
      this.stickerDetail = {...res.Results[0]};
  ...
  printStickerServer() {
    ...
    var printableHTML = '<html><head><link rel="stylesheet" type="text/css" href="FromzaStyle.css" />';
  ...
  public UpdateNewPrinter() {
    if (this.printerNameSelected) {
      if (localStorage.getItem('Fromza_ER_Default_PrinterName')) {
        localStorage.removeItem('Fromza_ER_Default_PrinterName');
      }
      localStorage.setItem('Fromza_ER_Default_PrinterName', this.printerNameSelected);
  ...
  public PrintDotMatrix() {
    ...
    let address = '';
    if (this.stickerDetail.CountryName === this.CountryNepal) {
      address = "Address:" + ((this.showMunicipality && this.stickerDetail.MunicipalityName)? this.stickerDetail.MunicipalityName + (this.stickerDetail.WardNumber? "-" + this.stickerDetail.WardNumber : "") + ", " :"") + this.stickerDetail.CountrySubDivisionName + nline;
    } else {
      address = "Address:" + (this.stickerDetail.Address? this.stickerDetail.Address + ", " : "") + this.stickerDetail.CountrySubDivisionName + ", " + this.stickerDetail.CountryName + nline;
    }
    finalDataToPrint += address;
    finalDataToPrint += 'Type: ' + this.stickerDetail.MembershipTypeName 
                                 + ((this.stickerDetail.SSFPolicyNo && (this.stickerDetail.MembershipTypeName === ENUM_MembershipTypeName.SSF))? '  ' + 'SSF Policy No: ' + this.stickerDetail.SSFPolicyNo : "")
                                 + ((this.stickerDetail.PolicyNo && (this.stickerDetail.MembershipTypeName === ENUM_MembershipTypeName.ECHS))?  '  ' + 'ECHS No: ' + this.stickerDetail.PolicyNo : "")
                                 + nline;
