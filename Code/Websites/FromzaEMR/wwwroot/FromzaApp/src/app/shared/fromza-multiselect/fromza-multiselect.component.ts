...
@Component({
    selector: "fromza-multiselect",
    templateUrl: "./fromza-multiselect.html"
})
export class FromzaMultiSelectComponent implements OnInit {
    //NBB-
    ...
    MakeFullPropertiesSelectedList() {
        if (this.dropdownList.length > 0) {
            ...
            this.PreSelected.forEach(i => {
                let index = this.dropdownList.findIndex(t => t.id == i.id);
                if (this.dropdownList[index] !== undefined)
