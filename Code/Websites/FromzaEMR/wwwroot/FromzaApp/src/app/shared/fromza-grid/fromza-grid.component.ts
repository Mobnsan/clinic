...
@Component({
  selector: "fromza-grid",
  templateUrl: "./fromza-grid.html",
})
export class FromzaGridComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit() {
    if (this.showCustomSearch) {
      ...
      if (this.searchUsingHospitalNo) {
        fromEvent(this.searchPatientUsingHospitalNo.nativeElement, "keyup")
          .pipe(
            map((event: any) => {
              return event.target.value;
            }),
            ...
            if (!text) {
              let emittingObj = { searchUsingHospitalNo: true, text: text };
              let emittingString = JSON.stringify(emittingObj);
              this.ssTextEmitter.emit(emittingString);
            }
          });
      }
      if (this.searchUsingIdCardNo) {
        fromEvent(this.searchPatientUsingIdCardNo.nativeElement, "keyup")
          .pipe(
            map((event: any) => {
              return event.target.value;
            }),
            ...
            if (!text) {
              let emittingObj = { searchUsingIdCardNo: true, text: text };
              let emittingString = JSON.stringify(emittingObj);
              this.ssTextEmitter.emit(emittingString);
            }
          });
  ...
  SearchPatientUsingHospitalNoCallBack($event) {
    if (this.searchUsingHospitalNo && $event.target.value) {
  ...
  SearchPatientUsingIdCardNoCallBack($event) {
    if (this.searchUsingIdCardNo && $event.target.value) {
      let text = $event.target.value;
      let emittingObj = { searchUsingIdCardNo: true, text: text };
      let emittingString = JSON.stringify(emittingObj);
      this.ssTextEmitter.emit(emittingString);
  ...
  onBtPrint() {
    if (this.reportFor == 'inventory') {
      this.onBtPrintForInventory();
    }
    else if (this.reportFor == 'pharmacy') {
      this.onBtPrintForInventory();
      return;
    }
    else if (this.reportFor == 'Report') {
      this.onBtPrintForReport();
      return;
    }
    else {
      ...
      //documentContent += '<link rel="stylesheet" type="text/css" media="print" href="../../../themes/theme-default//FromzaPrintStyle.css"/>';
      documentContent +=
        '<link rel="stylesheet" type="text/css" href="../../../assets/global/plugins/bootstrap/css/bootstrap.min.css"/>';
      documentContent +=
        '<link rel="stylesheet" type="text/css" href="../../../themes/theme-default//FromzaStyle.css"/>';
  ...
  // NOTE: each custom item(eg, image, button, anchor, should have this attribute "fromza-grid-action" in its template, else it won't work..
  public onCellClicked($event) {
    if ($event.event.target !== undefined) {
      let data = $event.data;
      let actionType = $event.event.target.getAttribute("fromza-grid-action");
  ...
  public ExportToExcel($event) {
    ...
    if (1) {
      if (this.reportFor == 'inventory') {
        this.ExportGridToExcelForInventory();
        return;
      }
      if (this.reportFor == 'pharmacy') {
        ...
        this.ExportGridToExcelForReports();
        return;
      }
      this.ExportGridToExcel();
      return;
    }
    FromzaGridComponent.colIdObj = {};
    if (this.showNepaliDateOption) {
      this.nepaliDateInGridColDetail.NepaliDateColumnList.forEach((col) => {
        let ind = this.columnDefs.findIndex(
          (cldf) => cldf.field == col.EnglishColumnName
        ...
        let fld = this.columnDefs[ind].field;
        FromzaGridComponent.colIdObj[fld] = true;
        FromzaGridComponent.colIdObj[fld + "showTime"] = col.ShowTime;
      });
    }
    ...
    function cellFormattingFunction(params) {
      ...
      if (FromzaGridComponent.colIdObj[colId]) {
        if (FromzaGridComponent.colIdObj[colId + "showTime"]) {
    ...
    try {
      ...
      if (!!printSettingParameter || !!exportToExcelSettingParameter) {
        ...
        switch (this.router.url) {
          ...
          case "/Reports/BillingMain/PaymentModeWiseReport": {
            if (!!printSettingParameter) this.paramData = printSettingParameter["PaymentModeWiseReport"];
            if (!!exportToExcelSettingParameter) this.paramExportToExcelData = exportToExcelSettingParameter["PaymentModeWiseReport"]
            break;
          }
          case "/Reports/BillingMain/SchemeDetailInvoice": {
            if (!!printSettingParameter) this.paramData = printSettingParameter["SchemeDetailInvoiceReport"];
            if (!!exportToExcelSettingParameter) this.paramExportToExcelData = exportToExcelSettingParameter["SchemeDetailInvoiceReport"]
            break;
          }
          ...
          case "/Pharmacy/Report/Stock/SupplierWiseStockReport": {
            if (!!printSettingParameter) this.paramData = printSettingParameter["SupplierWiseStockReport"];
            if (!!exportToExcelSettingParameter) this.paramExportToExcelData = exportToExcelSettingParameter["SupplierWiseStockReport"]
            break;
          }
  ...
  GetHeaderParameter(reportFor) {
    ...
    else if (reportFor == 'inventory') {
      var paramValue = this.coreService.Parameters.find(a => a.ParameterGroupName == "Inventory" && a.ParameterName == "Inventory Receipt Header").ParameterValue;
      if (paramValue)
        this.headerDetail = JSON.parse(paramValue);
      else
        this.msgBoxServ.showMessage("error", ["Error getting parameters"]);
  ...
  onBtPrintForInventory() {
    let dateType = this.coreService.DatePreference;
    let popupWinindow;
    var currDate = moment().format("YYYY-MM-DD HH:mm");
    var nepCurrDate = NepaliCalendarService.ConvertEngToNepaliFormatted_static(currDate, "YYYY-MM-DD hh:mm");
    var printDate = dateType == 'np' ? nepCurrDate : currDate;
    var datePref = dateType == 'np' ? 'BS' : 'AD';


    this.printBy = this.securityService.loggedInUser.Employee.FullName;
    let printedByMessage = "<b>Printed By:</b>&nbsp;";
    this.printBy = this.printBy != "" ? this.printBy : "";

    this.reportTitle = this.reportTitle != "" ? this.reportTitle : "";
    this.headerDetail.hospitalName ? this.headerDetail.hospitalName : "";
    this.headerDetail.address ? this.headerDetail.address : "";
    this.headerDetail.PANno ? this.headerDetail.PANno : "";
    this.headerDetail.tel ? this.headerDetail.tel : "";
    ...
    if (this.showHeader) {
      var printContents =
        `<div>
          <p style="text-align:center;"><strong>${this.headerDetail.hospitalName}</strong></p>
          <p style="text-align:center;"><strong>${this.headerDetail.address}</strong></p>
          <p style="text-align:center;"><strong><span>PAN No: </span>${this.headerDetail.PANno}</strong></p>
          <p style="text-align:center;"><strong><span>Ph No: </span>${this.headerDetail.tel}</strong></p>
          <p style="text-align:center;"><strong>${this.reportTitle}</strong></p>
      ...
      var printContents = `<div class="row">
  ...
  ExportGridToExcelForInventory() {
    try {
      ...
      var nepCurrDate = NepaliCalendarService.ConvertEngToNepaliFormatted_static(currentDate, "YYYY-MM-DD hh:mm");
      var printDate = dateType == 'np' ? nepCurrDate : currentDate;
      var datePref = dateType == 'np' ? 'BS' : 'AD';
      let filename;
      let workSheetName;
      filename = workSheetName = this.exportOptions.fileName;
      ...
      let collen1;
      let actionColumn = col.find(a => a.headerName == 'Actions' || a.headerName == 'BarCodes' || a.headerName == 'Action');
      if (actionColumn) {
        collen1 = col.length - 4;
      }
      else {
        collen1 = col.length - 3;
      }
      ...
      if (this.showHeader) {
        var Header = '<tr><td colspan="' + collen + '" style="text-align:center;font-size:large;"><strong>' + this.headerDetail.hospitalName + '</strong></td></tr><br/>' +
          '<tr> <td colspan="' + collen + '" style="text-align:center;font-size:small;"><strong>' + this.headerDetail.address + '</strong></td></tr><br/>' +
          '<tr> <td colspan="' + collen + '" style="text-align:center;font-size:small;"><strong>' + 'PAN No:' + this.headerDetail.PANno + '</strong></td></tr><br/>' +
          '<tr> <td colspan="' + collen + '" style="text-align:center;font-size:small;"><strong>' + ' Ph No: ' + this.headerDetail.tel + '</strong></td></tr><br/>' +
          '<tr> <td colspan="' + collen + '" style="text-align:center;font-size:small;width:600px;"><strong>' + this.reportTitle + '</strong></td></tr><br/>' +
          '<tr> <td style="text-align:center;"><strong>' + '</strong></td><td colspan="' + collen1 + '"></td><td style="text-align:center;"><strong>' + printByMessage + printBy + `</strong></td><td><strong>Exported On: ${printDate}&nbsp; ${datePref}</strong></td></tr><br>`
          + '';
      }
      else {
        Header += '<tr><td colspan="' + collen + '" style="text-align:center;font-size:small;width:600px;"><strong>' + this.reportTitle + '</strong></td></tr><br/>' +
          '<tr> <td style="text-align:center;"></td><td  colspan="' + collen1 + '"></td><td style="text-align:center;"><strong>' + printByMessage + printBy + `</strong></td><td><strong>Exported On: ${printDate}&nbsp; ${datePref}</strong></td></tr><br>`
      }
      Header += filterDescription;
      this.ManagePrintDataList();
      var htmlTable = this.JsonDataToHTMLTable(
        this.PrintRowData,
        this.columnDefs
      ...
      let uri = 'data:application/vnd.ms-excel;base64,'
        , template = '<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns="http://www.w3.org/TR/REC-html40"><head><!--[if gte mso 9]><xml><x:ExcelWorkbook><x:ExcelWorksheets><x:ExcelWorksheet><x:Name>{worksheet}</x:Name><x:WorksheetOptions><x:DisplayGridlines/></x:WorksheetOptions></x:ExcelWorksheet></x:ExcelWorksheets></x:ExcelWorkbook></xml><![endif]--><meta http-equiv="content-type" content="text/plain; charset=UTF-8"/></head><body><table>{header}<br>{content}<br><table><tr><td>{footer}</td></tr></table></table></body></html>'
        , base64 = function (s) { return Base64.toBase64(decodeURIComponent(encodeURIComponent(s))) } //Base64 is coming from 'js-base64' package..
        , format = function (s, c) { return s.replace(/{(\w+)}/g, function (m, p) { return c[p]; }) }
      var ctx = { worksheet: workSheetName, header: Header, content: htmlTable.innerHTML, footer: Footer }
      var link = document.createElement('a');
  ...
  onBtPrintForReport() {
    ...
    var filterDescription =
      "<div><strong>Filter Date Range</strong></div>" +
      '<div style="display:flex;" class="border border-dark">' +
      this.filterParameters.reduce(
        (accumulator, current, index) => {
          if (index % 2 == 0) accumulator += '<div>';
          accumulator +=
            `
          <div class="col-md-12">
            <div class="col-md-6"><strong>${current.DisplayName}</strong>
            ${NepaliCalendarService.ConvertEngToNepaliFormatted_static(current.Value, "YYYY-MM-DD")}</div>
          </div>
          `
          if (index % 2 != 0) accumulator += '</div>';
          return accumulator;
  ...
  ExportGridToExcelForReports() {
    try {
      ...
      var filterDescription =
        "<tr><td><strong>Filter Date Range(BS)</strong></td></tr>" +
        // `<th style='text-align:center;'><strong> Date Range</strong></th>` +
        this.filterParameters.reduce(
          (accumulator, current, index) => {
            if (index % 2 == 0) accumulator += '<tr>';
            accumulator +=
              `
              <th style='text-align:center;'><strong>${current.DisplayName}</strong></th>
              <th style='text-align:center;'>${NepaliCalendarService.ConvertEngToNepaliFormatted_static(current.Value, "YYYY-MM-DD")}</th>
              `
            if (index % 2 != 0) accumulator += '</tr>';
            return accumulator;
          }
      ...
      if (this.showHeader) {
        var Header = '<tr> <td colspan="' + collen + '" style="text-align:center; margin-top:15px; font-size:small;width:600px;"><strong>' + this.reportTitle + '</strong></td></tr><br/>'
          + '';
      }
