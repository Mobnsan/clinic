 ...
 */
@Directive({
  selector: "[auto-complete], [fromza-auto-complete]"
})
export class FromzaAutoCompleteDirective implements OnInit {
  ...
  componentRef: ComponentRef<FromzaAutoCompleteComponent>;
  ...
  ngOnInit(): void {
    ...
    this.wrapperEl.className = "fromza-auto-complete-wrapper";
  ...
  showAutoCompleteDropdown = (event?: any): void => {
    ...
    let factory = this.resolver.resolveComponentFactory(FromzaAutoCompleteComponent);
  ...
  styleAutoCompleteDropdown = () => {
    if (this.componentRef) {
      ...
      this.acDropdownEl.style.zIndex = "12";//sud:23Jul'19--change to 12 from 1 since it was coming below focused cell of fromza-grid.
  ...
  public keydownEventHandler = (evt: any) => {
    if (this.componentRef) {
      let component = <FromzaAutoCompleteComponent>this.componentRef.instance;
