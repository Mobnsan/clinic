...
import { ENUM_AdditionalServiceItemGroups, ENUM_BillPaymentMode, ENUM_BillingStatus, ENUM_CancellationService, ENUM_FromzaHTTPResponseText, ENUM_FromzaHTTPResponses, ENUM_InvoiceType, ENUM_MembershipTypeName, ENUM_MessageBox_Status, ENUM_OrderStatus, ENUM_Scheme_ApiIntegrationNames, ENUM_ServiceBillingContext } from '../../../shared/shared-enums';
...
export class PatientIpSummaryComponent {
  ...
  //     }
  //   }, err => {
  //     this.msgBoxServ.showMessage(ENUM_FromzaHTTPResponses.Failed, ["Could not fetch medicare member detail"]);
  //   });
  // }
    ...
    if (this.SchemePriceCategory.SchemeId && this.SchemePriceCategory.PriceCategoryId) {
      this.billingMasterBlService.GetServiceItems(ENUM_ServiceBillingContext.IpBilling, this.SchemePriceCategory.SchemeId, this.SchemePriceCategory.PriceCategoryId)
        .subscribe((res: FromzaHTTPResponse) => {
          if (res.Status === ENUM_FromzaHTTPResponses.OK && res.Results) {
  ...
  ConfirmDischarge() {
    ...
    if ((this.patAllPendingItems && this.patAllPendingItems.length) || (this.PharmacyTotal > 0)) {//&& this.model.Tender >= this.model.ToBePaid
      ...
      if (this.model.PayType.toLowerCase() === ENUM_BillPaymentMode.credit.toLowerCase() && !this.model.OrganizationId || this.model.OrganizationId === 0) {
        this.msgBoxServ.showMessage(ENUM_FromzaHTTPResponses.Failed.toLowerCase(), ["Credit Organization is mandatory for credit bill"]);
        return;
      }
        ...
        this.msgBoxServ.showMessage(ENUM_FromzaHTTPResponses.Failed.toLowerCase(), [" Remarks is mandatory."]);
        return;
      }
      else {
        let sure = true;
  ...
  PostBillingTransaction() {
    ...
    if (this.ipBillingTxnVM.billingTransactionModel.PaymentMode.toLowerCase() === ENUM_BillPaymentMode.credit.toLowerCase() && (!this.ipBillingTxnVM.billingTransactionModel.OrganizationId || this.ipBillingTxnVM.billingTransactionModel.OrganizationId === 0)) {
      this.msgBoxServ.showMessage(ENUM_FromzaHTTPResponses.Failed.toLowerCase(), ["Credit Organization is mandatory for credit"]);
      this.loading = false;
      return;
    }
  ...
  PostProvisionalDischarge(): void {
    ...
    if (provisionalDischarge) {
      ...
      this.loading = true;
      this.billingBLService.PostProvisionalDischarge(provisionalDischarge).finally(() => this.loading = false).subscribe((res: FromzaHTTPResponse) => {
        if (res.Status === ENUM_FromzaHTTPResponses.OK && res.Results) {
