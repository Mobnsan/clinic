...
export class BillingMasterBlService {
  ...
  FetchServiceItemsBasedOnCurrentVisitSchemeAndPriceCategory(serviceBillingContext: string, schemeId: number, priceCategoryId: number) {
    ...
    this.billingMasterDlService.GetServiceItems(serviceBillingContext, schemeId, priceCategoryId).subscribe((res: FromzaHTTPResponse) => {
  ...
  FetchAdditionalServiceItems(groupName: string, priceCategoryId: number) {
    this.billingMasterDlService.GetAdditionalServiceItems(groupName, priceCategoryId).subscribe((res: FromzaHTTPResponse) => {
      if (res.Status === ENUM_FromzaHTTPResponses.OK) {
