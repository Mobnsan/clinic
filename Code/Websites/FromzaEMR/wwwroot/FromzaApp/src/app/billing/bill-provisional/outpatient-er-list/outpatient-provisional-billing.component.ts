...
//to add fromza-grid in credit-details page:sudarshan 26Mar'17
import { CoreService } from "../../../core/shared/core.service";
import { SecurityService } from '../../../security/shared/security.service';
import { CallbackService } from '../../../shared/callback.service';
import { CancelStatusHoldingModel, FromzaHTTPResponse } from "../../../shared/common-models";
...
// App Component class
export class OutpatientProvisionalBillingComponent {
  ...
  constructor(public billingService: BillingService,
    ...
    if (!this.counterId || this.counterId < 1) {
      this.callbackService.CallbackRoute = '/Billing/ProvisionalClearance';
    } else {
    ...
    this.patientService.globalPatient = new Patient();
    ...
    this.billingBLService.GetProvisionalItemsByPatientId(patientId, schemeId)
      .subscribe((res: FromzaHTTPResponse) => {
        ...
        if (res.Status == "OK") {
          ...
          this.messageBoxService.showMessage("Failed", ["Couldn't load Provisional Details of this Patient. Please try again."]);
          this.BackToGrid();
        }
      });
  }
  ...
  ShowPatientProvisionalItems(row): void {
    ...
    this.SchemeId = row.SchemeId;

    this.GetPatientProvisionalItems(patient.PatientId, this.SchemeId);
  ...
  PayAll() {
    ...
    if (unique2 && unique2.length > 1) {
      this.messageBoxService.showMessage("Notice", ["Provisional billing was done using Multiple Schemes. Please change to the correct one during Final Invoice."]);
    }
  ...
  OutPatientProvisionalGridActions($event: GridEmitModel) {
    switch ($event.Action) {
      case "showDetails":
        {
      ...
      //     this.GetPatientProvisionalItems(data.PatientId, this.SchemeId, true);
        ...
        this.billingBLService.CancelMultipleTxnItems(txnItemsToCancel)
          .subscribe((res: FromzaHTTPResponse) => {
            ...
            if (res.Status == "OK") {
              ...
              this.messageBoxService.showMessage(ENUM_MessageBox_Status.Success, ["Item cancelled successfully"], res.ErrorMessage);

            }
            else {
              this.messageBoxService.showMessage(ENUM_MessageBox_Status.Failed, ["Some error occured, please try again."], res.ErrorMessage);
            }
          });
      }
      else {
  ...
  OnCancelSuccess(itms: Array<BillingTransactionItem>) {
    this.showActionPanel = false;
    this.remarks = null;
    this.messageBoxService.showMessage("success", ["Items Cancelled successfully"]);
    if (itms) {
      itms.forEach(itm => {
  ...
  EditItemBtnOnClick(index, txnItem) {
    ...
    var selItemDetails = this.itemList.find(a => a.ServiceItemId == serviceItemId && a.ItemName == itmName)
  ...
  CloseItemEditWindow($event) {
    ...
    if ($event && ($event.EventName == "update" || $event.EventName == "cancelled")) {
      this.GetPatientProvisionalItems(this.patientService.globalPatient.PatientId, this.SchemeId);
    }
  }
  ...
  HasZeroPriceItems(): boolean {
    ...
    if (items && items.length) {
      ...
      this.messageBoxService.showMessage("Warning!", ["Some of the items has price 0. Please update."]);
      return true;
    }
  }
  ...
  CloseUpdatePricePopup($event) {
    ...
    this.LoadPatientPastBillSummary(this.patientService.globalPatient.PatientId, this.SchemeId);
    this.CalculationForAll();
  }
