...
export class INSBillingTransactionComponent {
        ...
        if (res.Status == 'OK') {
          if (res.Results.length) {
            ...
            if (this.model.BillingTransactionItems.length) {
              ...
              this.currentPrescriber = this.visitList[0].PerformerName == "Duty Doctor" ? "SELF" : this.visitList[0].PerformerName;
      ...
      .subscribe(res => {
        if (res.Status == 'OK') {
          if (res.Results.length) {
            ...
            this.messageBoxService.showMessage('Failed', ["unable to get items for searchbox.. check logs for more details."]);
            console.log(res.ErrorMessage);
          }
        }
      },
  ...
  CheckAndSubmitBillingTransaction(billingFlow: string = null) {
    ...
    if (this.model.DiscountPercent < 0 || this.model.DiscountPercent > 100) {
      this.messageBoxService.showMessage("failed", ["Additional Discount Percent is invalid. It must be between 0 and 100."]);
      this.loading = false;
      return;
    }
  ...
  CheckBillingValidations(): boolean {
    ...
    for (var j = 0; j < this.model.BillingTransactionItems.length; j++) {
      if (this.model.BillingTransactionItems[j].Price <= 0) {
        this.messageBoxService.showMessage("error", ["The price of some items is zero "]);
        this.loading = false;
        break;
      }
      if (this.model.BillingTransactionItems) {
    ...
    if (this.model.PaymentMode == "credit" && !this.model.Remarks) {
      isFormValid = false;
      this.messageBoxService.showMessage("failed", ["Remarks is mandatory for credit bill"]);
    }
    return isFormValid;
  }
  ...
  AssignBillTxnItemsValuesForSubmit() {
    for (var j = 0; j < this.model.BillingTransactionItems.length; j++) {
      ...
      //incase of emergency do not assign other visit details by default.
        ...
        let visit = this.visitList.find(a => a.ProviderId == this.model.BillingTransactionItems[j].PrescriberId)
        if (visit)
    ...
    this.BillingBLService.PostDepartmentOrders(billTxnItems, "active", "provisional", this.insuranceApplicableFlag, this.currPatVisitContext)
      .subscribe(res => {
        if (res.Status == "OK" && res.Results.length) {
          ...
          this.messageBoxService.showMessage("failed", ["Unable to add department requisitions"]);
    ...
    this.BillingBLService.PostDepartmentOrders(items, "active", "provisional", this.insuranceApplicableFlag)
      .subscribe(res => {
        if (res.Status == "OK" && res.Results.length) {
          ...
          this.messageBoxService.showMessage("failed", ["Unable to add emergency visit"]);
          console.log(res.ErrorMessage);
          this.isProvisionalBilling = false;
          this.loading = false;
        }
    ...
    this.BillingBLService.GetServiceDepartments()
      .subscribe(res => {
        if (res.Status == "OK") {
          ...
          this.messageBoxService.showMessage("failed", [res.ErrorMessaage], res.ErrorMessaage);
        }
      },
        err => {
          this.messageBoxService.showMessage("error", [err.ErrorMessaage], err.ErrorMessaage);
        }
      );
  }
  ...
  ShowPackage() {
    if (this.billingService.BillingFlow != "normal"
      ...
      this.messageBoxService.showMessage("failed", ["Close current Insurance Package to start new one."]);
    }
    else {
        ...
        if (this.billingType && this.billingType.toLowerCase() != "inpatient" && !this.isPackageBilling) {
          ...
          if (!paramDupItm) {
            if (extItem && index != extItemIndex) {
              this.messageBoxService.showMessage("failed", [item.ItemName + " is already entered."]);
              this.changeDetectorRef.detectChanges();
  ...
  public AssignRequestedByDoctor(index) {
    let doctor = null;
    if (this.currentPrescriber) {
      if (typeof (this.currentPrescriber) == 'string' && this.reqDoctorsList.length) {
        doctor = this.reqDoctorsList.find(a => a.FullName.toLowerCase() == this.currentPrescriber.toLowerCase());
      }
      else if (typeof (this.currentPrescriber) == 'object')
        doctor = this.currentPrescriber;
      if (doctor) {

        this.model.BillingTransactionItems[index].PrescriberId = doctor.EmployeeId;
        //this.model.BillingTransactionItems[index].BillingTransactionItemValidator.controls["RequestedBy"].setValue(doctor.EmployeeId);
        this.model.BillingTransactionItems[index].PrescriberName = doctor.FullName;
  ...
  public RequestedByDrOnChange() {
    let doctor = null;
    console.log(this.currentPrescriber);
      ...
      if (doctor) {
        ...
        if (this.model.BillingTransactionItems) {
          this.model.BillingTransactionItems.forEach(billTxnItem => {
            billTxnItem.PrescriberId = this.currentPrescriber.EmployeeId;
            billTxnItem.PrescriberName = this.currentPrescriber.FullName;
    ...
    if (this.model.BillingTransactionItems.length) {
      for (let itm of this.model.BillingTransactionItems) {
        if (!itm.IsValidSelDepartment) {
          this.messageBoxService.showMessage("failed", ["Invalid Department. Please select Department from the list."]);
          this.loading = false;
          return false;
        }
        if (!itm.IsvalidSelPerformerDr) {
          this.messageBoxService.showMessage("failed", ["Invalid Assigned To Dr. Name. Please select doctor from the list."]);
          this.loading = false;
          return false;
        }
        ...
        if (itm.IsDuplicateItem) {
          this.messageBoxService.showMessage("failed", ["Duplicate Item now allowed." + itm.ItemName + " is entered more than once"]);
          this.loading = false;
          return false;
        }
      }
  ...
  ResetSelectedRow(index) {
    ...
    this.model.BillingTransactionItems[index].ItemList = this.itemList.filter(a => a.ServiceDepartmentName != "OPD");;
    this.model.BillingTransactionItems[index].PrescriberId = index > 0 ? this.model.BillingTransactionItems[index - 1].PrescriberId : (this.visitList[0].PerformerName == "Duty Doctor" ? null : this.visitList[0].PerformerId);
    this.ReCalculateInvoiceAmounts();//sud:10Mar'19
    //this.Calculationforall();//sud:10Mar'19--this method is replaced by above.
    ...
    if (this.billingService.BillingFlow == "insurance") {
      ...
      if (this.currBillingContext.Insurance.CurrentBalance) {
        if (this.currBillingContext.Insurance.CurrentBalance - (this.currBillingContext.Insurance.InsuranceProvisionalAmount + this.model.TotalAmount) < 0) {
          this.messageBoxService.showMessage("failed", ["Insufficient Insurance billing, please remove some items and proceed"]);
          return false;
        }
      }
      else {
  ...
  currentPrescriber: any = null; // Krishna, 16th,jun'22, changed currentRequestedByDoctor to currentPrescriber.
