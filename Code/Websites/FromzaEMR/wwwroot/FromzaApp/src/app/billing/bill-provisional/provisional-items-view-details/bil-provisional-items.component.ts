...
import { ENUM_FromzaHTTPResponses, ENUM_IntegrationNames, ENUM_MessageBox_Status, ENUM_OrderStatus, ENUM_ProvisionalBillingContext, ENUM_ServiceBillingContext } from "../../../shared/shared-enums";
  ...
  DiscardAll(): void {
    if (this.DiscardProvisionalItems) {
      if (this.DiscardProvisionalItems.DiscardRemarks && this.DiscardProvisionalItems.DiscardRemarks.trim().length > 0) {
        this._billingBlService.DiscardProvisionalItems(this.DiscardProvisionalItems).subscribe((res: FromzaHTTPResponse) => {
