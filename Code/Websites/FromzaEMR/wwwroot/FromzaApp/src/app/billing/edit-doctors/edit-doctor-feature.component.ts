...
export class EditDoctorFeatureComponent {
  ...
  serverSearchTxt(searchTxt) {
    this.searchText = searchTxt ? searchTxt : null;
  ...
  Callback(res: Array<EditDoctorFeatureViewModel>) {
    ...
    if (res.length != 0) {
      ...
      for (var i = 0; i < res.length; i++) {
        ...
        editDoctorModel.PerformerId = res[i].PerformerId;
        ...
        editDoctorModel.PerformerName = res[i].PerformerName;
        ...
        if (this.doctorList && res[i].PrescriberId) {
          var docObj = this.doctorList.find(a => a.EmployeeId == res[i].PrescriberId);
          if (docObj) {
            editDoctorModel.PrescriberName = docObj.EmployeeName;
            editDoctorModel.PrescriberId = docObj.EmployeeId;
          }
