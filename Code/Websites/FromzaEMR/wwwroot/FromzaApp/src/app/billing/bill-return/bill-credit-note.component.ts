...
import { ENUM_BillPaymentMode, ENUM_FromzaHTTPResponses, ENUM_MessageBox_Status, ENUM_ProcessConfirmationActions, ENUM_ProcessesToConfirmDisplayNames } from "../../shared/shared-enums";
...
export class BILL_CreditNoteComponent {
  ...
  public IsCoPaymentInvoice = false;
  public allPriceCategories = new Array<PriceCategory>();
  public ReturnCashAmount: number = 0;
  public ReturnCreditAmount: number = 0;
  public IsInvoiceClaimed = false;
  public requiresProcessConfirmation: boolean = false;
  public RequiredPermissionNameToConfirmProcess: string = 'billing-invoice-return-process';
  public ProcessToConfirmDisplayName: string = ENUM_ProcessesToConfirmDisplayNames.BillInvoiceReturn;
  public confirmationTitle: string = "Confirm !";
  public confirmationMessage: string = "Are you sure you want to Return Invoice ?";
  ...
  GetInvoiceByReceiptNo(receiptNo: number) {
    ...
    if (receiptNo && receiptNo != 0) {
      ...
      this.BillingBLService.GetInvoiceDetailsForCreditNote(recptNo, this.selFiscYrId, getVisitInfo, this.isInsuranceReceipt)
        .subscribe(res => {
          if (res.Status === ENUM_FromzaHTTPResponses.OK && res.Results && res.Results.IsInvoiceFound) {
            ...
            this.IsCoPaymentInvoice = this.crnInfoVM.InvoiceInfo.IsCoPayment;
            if (this.IsCoPaymentInvoice) {
              this.CheckClaimStatus(this.crnInfoVM.InvoiceInfo.ClaimCode, this.crnInfoVM.InvoiceInfo.PatientId);
            }
            ...
            //add messagebox here..
            this.messageBoxService.showMessage("error", ["Invoice Not Found"]);
      ...
      this.messageBoxService.showMessage('Failed', ["Your Receipt Number is not proper."]);
    }
  }
  ...
  CheckClaimStatus(claimCode: number, patientId: number): void {
    this.BillingBLService.IsClaimed(claimCode, patientId)
      .subscribe((res: FromzaHTTPResponse) => {
        ...
        (err: FromzaHTTPResponse) => {
          this.messageBoxService.showMessage(ENUM_FromzaHTTPResponses.Failed, ["Cannot Check the Claim Status"]);
        }
      );
  }
  ...
  ReCalculateReturnAmount() {
    ...
    if (selItems && selItems.length > 0) {
      ...
      //! Krishna, 11thMay'23 Need to revise below logic of calculating ReturnCashAmount and ReturnCreditAmount.
      if (this.IsCoPaymentInvoice && this.crnInfoVM.InvoiceInfo.PaymentMode.toLowerCase() === ENUM_BillPaymentMode.credit.toLowerCase()) {
        ...
        this.ReturnCreditAmount = this.totalReturnAmt;
      } else {
        this.ReturnCashAmount = this.totalReturnAmt;
      }
    }
  }
  ...
  GotoProcessConfirmation(): void {
    if (this.securityService.HasPermission(this.RequiredPermissionNameToConfirmProcess)) {
  ...
  SubmitCreditNote() {
    ...
    if (validationObj.IsValid) {
      ...
      retInvoiceModel.SchemeId = this.crnInfoVM.InvoiceInfo.SchemeId;
      ...
      retInvoiceModel.IsCoPayment = this.IsCoPaymentInvoice;
      retInvoiceModel.ReturnCashAmount = this.ReturnCashAmount;
      retInvoiceModel.ReturnCreditAmount = this.ReturnCreditAmount;
      retInvoiceModel.PriceCategoryId = this.crnInfoVM.InvoiceInfo.PriceCategoryId;
      retInvoiceModel.TransactionType = this.crnInfoVM.InvoiceInfo.TransactionType;
      retInvoiceModel.OrganizationId = this.crnInfoVM.InvoiceInfo.OrganizationId;
      retInvoiceModel.ClaimCode = this.crnInfoVM.InvoiceInfo.ClaimCode;
      ...
      this.BillingBLService.PostCreditNote(retInvoiceModel)
        .subscribe(res => {
          if (res.Status == 'OK') {
            this.messageBoxService.showMessage("success", ["Credit note saved successfully."]);
            this.selCreditNote = res.Results;
            this.showPrintPage = true;
            this.ResetValuesAfterSubmit();
            this.loading = false;
          }
          else if (res.ErrorMessage.match(/Sorry !!! Some Items are already returned./g)) {
            this.messageBoxService.showMessage("failed", [res.ErrorMessage]);
            this.loading = false;
          }
          else {
            this.messageBoxService.showMessage('Failed', ["Unable to save Credit Note. <br/>Please check log for details."]);
            console.log(res.ErrorMessage);
            this.loading = false;
          }
        },
          err => {
            this.messageBoxService.showMessage('Failed', ["Unable to save Credit Note"]);
      ...
      this.messageBoxService.showMessage("Failed", validationObj.Msgs);
      this.loading = false;
    }
  ...
  public GetReturnItemFromTransactionItem(ipTxnItemArr: Array<CRN_InvoiceItemsVM>) {
    ...
    if (ipTxnItemArr && ipTxnItemArr.length > 0) {
      ipTxnItemArr.forEach(itm => {
        ...
        newRetItmObj.ServiceItemId = itm.ServiceItemId;
        ...
        newRetItmObj.PrescriberId = itm.PrescriberId;
        newRetItmObj.PerformerId = itm.PerformerId;
        ...
        newRetItmObj.PriceCategory = itm.PriceCategory;//this need to be removed--Sud:16Apr'23
        newRetItmObj.PriceCategoryId = itm.PriceCategoryId;
        ...
        newRetItmObj.IsCashBillSyncToAcc = false;
        newRetItmObj.IsCreditBillSyncToAcc = false;
