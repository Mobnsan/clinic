...
import { ENUM_BillingStatus, ENUM_BillingType, ENUM_BillPaymentMode, ENUM_CurrentBillingFlow, ENUM_FromzaHTTPResponses, ENUM_FromzaHTTPResponseText, ENUM_InvoiceType, ENUM_LabTypes, ENUM_MembershipTypeName, ENUM_MessageBox_Status, ENUM_OrderStatus, ENUM_PriceCategory, ENUM_ServiceBillingContext, ENUM_ValidatorTypes } from '../../shared/shared-enums';
...
export class BillingTransactionComponent {
  ...
  CheckBillingValidations(): boolean {
    ...
    if (!this.DiscountPercentSchemeValid) {
      isFormValid = false;
      this.messageBoxService.showMessage(ENUM_FromzaHTTPResponseText.Failed, ["Discount scheme is mandatory. Default is: General(0%)"]);
    }
    ...
    if (this.model.PaymentMode.toLowerCase() === ENUM_BillPaymentMode.credit.toLowerCase() || (this.selectedPriceCategoryObj.IsCoPayment && this.model.CoPayment_PaymentMode.toLowerCase() === ENUM_BillPaymentMode.credit.toLowerCase())) {
      if (!this.model.OrganizationId || this.model.OrganizationId == 0) {
        isFormValid = false;
        this.messageBoxService.showMessage(ENUM_FromzaHTTPResponseText.Failed, ["Credit Organization is mandatory for credit bill"]);
      }
    }
    if (this.IsRemarksMandatory && !this.model.Remarks) {
      isFormValid = false;
      this.messageBoxService.showMessage(ENUM_FromzaHTTPResponseText.Failed, ["Remarks is mandatory for this payment mode"]);
    }

    if (this.model.BillingTransactionItems.some(a => a.DiscountAmount > a.SubTotal)) {
      isFormValid = false;
      this.messageBoxService.showMessage(ENUM_FromzaHTTPResponseText.Failed, ["Item Level Discount is not valid."]);
    }

    if (this.IsMedicarePatientBilling && !this.CheckMedicarePatientBillingValidations()) {
      isFormValid = false;
      this.messageBoxService.showMessage(ENUM_FromzaHTTPResponses.Failed, ["Credit Amount cannot exceed Credit limit."]);
    }
    ...
    if (this.isClaimSuccessful) {
      isFormValid = false;
      this.messageBoxService.showMessage(ENUM_FromzaHTTPResponses.Failed, ["This Visit context is claimed, create a new visit"]);
    ...
    if (this.model.BillingTransactionItems.length) {
      ...
      for (let i = 0; i < this.model.BillingTransactionItems.length; i++) {
        if (!this.model.BillingTransactionItems[i].IsValidSelDepartment) {
          this.messageBoxService.showMessage(ENUM_FromzaHTTPResponseText.Failed, ["Invalid Department. Please select Department from the list."]);
          this.loading = false;
          return false;
        }
        if (this.model.BillingTransactionItems[i].IsDoctorMandatory && !this.model.BillingTransactionItems[i].PerformerId) {
          this.messageBoxService.showMessage(ENUM_FromzaHTTPResponseText.Failed, ["Performer is mandatory for some items"]);
          this.loading = false;
          return false;
        }
        if (!this.model.BillingTransactionItems[i].IsvalidSelPerformerDr) {
          this.messageBoxService.showMessage(ENUM_FromzaHTTPResponseText.Failed, ["Invalid Assigned To Dr. Name. Please select doctor from the list."]);
          this.loading = false;
          return false;
        }
        if (!this.model.BillingTransactionItems[i].IsValidSelItemName) {
          this.messageBoxService.showMessage(ENUM_FromzaHTTPResponseText.Failed, ["Invalid Item Name. Please select Item from the list."]);
          this.loading = false;
          return false;
        }
        ...
        if (integrationName == "LAB" && this.LabTypeName == 'er-lab') {
          ...
          if (!aaa || !aaa.IsErLabApplicable) {
            this.messageBoxService.showMessage(ENUM_FromzaHTTPResponseText.Failed, ["Some items are not found in " + this.LabTypeName + ". "]);
            this.loading = false;
            return false;
          }
        }
  ...
  GetVisitContext(patientId: number, visitId: number) {
    if (patientId && visitId) {
      this.BillingBLService.GetDataOfInPatient(patientId, visitId)
        .subscribe(res => {
          if (res.Status === ENUM_FromzaHTTPResponseText.OK && res.Results) {
    ...
    this.BillingBLService.IsClaimed(LatestClaimCode, PatientId)
      .subscribe((res: FromzaHTTPResponse) => {
        ...
        (err: FromzaHTTPResponse) => {
          this.messageBoxService.showMessage(ENUM_FromzaHTTPResponses.Failed, ["Unable to check for pending claims"]);
        }
      );
  }
  ...
  GetMedicareMemberDetail(patientId: number): void {
    this.BillingBLService.GetMedicareMemberDetail(patientId).subscribe((res: FromzaHTTPResponse) => {
      if (res.Status === ENUM_FromzaHTTPResponses.OK) {
      ...
      this.messageBoxService.showMessage(ENUM_FromzaHTTPResponses.Failed, ["Could not fetch medicare member detail"]);
    });
  }
  ...
  GetServiceItems(serviceBillingContext: string, schemeId: number, priceCategoryId: number) {
    this.billingMasterBlService.GetServiceItems(serviceBillingContext, schemeId, priceCategoryId)
      ...
      .subscribe((res: FromzaHTTPResponse) => {
        if (res.Status === ENUM_FromzaHTTPResponses.OK && res.Results.length > 0) {
  ...
  GetServiceItemSchemeSetting(serviceBillingContext: string, schemeId: number, priceCategoryId: number) {
    this.billingMasterBlService.GetServiceItemSchemeSetting(serviceBillingContext, schemeId)
      ...
      .subscribe((res: FromzaHTTPResponse) => {
        if (res.Status === ENUM_FromzaHTTPResponseText.OK && res.Results.length > 0) {
