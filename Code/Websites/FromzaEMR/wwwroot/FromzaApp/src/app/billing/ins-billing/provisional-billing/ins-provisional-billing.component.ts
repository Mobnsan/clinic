    ...
    this.billingBLService.GetUnpaidInsuranceTotalBills()
      .subscribe((res: FromzaHTTPResponse) => {
        if (res.Status == "OK") {
          ...
          this.messageBoxService.showMessage("failed", ["Not able to get Insurance Provisional items."]);
        }
      });
  }
      ...
      this.billingBLService.PayProvisional(this.billingTransaction)
        .subscribe((res: FromzaHTTPResponse) => {
          if (res.Status == "OK") {
            ...
            this.messageBoxService.showMessage("failed", ["couldn't submit the request. please check the log for detail."], res.ErrorMessage);
            this.loading = false;
          }
        },
          err => {
            this.loading = false;
            this.messageBoxService.showMessage("error", ["couldn't submit the request. please check the log for detail."], err);
          });
    }
    else {
      this.messageBoxService.showMessage("error", ["Sorry..! Unable to process empty receipt."]);
