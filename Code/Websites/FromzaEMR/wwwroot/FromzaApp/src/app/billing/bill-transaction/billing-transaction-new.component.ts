...
import { ENUM_AdditionalServiceItemGroups, ENUM_BillPaymentMode, ENUM_BillingStatus, ENUM_BillingType, ENUM_CurrentBillingFlow, ENUM_FromzaHTTPResponses, ENUM_InvoiceType, ENUM_MessageBox_Status, ENUM_OrderStatus, ENUM_ServiceBillingContext } from "../../shared/shared-enums";
...
export class BillingTransactionComponent_New {
    ...
    this.billingBlService.GetPatientPastBillSummary(patientId)
      .subscribe(res => {
        if (res.Status === ENUM_FromzaHTTPResponses.OK) {
          ...
          this.msgBoxService.showMessage(ENUM_FromzaHTTPResponses.Failed, [res.ErrorMessage]);
          this.loading = false;
        }
      });
  }
  ...
  public GetPatientVisitList(patientId: number) {
    this.billingBlService.GetPatientVisitsProviderWise(patientId)
      .subscribe((res: FromzaHTTPResponse) => {
        ...
        err => {
          this.msgBoxService.showMessage(ENUM_FromzaHTTPResponses.Failed, ["unable to get PatientVisit list.. check log for more details."]);
      ...
      this.billingBlService.GetDataOfInPatient(patientId, visitId)
        .subscribe(res => {
          if (res.Status === ENUM_FromzaHTTPResponses.OK && res.Results) {
            ...
            console.log(ENUM_FromzaHTTPResponses.Failed, ["Problem! Cannot get the Current Visit Context ! "]);
          }
        },
    ...
    this.billingBlService.IsClaimed(LatestClaimCode, PatientId)
      .subscribe((res: FromzaHTTPResponse) => {
        ...
        (err: FromzaHTTPResponse) => {
          this.msgBoxService.showMessage(ENUM_FromzaHTTPResponses.Failed, ["Unable to check for pending claims"]);
        }
      );
  }
  ...
  CheckAndSubmitBillingTransaction() {
    ...
    if (this.isClaimSuccessful) {
      this.msgBoxService.showMessage(ENUM_FromzaHTTPResponses.Failed, ["This Visit context is claimed, create a new visit"]);
    }
  ...
  GetServicePackages(schemeId: number, priceCategoryId: number): void {
    if (schemeId && priceCategoryId) {
      ...
      this.billingMasterBlService.GetServicePackages(schemeId, priceCategoryId).subscribe((res: FromzaHTTPResponse) => {
        if (res.Status === ENUM_FromzaHTTPResponses.OK && res.Results) {
