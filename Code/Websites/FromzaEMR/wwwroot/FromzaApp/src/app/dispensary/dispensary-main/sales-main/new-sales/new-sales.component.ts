...
import { ENUM_BillPaymentMode, ENUM_BillingStatus, ENUM_FromzaHTTPResponses, ENUM_MessageBox_Status, ENUM_ModuleName, ENUM_PriceCategory, ENUM_ServiceBillingContext } from '../../../../shared/shared-enums';
...
export class NewSalesComponent implements OnInit, OnDestroy {
  ...
  CallBackGetItemTypeList(res: FromzaHTTPResponse) {
    try {
      if (res.Status == ENUM_FromzaHTTPResponses.OK) {
        if (res.Results) {
          this.ItemList = [];
  ...
  SaveNMCNo() {
    ...
    this.pharmacyBLService.UpdateNMCNo(this.currSale.PrescriberId, this.MedicalCertificateNo).subscribe((res: FromzaHTTPResponse) => {
      if (res.Status === ENUM_FromzaHTTPResponses.OK) {
        this.NMCNoAddPopup = false;
  ...
  GetPriceCategories() {
    this.pharmacyBLService.GetPriceCategories().subscribe((res: FromzaHTTPResponse) => {
      if (res.Status === ENUM_FromzaHTTPResponses.OK) {
  ...
  LoadDefaultScheme() {
    this.pharmacyBLService.GetDefaultScheme(ENUM_ServiceBillingContext.OpPharmacy).subscribe((res: FromzaHTTPResponse) => {
      if (res.Status === ENUM_FromzaHTTPResponses.OK && res.Results && res.Results.length) {
