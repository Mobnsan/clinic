...
export class SalesListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.isItemLevelVATApplicable || this.isMainVATApplicable) {
      let checkField = this.saleGridColumns.filter(x => x.headerName == 'Vat Amt');
      if (checkField.length < 1) {
        this.saleGridColumns.splice(5, 0, addVATAmountCol);
      }
  ...
  LoadSaleInvoiceList(): void {
    ...
    try {
      this.loading = true;
      this.pharmacyBLService.GetSaleInvoiceList(this.fromDate, this.toDate, this.currentDispensary.StoreId).finally(() => { this.loading = false; })
        .subscribe(res => {
          if (res.Status == 'OK') {
