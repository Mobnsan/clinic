...
import { ENUM_BillPaymentMode, ENUM_FromzaHTTPResponses, ENUM_Dispensary_ReturnInvoiceBy, ENUM_MessageBox_Status, ENUM_PriceCategory, ENUM_ServiceBillingContext, ENUM_VisitType } from '../../../../shared/shared-enums';
...
export class SalesReturnComponent implements OnInit {
  ...
  private checkIfInvoiceIsClaimedForSSF(res: FromzaHTTPResponse) {
    this.pharmacyBLService.IsClaimed(res.Results.invoiceHeader.ClaimCode, res.Results.patient.PatientId)
      .subscribe((response: FromzaHTTPResponse) => {
        if (response.Status === ENUM_FromzaHTTPResponses.OK && response.Results === true) {
  ...
  private handleNormalInvoiceReturnScenario(res: FromzaHTTPResponse) {
  ...
  private setInvoiceReturnDetailsToReturn(i: number, res: FromzaHTTPResponse) {
  ...
  GetInvoiceDetailsToReturnByHospitalNo(): void {
    ...
    this.pharmacyBLService.GetReturnFromCustomerModelDataByHospitalNo(this.HospitalNo, this.PaymentMode, this.fromDate, this.toDate, this.storeId, this.SchemeId).subscribe((res: FromzaHTTPResponse) => {
      if (res.Status === ENUM_FromzaHTTPResponses.OK) {
  ...
  public GetPriceCategories(): void {
    this.coreBLService.GetPriceCategories().subscribe((res: FromzaHTTPResponse) => {
      if (res.Status === ENUM_FromzaHTTPResponses.OK) {
