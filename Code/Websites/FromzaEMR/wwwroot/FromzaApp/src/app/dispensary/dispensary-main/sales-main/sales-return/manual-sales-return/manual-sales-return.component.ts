...
import { ENUM_BillPaymentMode, ENUM_FromzaHTTPResponses, ENUM_MessageBox_Status } from '../../../../../shared/shared-enums';
...
export class ManualSalesReturnComponent implements OnInit {
  ...
  onBatchNoClicked(index: number) {
    ...
    if (selectedItemRow.selectedBatch.BatchNo == null) {
      ...
      selectedItemRow.SalePrice = selectedItemRow.selectedBatch.SalePrice;
  ...
  calculation() {
    ...
    this.salesReturn.ReturnCashAmount = totalAmount;
  }
  // Post Return
  postManualReturn() {
  ...
  showReceipt(invoiceDataFromServer) {
    ...
    if (this.pharmacyReceipt.PaymentMode !== ENUM_BillPaymentMode.cash) {
      this.pharmacyReceipt.CreditAmount = this.pharmacyReceipt.TotalAmount;
    }
    else {
      this.pharmacyReceipt.CashAmount = this.pharmacyReceipt.TotalAmount;
    }
