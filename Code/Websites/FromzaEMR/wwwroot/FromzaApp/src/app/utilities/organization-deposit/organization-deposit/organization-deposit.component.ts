...
import { ENUM_ACC_PaymentMode, ENUM_BillDepositType, ENUM_BillPaymentMode, ENUM_FromzaHTTPResponses, ENUM_MessageBox_Status } from '../../../shared/shared-enums';
...
export class OrganizationDepositComponent implements OnInit {
  ...
  public getCreditOrganizationList(searchText: string) {
    if (searchText.trim().length < 3) {
      this.utilitiesBlService.GetCreditOrganizationList(searchText).subscribe((res) => {
        if (res.Status === ENUM_FromzaHTTPResponses.OK && res.Results && res.Results.length > 0) {
  ...
  public onDepositSaved(depositId: number) {
    ...
    this.utilitiesBlService.GetDepositDetails(depositId).subscribe((res: FromzaHTTPResponse) => {
      if (res.Status === ENUM_FromzaHTTPResponses.OK) {
