...
export class AccountingService {
  ...
  public accCacheData: AccCacheDataVM = new AccCacheDataVM();//mumbai-team-june2021-fromza-accounting-cache-change
  ...
  public getnamebyCode(code) {//mumbai-team-june2021-fromza-accounting-cache-change
    try {
      var codeList = this.accCacheData.CodeDetails.filter(a => a.Code == code);//mumbai-team-june2021-fromza-accounting-cache-change
    ...
    // return (codeList.length > 0) ? codeList.find(a => a.Code == code).Name : "";//mumbai-team-june2021-fromza-accounting-cache-change
  }

  //mumbai-team-june2021-fromza-accounting-cache-change
  public clearAccCacheDataFromFromzaCache() {
    FromzaCache.clearFromzaCacheByType(MasterType.Ledgers);
    FromzaCache.clearFromzaCacheByType(MasterType.VoucherHead);
    FromzaCache.clearFromzaCacheByType(MasterType.VoucherType);
    FromzaCache.clearFromzaCacheByType(MasterType.LedgerGroups);
    FromzaCache.clearFromzaCacheByType(MasterType.CodeDetails);
    FromzaCache.clearFromzaCacheByType(MasterType.PrimaryGroup);
    FromzaCache.clearFromzaCacheByType(MasterType.COA);
    FromzaCache.clearFromzaCacheByType(MasterType.LedgersAll);
    FromzaCache.clearFromzaCacheByType(MasterType.SubLedgerAll);
    FromzaCache.clearFromzaCacheByType(MasterType.CostCenters);
  }
  ...
  public async getAccCacheData() {
    ...
    this.accCacheData.Ledgers = await FromzaCache.GetAccCacheData(MasterType.Ledgers, null);
    this.accCacheData.VoucherHead = await FromzaCache.GetAccCacheData(MasterType.VoucherHead, null);
    this.accCacheData.VoucherType = await FromzaCache.GetAccCacheData(MasterType.VoucherType, null);
    this.accCacheData.LedgerGroups = await FromzaCache.GetAccCacheData(MasterType.LedgerGroups, null);
    this.accCacheData.CodeDetails = await FromzaCache.GetAccCacheData(MasterType.CodeDetails, null);
    this.accCacheData.PrimaryGroup = await FromzaCache.GetAccCacheData(MasterType.PrimaryGroup, null);
    this.accCacheData.COA = await FromzaCache.GetAccCacheData(MasterType.COA, null);
    this.accCacheData.LedgersALL = await FromzaCache.GetAccCacheData(MasterType.LedgersAll, null);
    this.accCacheData.SubLedgerAll = await FromzaCache.GetAccCacheData(MasterType.SubLedgerAll, null);
    this.accCacheData.CostCenters = await FromzaCache.GetAccCacheData(MasterType.CostCenters, null);
  ...
  Print(table, dateRange) {
    ...
    popupWinindow = window.open(
      "",
      "_blank",
      "width=600,height=700,scrollbars=no,menubar=no,toolbar=no,location=no,status=no,titlebar=no,fromza-date-change=no "
    );
