...
import { ENUM_ACC_ADDLedgerLedgerType, ENUM_FromzaHTTPResponses, ENUM_Data_Type, ENUM_MessageBox_Status } from "../../../../shared/shared-enums";
import { AccountingSettingsBLService } from "../../shared/accounting-settings.bl.service";
import { LedgerModel } from "../../shared/ledger.model";
import { ledgerGroupModel } from "../../shared/ledgerGroup.model";

@Component({
    selector: 'credit-organization-ledger-mapping',
    templateUrl: "./credit-org-ledger-mapping.component.html"
})

export class CreditOrganizationLedgerMappingComponent {
    ...
    public creditOrganizationList: any;
    ...
    public creditOrganizationLedgerParam = {
    ...
    public getCoaList() {
        if (!!this.accountingService.accCacheData.COA && this.accountingService.accCacheData.COA.length > 0) { //mumbai-team-june2021-fromza-accounting-cache-change
            this.allcoaList = this.accountingService.accCacheData.COA; //mumbai-team-june2021-fromza-accounting-cache-change
            this.allcoaList = this.allcoaList.slice();//mumbai-team-june2021-fromza-accounting-cache-change
        }
    }
    public getPrimaryGroupList() {
        if (!!this.accountingService.accCacheData.PrimaryGroup && this.accountingService.accCacheData.PrimaryGroup.length > 0) {//mumbai-team-june2021-fromza-accounting-cache-change
            this.primaryGroupList = this.accountingService.accCacheData.PrimaryGroup;//mumbai-team-june2021-fromza-accounting-cache-change
            this.primaryGroupList = this.primaryGroupList.slice();//mumbai-team-june2021-fromza-accounting-cache-change
        }
    }
    public Getledgers() {
        try {
            ...
            if (ledgers.length > 0) {
                ...
                this.creditOrganizationLedgerParam = this.ledgerTypeParamter.find(a => a.LedgerType === ENUM_ACC_ADDLedgerLedgerType.CreditOrganization);
            } else {
    ...
    GetLedgerGroup() {
        if (!!this.accountingService.accCacheData.LedgerGroups && this.accountingService.accCacheData.LedgerGroups.length > 0) { //mumbai-team-june2021-fromza-accounting-cache-change
            this.CallBackLedgerGroup(this.accountingService.accCacheData.LedgerGroups);//mumbai-team-june2021-fromza-accounting-cache-change
        }
    }
    ...
    //adding new Ledger
    AddLedger() {
        this.NewledgerList = this.creditOrganizationList.filter(a => a.IsSelected == true);
    ...
    CallBackAddLedger(res) {
        if (res.Status === ENUM_FromzaHTTPResponses.OK && res.Results != null) {
            res.Results.forEach(ledger => {//mumbai-team-june2021-fromza-accounting-cache-change
                ...
                this.accountingService.accCacheData.LedgersALL.push(ledger);//mumbai-team-june2021-fromza-accounting-cache-change
            });
        }
        else if (res.Status === ENUM_FromzaHTTPResponses.OK && res.Results == null) {
    ...
    public getLedgerList() {
        if (!!this.accountingService.accCacheData.LedgersALL && this.accountingService.accCacheData.LedgersALL.length > 0) {//mumbai-team-june2021-fromza-accounting-cache-change
            this.sourceLedgerList = this.accountingService.accCacheData.LedgersALL;//mumbai-team-june2021-fromza-accounting-cache-change
            this.sourceLedgerList = this.sourceLedgerList.slice();//mumbai-team-june2021-fromza-accounting-cache-change
        }
    }
    ...
    ChangeOpeningBalType(e, index: number) {
        ...
        if (e.target.name == "Dr") {
            if (e.target.checked) {
                this.creditOrganizationList[index].DrCr = true;
                this.creditOrganizationList[index].Cr = false;
                this.creditOrganizationList[index].Dr = true;
            }
        }
        if (e.target.name == "Cr") {
            if (e.target.checked) {
                this.creditOrganizationList[index].DrCr = false;
                this.creditOrganizationList[index].Dr = false;
                this.creditOrganizationList[index].Cr = true;
            }
        }
    }
    CheckDrCrValidation() {
        ...
        try {
            if (searchKey && searchKey.trim()) {
                ...
                if (this.ConsultantfilterType == 'withacchead') {
                    let crOrganizationData = this.creditOrganizationList.filter(l => (l.LedgerId > 0));
                    this.creditOrganizationList = crOrganizationData.filter(l => l.OrganizationName.toLowerCase().indexOf(searchKey.toLowerCase()) > -1 && l.LedgerId > 0);
                }
                    ...
                    this.creditOrganizationList = this.creditOrgledgerList.filter(l => l.OrganizationName.toLowerCase().indexOf(searchKey.toLowerCase()) > -1);
                }
            }
            else {
                if (this.ConsultantfilterType == 'withacchead') {
                    this.creditOrganizationList = this.creditOrgledgerList.filter(l => l.LedgerId > 0);
                }
                else if (this.ConsultantfilterType == 'withoutacchead') {
                    this.creditOrganizationList = this.creditOrgledgerList.filter(l => l.LedgerId == 0);
                }
                else {
                    this.creditOrganizationList = this.creditOrgledgerList;
                }
            }
        }
        catch (ex) {
    ...
    AssignSelectedLedger(index) {
        try {
            let oldLedgerId = this.creditOrganizationList[index] ? this.creditOrganizationList[index].LedgerId : 0;
            var ledgerNameSelected = (typeof (this.creditOrganizationList[index].LedgerName) == 'object') ? this.creditOrganizationList[index].LedgerName.LedgerName.trim().toLowerCase() : this.creditOrganizationList[index].LedgerName.trim().toLowerCase();
            var ledger = this.ledgerListAutoComplete.filter(l => l.LedgerName.trim().toLowerCase() == ledgerNameSelected);
            // var ledger = this.ledgerListAutoComplete.filter(l => l.LedgerName.trim().toLowerCase()  == this.creditOrganizationList[index].LedgerName.trim().toLowerCase() );
            if (ledger.length > 0) {
                this.creditOrganizationList[index].Code = ledger[0].Code;
                this.creditOrganizationList[index].LedgerId = ledger[0].LedgerId;
                this.creditOrganizationList[index].LedgerName = ledger[0].LedgerName;
            } else {
                this.creditOrganizationList[index].Code = "";
                this.creditOrganizationList[index].LedgerId = 0;
            }
            if (oldLedgerId !== this.creditOrganizationList[index].LedgerId) {
                this.creditOrganizationList[index].SubLedgerName = "";
                this.creditOrganizationList[index].SubLedgerId = 0;
    ...
    SelectAllChkOnChange() {
        if (this.isSelectAll) {
            let ledgerObj = this.ledgerListAutoComplete.find(a => a.Name === this.creditOrganizationLedgerParam.LedgerName)
            this.creditOrganizationList.forEach((a, index) => {
                a.IsSelected = true;
                a.IsActive = true;
                if (a.IsSelected) {
                    if (a.IsMapped == false) {
                        a.LedgerName = this.subLedgerAndCostCenterSetting.EnableSubLedger ? (ledgerObj.LedgerName) : a.OrganizationName;
                        ...
                        a.SubLedgerName = a.OrganizationName;
                        this.selectedSubLedger[index] = a.OrganizationName;
                    }
                }
    ...
    SingleCkboxChange(index) {
        this.selectedLedgerCount = this.creditOrganizationList.filter(a => a.IsSelected == true).length;

        if (this.creditOrganizationList[index].IsSelected) {
            if (this.creditOrganizationList[index].IsMapped == false) {
                this.creditOrganizationList[index].LedgerName = ""; // remove autocomplete Ledgername
                this.creditOrganizationList[index].Code = "";
                this.creditOrganizationList[index].LedgerId = 0;
            }
            this.creditOrganizationList[index].LedgerValidator.get("LedgerName").enable();
        } else {
            if (this.creditOrganizationList[index].IsMapped == false) {
                this.creditOrganizationList[index].LedgerName = "";
                this.creditOrganizationList[index].Code = "";
                this.creditOrganizationList[index].LedgerId = 0;
                this.creditOrganizationList[index].SubLedgerId = 0;
                this.creditOrganizationList[index].SubLedgerName = "";
                ...
                var ledger = this.ledgerListAutoComplete.find(l => l.LedgerName == this.creditOrganizationList[index].LedgerName);
                if (!ledger) {
                    let data = this.ledgerListAutoComplete.find(l => l.LedgerId == this.creditOrganizationList[index].LedgerId);
                    if (data) {
                        this.creditOrganizationList[index].Code = data.Code;
                        this.creditOrganizationList[index].LedgerId = data.LedgerId;
                        this.creditOrganizationList[index].LedgerName = data.LedgerName;
                    }
                }
            }
            this.creditOrganizationList[index].LedgerValidator.get("LedgerName").disable();
        }
    }


    ShowSaveButtonOnCkboxChange() {
        this.isSelectAll = this.creditOrganizationList.every(a => a.IsSelected == true);
        this.selectedLedgerCount = this.creditOrganizationList.filter(a => a.IsSelected == true).length;
    }
    ...
    SetCreditOrganizationData() {
        ...
        this.CurrentLedger = new LedgerModel();
        let LedgerGroupName = this.ledgerTypeParamter.find(a => a.LedgerType == ENUM_ACC_ADDLedgerLedgerType.CreditOrganization).LedgergroupUniqueName;
        ...
        if (consultLedger != null || consultLedger != undefined) {
            ...
            this.subLedgerListForCreditOrganization = this.subLedgerMaster.filter(a => this.ledgerListAutoComplete.some(b => a.LedgerId === b.LedgerId));
            ...
            .subscribe(res => {
                if (res.Status === ENUM_FromzaHTTPResponses.OK) {
                    ...
                    data.forEach((emp, index) => {
                        ...
                        led.LedgerType = ENUM_ACC_ADDLedgerLedgerType.CreditOrganization,
                    ...
                    this.creditOrganizationList = this.creditOrgledgerList;
                    this.totalLedger = this.creditOrganizationList.length;
                    this.mappedLedger = this.creditOrganizationList.filter(l => l.IsMapped == true).length;
                    this.notmappedLedger = this.creditOrganizationList.filter(l => l.IsMapped == false).length;
                }
            });
    }
    ...
    ToggleCreditOrgLedger(mapped) {
        if (mapped == 'true') {
            ...
            this.creditOrganizationList = this.creditOrgledgerList.filter(emp => emp.LedgerId > 0);
    ...
    AssignSelectedSubLedger(index) {
        if (typeof this.selectedSubLedger[index] === ENUM_Data_Type.Object && this.selectedSubLedger[index].SubLedgerId > 0) {
            this.creditOrganizationList[index].Code = this.selectedSubLedger[index].SubLedgerCode
            this.creditOrganizationList[index].LedgerId = this.selectedSubLedger[index].LedgerId;
            this.creditOrganizationList[index].SubLedgerName = this.selectedSubLedger[index].SubLedgerName;
            this.creditOrganizationList[index].SubLedgerId = this.selectedSubLedger[index].SubLedgerId;
            var Ledger = this.ledgerListAutoComplete.find(a => a.LedgerId === this.selectedSubLedger[index].LedgerId);
            if (Ledger) {
                this.creditOrganizationList[index].LedgerName = Ledger.LedgerName;
            }
        }
        else {
            if (this.selectedSubLedger[index].trim() === "") {
                ...
                this.creditOrganizationList[index].SubLedgerName = this.selectedSubLedger[index];
            }
        }
    }
