...
export class COAListComponent {
    ...
    public getCoaList() {
        if (!!this.accountingService.accCacheData.COA && this.accountingService.accCacheData.COA.length > 0) {//mumbai-team-june2021-fromza-accounting-cache-change
            this.coaList = this.accountingService.accCacheData.COA; //mumbai-team-june2021-fromza-accounting-cache-change
            this.coaList = this.coaList.slice();//mumbai-team-june2021-fromza-accounting-cache-change
        ...
        if (this.coaObj.IsValidCheck(undefined, undefined)) {
            this.accountingSettingsBLService.PostCOA(this.coaObj)
                .subscribe(
                    res => {
                        if (res.Status == "OK") {
                            this.accountingService.accCacheData.COA.push(res.Results); //mumbai-team-june2021-fromza-accounting-cache-change
                            this.getCoaList();
            ...
            this.accountingSettingsBLService.UpdateCOA(this.coaObj)
                .subscribe(
                    res => {
                        ...
                        let index = this.accountingService.accCacheData.COA.findIndex(x => x.ChartOfAccountId == this.coaObj.ChartOfAccountId)//mumbai-team-june2021-fromza-accounting-cache-change    
                        this.accountingService.accCacheData.COA.splice(index,1); //mumbai-team-june2021-fromza-accounting-cache-change    
                        this.accountingService.accCacheData.COA.push(res.Results); //mumbai-team-june2021-fromza-accounting-cache-change
                        this.getCoaList();
