...
export class LedgerGroupListComponent {
    ...
    public getLedgerGroupList() {
            if(!!this.accountingService.accCacheData.LedgerGroups && this.accountingService.accCacheData.LedgerGroups.length>0){//mumbai-team-june2021-fromza-accounting-cache-change
                ...
                this.ledgerGroupList = this.ledgerGroupList.slice();//mumbai-team-june2021-fromza-accounting-cache-change
                this.showGrid = true;
    ...
    CallBackAdd($event) {
      this.getLedgerGroupList();//mumbai-team-june2021-fromza-accounting-cache-change
        this.showAddPage = false;
    ...
    LedgerGroupGridActions($event: GridEmitModel) {
        switch ($event.Action) {
            case "activateDeactivateBasedOnStatus": {
                if ($event.Data != null) {
                    this.selectedLedgerGroup = null;
                    this.index = $event.RowIndex;//mumbai-team-june2021-fromza-accounting-cache-change
