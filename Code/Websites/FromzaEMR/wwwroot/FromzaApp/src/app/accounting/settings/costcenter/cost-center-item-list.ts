...
import { ENUM_FromzaHTTPResponseText, ENUM_Data_Type, ENUM_MessageBox_Status } from "../../../shared/shared-enums";
...
export class CostCenterItemListComponent {
    ...
    public GetCostCenters(): void {
        try {
            this.accountingSettingsBLService.GetCostCenters().subscribe((res: FromzaHTTPResponse) => {
    ...
    public GetParentCostCenters(): void {
        this.accountingSettingsBLService.GetParentCostCenter().subscribe((res: FromzaHTTPResponse) => {
            if (res.Status === ENUM_FromzaHTTPResponseText.OK) {
    ...
    UpdateCostCenter(): void {
        if (this.CheckValidation()) {
            try {
                ...
                if (this.CostCenter.IsValidCheck(undefined, undefined)) {
                    this.accountingSettingsBLService.UpdateCostCenter(this.CostCenter).subscribe((res: FromzaHTTPResponse) => {
                        if (res.Status === ENUM_FromzaHTTPResponseText.OK) {
                            this.CostCenter = new CostCenterModel();
                            ...
                            this.msgBoxServ.showMessage(ENUM_FromzaHTTPResponseText.Failed, ["Failed to update costcenter details."]);
    ...
    ActivateDeactivateCostCenter(costCenter: CostCenterModel) {
        try {
            this.accountingSettingsBLService.ActivateDeactiveCostCenter(costCenter).subscribe((res: FromzaHTTPResponse) => {
                if (res.Status === ENUM_FromzaHTTPResponseText.OK) {
                    this.CostCenter = new CostCenterModel();
