...
import { ENUM_FromzaHTTPResponseText, ENUM_FromzaHTTPResponses, ENUM_DateTimeFormat, ENUM_MessageBox_Status } from '../../shared/shared-enums';
...
export class TransferToAccountingComponent {
    ...
    this.settingsBLService.GetApplicationList()
      .subscribe(res => {
        if (res.Status === ENUM_FromzaHTTPResponseText.OK) {
          ...
          let sList = this.accountingService.accCacheData.Sections.filter(sec => sec.SectionId != 4); // 4 is Manual_Voucher (FIXED for FromzaEMR) //mumbai-team-june2021-fromza-accounting-cache-change
          sList.forEach(s => {
            ...
            if (pp != null || pp != undefined) {
              ...
              this.sectionList = this.sectionList.slice();//mumbai-team-june2021-fromza-accounting-cache-change
            }
          })
  ...
  public GetVoucherList() {
    try {
      if (!!this.accountingService.accCacheData.VoucherType && this.accountingService.accCacheData.VoucherType.length > 0) {//mumbai-team-june2021-fromza-accounting-cache-change
        this.voucherList = this.accountingService.accCacheData.VoucherType//mumbai-team-june2021-fromza-accounting-cache-change
      }
    }
    catch (exception) {
  ...
  public GetLedgerGroupList() {
    try {
      if (!!this.accountingService.accCacheData.LedgerGroups && this.accountingService.accCacheData.LedgerGroups.length > 0) {//mumbai-team-june2021-fromza-accounting-cache-change
        this.ledgerGroupList = new Array<ledgerGroupModel>();//mumbai-team-june2021-fromza-accounting-cache-change
        this.ledgerGroupList = this.accountingService.accCacheData.LedgerGroups//mumbai-team-june2021-fromza-accounting-cache-change
      }
    } catch (ex) {
  ...
  GetCostCenterList() {
    try {
      if (!!this.accountingService.accCacheData.CostCenters && this.accountingService.accCacheData.CostCenters.length > 0) {//mumbai-team-june2021-fromza-accounting-cache-change
        this.costCenterList = this.accountingService.accCacheData.CostCenters;//mumbai-team-june2021-fromza-accounting-cache-change
  ...
  SaveConfirm() {
    try {
      ...
      this.accountingBLService.PostTxnListToACC(this.postData)
        .subscribe(res => {
          if (res.Status === ENUM_FromzaHTTPResponseText.OK && res.Results != null) {
            this.loading = false;
          ...
          else if (res.Status === ENUM_FromzaHTTPResponseText.Failed) {
            this.loading = false;
  ...
  PrintLedger() {
    ...
    documentContent += '<link rel="stylesheet" type="text/css" href="../../themes/theme-default/FromzaStyle.css"/>';
      ...
      if (this.IsVAlidDate) {
        this.accountingBLService.GetPharmItemsForTransferToACC(this.selectedDate, this.fiscalYearId)
          ...
          .subscribe(res => {
            ...
            // else if (res.Status === ENUM_FromzaHTTPResponses.Failed && res.Results.length > 0) {
            //   this.unavailableLedgerList = res.Results;
            //   this.showunavailableList = true;
            //   this.loadingScreen = false;
            // }
  ...
  public GetTransferData(itemList: Array<any>) {
    try {
      if (!!this.accountingService.accCacheData.Ledgers && this.accountingService.accCacheData.Ledgers.length) {//mumbai-team-june2021-fromza-accounting-cache-change
        this.ledgerList = new Array<LedgerModel>();
        this.ledgerList = this.accountingService.accCacheData.LedgersALL;//mumbai-team-june2021-fromza-accounting-cache-change
