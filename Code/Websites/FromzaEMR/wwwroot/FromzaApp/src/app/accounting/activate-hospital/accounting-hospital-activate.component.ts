...
export class ActivateAccountingHospitalComponent {
    ...
    public loading:boolean=false; //mumbai-team-june2021-fromza-accounting-cache-change
    ...
    AssignAccountingTenant(tenantId) {
        this.loading=true; //mumbai-team-june2021-fromza-accounting-cache-change
        let isClearAccCacheData = (this.securityServ.AccHospitalInfo.ActiveHospitalId == tenantId) ? false : true; //mumbai-team-june2021-fromza-accounting-cache-change
        let hosp = this.allHospitalsPermissions.find(h => h.HospitalId == tenantId);
        if (hosp) {
            this.accBLService.ActivateAccountingTenant(tenantId)
                .subscribe(async (res: FromzaHTTPResponse) => {
                    if (res.Status == "OK") {
                        if(isClearAccCacheData==true){ //mumbai-team-june2021-fromza-accounting-cache-change
                            this.accservice.clearAccCacheDataFromFromzaCache();//mumbai-team-june2021-fromza-accounting-cache-change
                        }
                        this.securityServ.SetAccHospitalInfo(res.Results);
                        await this.accservice.getAccCacheData().then(res=>{ //mumbai-team-june2021-fromza-accounting-cache-change
                            this.loading= false;//mumbai-team-june2021-fromza-accounting-cache-change
                        })
                        ...
                        if (!!this.accservice.accCacheData.CodeDetails && this.accservice.accCacheData.CodeDetails.length > 0) { //mumbai-team-june2021-fromza-accounting-cache-change
                            this.coreService.SetCodeDetails(this.accservice.accCacheData.CodeDetails); //mumbai-team-june2021-fromza-accounting-cache-change
                        }
                        if (!!this.accservice.accCacheData.FiscalYearList && this.accservice.accCacheData.FiscalYearList.length > 0) { //mumbai-team-june2021-fromza-accounting-cache-change
                            this.coreService.SetFiscalYearList(this.accservice.accCacheData.FiscalYearList); //mumbai-team-june2021-fromza-accounting-cache-change
                        }               
