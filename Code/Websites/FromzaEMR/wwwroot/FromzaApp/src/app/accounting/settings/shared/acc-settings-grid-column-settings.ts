...
export default class AccSettingsGridColumnSettings {
    ...
    static BillingLedgerMapingActions_CellRenderer(params) {
        let template = `
            <a fromza-grid-action="map" class="grid-action">Map</a>
        `;

        if (params.data.LedgerCode) {
            if (params.data.IsActive == true) {
                template += `
                    <a fromza-grid-action="activateDeactivateBillingLedgerMapping" class="grid-action">Disable</a>
                `;
            } else {
                template += `
                    <a fromza-grid-action="activateDeactivateBillingLedgerMapping" class="grid-action">Enable</a>
                `;
            }
        }
