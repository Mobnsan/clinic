...
import { ENUM_ACC_ADDLedgerLedgerType, ENUM_ACC_PaymentMode, ENUM_FromzaHTTPResponseText, ENUM_Data_Type, ENUM_MessageBox_Status } from "../../../shared/shared-enums";
...
export class PaymentComponent {
  ...
  public loadCacheList() {
    if (!!this.accountingService.accCacheData.Sections && this.accountingService.accCacheData.Sections.length > 0) {//mumbai-team-june2021-fromza-accounting-cache-change
      this.SectionList = this.accountingService.accCacheData.Sections;//mumbai-team-june2021-fromza-accounting-cache-change
    ...
    if (!!this.accountingService.accCacheData.LedgersALL && this.accountingService.accCacheData.LedgersALL.length > 0) {//mumbai-team-june2021-fromza-accounting-cache-change
      this.AllLedgerList = this.accountingService.accCacheData.LedgersALL;//mumbai-team-june2021-fromza-accounting-cache-change
      this.AllLedgerList = this.FilteredLedgerList = this.AllLedgerList.slice();//mumbai-team-june2021-fromza-accounting-cache-change
    }
  }
  ...
  public loadInvGrList(grId, SectionId, number, Date) {
    this.accountingBlService.GetGRList(grId, SectionId, number, Date)
      .subscribe(res => {
        if (res.Status === ENUM_FromzaHTTPResponseText.OK && res.Results.length) {
