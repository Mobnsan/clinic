  ...
  formatDataforDisplay() {
    ...
    if (this.RevenueData != undefined || this.RevenueData != null) {
      this.RevenueData.COAList.forEach(a => {
        if (true) {
          RevData = this.pushToList(RevData, a.COA, a.TotalAmount, "BoldCategoryCOA", 0, 0);
          a.LedgerGroupList.forEach(b => {
            ...
            b.LedgerList.forEach(c => {
              RevData = this.pushToList(RevData, c.LedgerName, CommonFunctions.parseAmount(c.Amount), "ledgerLevel", c.LedgerId, c.Code);
            });
          });
        }
      });
    ...
    let revData = _.cloneDeep(RevData.filter(a => a.Style === 'BoldCategoryCOA'));
    GrossAmt = revData.reduce((a, b) => a + b.Amount, 0);
    RevData = this.pushToList(RevData, "Total Income", GrossAmt, "BoldTotal", 0, 0);

    if (this.ExpenseData != undefined || this.ExpenseData != null) {
      this.ExpenseData.COAList.forEach(a => {
        if (true) {
          ExpData = this.pushToList(ExpData, a.COA, a.TotalAmount, "BoldCategoryCOA", 0, 0);
          RevData = this.pushToList(RevData, a.COA, a.TotalAmount, "BoldCategoryCOA", 0, 0);
          a.LedgerGroupList.forEach(b => {
            ...
            b.LedgerList.forEach(c => {
              RevData = this.pushToList(RevData, c.LedgerName, CommonFunctions.parseAmount(c.Amount), "ledgerLevel", c.LedgerId, c.Code);
              ExpData = this.pushToList(ExpData, c.LedgerName, CommonFunctions.parseAmount(c.Amount), "ledgerLevel", c.LedgerId, c.Code);
            });
          });
        }
      });
    ...
    let expData = _.cloneDeep(ExpData.filter(a => a.Style === 'BoldCategoryCOA'));
    GrossLoss = expData.reduce((a, b) => a + b.Amount, 0);
    RevData = this.pushToList(RevData, "Total Expense", GrossLoss, "BoldTotal", 0, 0);

    let netBalance = (GrossAmt - GrossLoss);
    if (netBalance >= 0) {
      RevData = this.pushToList(RevData, "Gross Profit", netBalance, "BoldTotal", 0, 0);
    } else {
      RevData = this.pushToList(RevData, "Gross Loss", Math.abs(netBalance), "BoldTotal", 0, 0);
    }
