...
export class IncentiveDLService {
  ...
  GetEmployeeBillItemsList(employeeId) {
    return this.http.get<FromzaHTTPResponse>(`/api/Incentive/EmployeeBillItems?employeeId=${employeeId}`, this.options);
  }

  GetItemsForIncentive(priceCategoryId: number) {
    return this.http.get<FromzaHTTPResponse>(`/api/Incentive/IncentiveItems?priceCategoryId=${priceCategoryId}`, this.options);
  }
