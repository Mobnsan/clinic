...
export class RPT_BIL_IncentiveReportSummaryComponent {
  ...
  public ExportToExcelDocSummaryReport(tableId) {
    ...
    if (tableId) {
      let workSheetName = this.IsReferralOnly ? 'Incentive Doctor Referral Summary Report':'Incentive Doctor Summary Report';
      let Heading = this.IsReferralOnly ? 'Incentive Doctor Referral Summary':'Incentive Doctor Summary';
      let filename = this.IsReferralOnly ? 'IncentiveDoctorReferralSummary':'IncentiveDoctorSummary';
  ...
  public LoadDoctorItems(row) {
    this.selDoctorId = row.PrescriberId;
