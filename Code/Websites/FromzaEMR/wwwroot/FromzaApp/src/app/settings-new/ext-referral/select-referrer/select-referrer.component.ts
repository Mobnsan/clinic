...
export class SelectReferrerComponent {
  ...
  @Input("reload-referrer-list") reloadReferrerList: boolean = false;
  ...
  public RequestedByDrOnChange() {
    ...
    if (this.currentRequestedByDoctor) {
      ...
      if (doctor) {
        ...
        this.onReferrerChange.emit({ ReferrerId: doctor.EmployeeId, ReferrerName: retName, MedicalCertificateNo: doctor.MedCertificationNo, IsExternal: doctor.IsExternal });
      }
      else {
        if (this.allowFreeText) {
          ...
          this.onReferrerChange.emit({ ReferrerId: null, ReferrerName: null, MedicalCertificateNo: null });
        }
      ...
      if (this.selectedRefId || this.selectedRefName) {
        ...
        if (selRefDoc) {
          if (selRefDoc.IsExternal) {
            ...
            this.onReferrerChange.emit({ ReferrerId: selRefDoc.EmployeeId, ReferrerName: selRefDoc.FullName, MedicalCertificateNo: selRefDoc.MedCertificationNo });
          }
          if (!this.includeExtReferrer) {
          ...
          this.onReferrerChange.emit({ ReferrerId: null, ReferrerName: this.selectedRefName, MedicalCertificateNo: null }); //* Krishna, 25thJan'23 Setting MedicalCertificateNo null here as it should come from SelRefDoc but this line gets executed when there is no selfRefDoc.
        }
      }
      else {
  ...
  ngOnInit() {
    ...
    if (this.reloadReferrerList) {
      this.GetAllReferrerList(true);
    }
    else {
  ...
  SelectDefault() {
    if (this.defaultRefInfo && this.defaultRefInfo.DefaultReferrerId) {
      ...
      if (selRefDoc) {
        this.currentRequestedByDoctor = selRefDoc.FullName;
        this.currentRequestedByDoctor
        //once doctor is asigned, emit the selected emplyee id to parent component again. 
        this.onReferrerChange.emit({ ReferrerId: selRefDoc.EmployeeId, ReferrerName: selRefDoc.FullName, MedicalCretificateNo: selRefDoc.MedCertificationNo });
      }
  ...
  setFocusOnReferrerForNarcoticDrug() {
    ...
    var Timer = setTimeout(() => {
      if (document.getElementById("id_input_currentRequestedByDoctor_selectReferral")) {
        let nextEl = <HTMLInputElement>document.getElementById("id_input_currentRequestedByDoctor_selectReferral");
        nextEl.focus();
        nextEl.select();
        clearTimeout(Timer);
      }
