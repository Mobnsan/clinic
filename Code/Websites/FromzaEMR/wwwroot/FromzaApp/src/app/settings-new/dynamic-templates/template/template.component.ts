...
export class TemplateComponent {
      ...
      this.settingsBLService.GetDynTemplateDataById(templateId)
        .subscribe((res: FromzaHTTPResponse) => {
          if (res.Status === ENUM_FromzaHTTPResponses.OK) {
            ...
            this.msgBoxServ.showMessage(ENUM_FromzaHTTPResponses.Failed, ["Record not found"]);
          }

        },
          err => {
            this.msgBoxServ.showMessage(ENUM_FromzaHTTPResponses.Failed, ["Check log for error message."]);
    ...
    if (currTemplate != null) {
      ...
      if (proceed) {
        this.settingsBLService.UpdateTemplateSettings(currTemplate.TemplateId)
          .subscribe((res: FromzaHTTPResponse) => {
            if (res.Status === ENUM_FromzaHTTPResponses.OK) {
              this.msgBoxServ.showMessage(ENUM_FromzaHTTPResponses.OK, ['template Status updated successfully']);
              this.GetTemplateList();
            }
            else {
              this.msgBoxServ.showMessage(ENUM_FromzaHTTPResponses.Failed, ['Something wrong, Please Try again..!']);
            }
          },
            err => {
              this.logError(err);
  ...
  logError(err: any) {
    console.error(err);
    this.msgBoxServ.showMessage(ENUM_FromzaHTTPResponses.Failed, [err]);
  }
  ...
  ShowCatchErrMessage(exception) {
    if (exception) {
      ...
      this.msgBoxServ.showMessage(ENUM_FromzaHTTPResponses.Failed, ["Check error in Console log !"]);
      console.error(ex);
    }
  }
    ...
    try {
      if (this.IsValidModelCheck()) {
        ...
        this.settingsBLService.AddNewTemplate(this.currentTemplate)
          .subscribe((res: FromzaHTTPResponse) => {
            if (res.Status = ENUM_FromzaHTTPResponses.OK) {
              this.msgBoxServ.showMessage(ENUM_FromzaHTTPResponses.OK, ["Template Saved."]);
  ...
  CallBackAddUpdate(res, action) {
    try {
      if (res.Status === ENUM_FromzaHTTPResponses.OK) {
        ...
        this.msgBoxServ.showMessage(ENUM_FromzaHTTPResponses.Failed, ["Check log for details"]);
