...
export class AddSchemePriceCategoryItemsComponent implements OnInit {
  ...
  public async GetBillingSchemes() {
    try {
      const res: FromzaHTTPResponse = await this.settingsBLService.GetBillingSchemes().toPromise();
  ...
  public async GetPriceCategory() {
    try {
      const res: FromzaHTTPResponse = await this.settingsBLService.GetPriceCategory().toPromise();
