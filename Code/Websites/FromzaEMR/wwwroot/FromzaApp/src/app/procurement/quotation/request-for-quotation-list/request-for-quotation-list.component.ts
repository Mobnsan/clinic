...
export class RequestForQuotationListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    let signatoriesParameterValue = this.coreService.Parameters.find(a => a.ParameterName == 'QuotationPrintSignatorySettings' && a.ParameterGroupName == 'Quotation').ParameterValue;
    if (signatoriesParameterValue) {
      let signatures = JSON.parse(signatoriesParameterValue);
      this.signature1 = signatures.Signature1 != "" ? signatures.Signature1 : null;
      this.signature2 = signatures.Signature2 != "" ? signatures.Signature2 : null;
    }
    else {
      this.msgBoxServ.showMessage("Failed", ["Error getting QuotationPrintSignatorySettings parameters."]);
    }
  ...
  AddQuotationFiles(filesToUpload, quotationfiles): void {
    ...
    try {
      ...
      if (filesToUpload.length || quotationfiles) {
        this.quotationBLService.AddQuotationFiles(filesToUpload, quotationfiles).finally(() => { this.loading = false; })
          .subscribe(res => {
            if (res.Status == "OK") {
  ...
  print() {
    ...
    popupWinindow.document.write(`<html><head><link rel="stylesheet" type="text/css" href="../../../themes/theme-default/ReceiptList.css" /></head><body onload="window.print()">` +
      ...
      `<link rel="stylesheet" type="text/css" href="../../../../../../themes/theme-default/FromzaPrintStyle.css" />` + printContents +
      `</body></html>`);
