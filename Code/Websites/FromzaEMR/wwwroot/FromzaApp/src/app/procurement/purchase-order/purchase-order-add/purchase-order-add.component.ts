...
import { ENUM_FromzaHTTPResponseText, ENUM_GRItemCategory, ENUM_MessageBox_Status, ENUM_TermsApplication } from '../../../shared/shared-enums';
...
export class PurchaseOrderAddComponent implements AfterViewInit {
      ...
      this.procurementBLService.GetPurchaseRequestById(this.inventoryService.PurchaseRequestId)
        .subscribe(res => {
          if (res.Status == "OK") {
            ...
            RequisitionItemArray = RequisitionItemArray.filter(itm => itm.RequestItemStatus == 'active' || itm.RequestItemStatus == 'partial');
            ...
            for (var i = 0; i < RequisitionItemArray.length; i++) {
              if (RequisitionItemArray[i].IsActive == true) {
                ...
                PoItem.RequestedQuantity = RequisitionItemArray[i].RequestedQuantity;
                PoItem.PendingQuantity = RequisitionItemArray[i].PendingQuantity;
                PoItem.OrderedQuantity = RequisitionItemArray[i].RequestedQuantity - RequisitionItemArray[i].PendingQuantity
  ...
  GetVendorDetails() {
    ...
    if (this.selectedVndr != null && typeof (this.selectedVndr) == 'object') {
      ...
      if (selVndr && selVndr.VendorId) {
        ...
        //assign the default currency id of vendor to the current po
        if (this.selectedVndr.DefaultCurrencyId != null && this.selectedVndr.DefaultCurrencyId != 0) {
    ...
    this.procurementBLService.getQuotationDetailsToAddPO(reqForquotId)
      .subscribe(res => {
        if (res.Status == "OK") {
          ...
          let vendor = this.VendorList.find(a => a.VendorId == this.currentPO.VendorId)
          if (vendor) {
            this.selectedVndr.VendorName = vendor.VendorName;
          }
  ...
  ShowPurchaseOrderDetails(res) {
    ...
    if (res.Status == "OK") {
      ...
      var ItemList: Array<any> = res.Results.poItems.filter(itm => itm.POItemStatus != "cancelled");
  ...
  AddPurchaseOrder() {
    ...
    if (this.currentPO.PurchaseOrderItems.some(a => a.Quantity <= 0)) {
      this.messageBoxService.showMessage('Failed', ["Please enter valid quantity ...Before Requesting"]);
      return;
    }
    if (this.currentPO.PurchaseOrderItems.some(a => a.StandardRate <= 0)) {
      this.messageBoxService.showMessage('Failed', ["Please enter valid standard rate ...Before Requesting"]);
      return;
    }
    if (this.currentPO.PurchaseOrderItems.some(a => a.VatPercentage < 0 || a.VatPercentage == null)) {
      CheckIsValid = false;
      this.messageBoxService.showMessage("Failed", ['Please provide valid VAT%.']);
      return;
    }
    ...
    if (CheckIsValid == true && this.currentPO.PurchaseOrderItems != null) {
      ...
      this.procurementBLService.PostToPurchaseOrder(this.currentPO).finally(() => {
        this.loading = false;
      }).subscribe(res => {
        if (res.Status == 'OK') {
          ...
          this.selectedVndr = new VendorMaster();
  ...
  UpdatePurchaseOrder() {
    ...
    if (CheckIsValid == true && this.currentPO.PurchaseOrderItems != null) {
      ...
      this.loading = true;
      // filtering our the last item if left blank is done in blservice.
      this.procurementBLService.UpdatePurchaseOrder(this.currentPO).finally(() => {
        this.loading = false;
      }).
        subscribe(res => {
          if (res.Status == 'OK') {
  ...
  ShowPurchaseOrderDraftDetails(res) {
    if (res.Status === ENUM_FromzaHTTPResponseText.OK) {
      ...
      this.selectedCurrencyCode = poDraftDetails.CurrencyCode;
      this.currentPO.VAT = poDraftDetails.VATAmount;
      this.currentPO.PORemark = poDraftDetails.Remarks;
      this.currentPO.PaymentMode = 'Credit';
      this.currentPO.PurchaseOrderItems = poDraftItems.map(poItem =>
        Object.assign(new PurchaseOrderItems(), poItem, {
          StandardRate: poItem.ItemRate,
          VatPercentage: poItem.VATPercentage,
          Remark: poItem.Remarks,
      ...
      this.currentPO.POStatus = poDraftDetails.Status;
      this.selectedVndr = this.currentPO.VendorName;
      this.currentPO.VendorId = poDraftDetails.VendorId;
      this.currentPO.DeliveryDate = moment().add(7, 'days').format('YYYY-MM-DD');
      this.currentPO.PurchaseOrderItems = this.currentPO.PurchaseOrderItems.filter(p => p.IsActive == true);
      this.currentPO.PurchaseOrderValidator.get("PaymentMode").setValue(this.currentPO.PaymentMode);
    }
  }

  RoutePurchaseOrderList() {
    this.router.navigate(['/ProcurementMain/PurchaseOrder/PurchaseOrderList']);
  }
