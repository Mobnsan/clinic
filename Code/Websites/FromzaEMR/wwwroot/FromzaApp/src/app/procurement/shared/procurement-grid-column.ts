...
export default class ProcurementGridColumns {
  ...
  static GRActionRenderer(params) {
    let template = `<a fromza-grid-action="view" class="grid-action">View</a>`;
    if (params.data.IsDonation) {
      template += `<a fromza-grid-action="show-donation-detail" class="grid-action">Donation Detail</a>`;
    }
    return template;
  }
  ...
  static ShowActionForRFQList(params) {
    if (params.data.Status == "active" || params.data.Status == "partial") {
      ...
      <a fromza-grid-action="AttachQuotationDocuments" class="grid-action">Attach Files</a>
      <a fromza-grid-action="AddQuotationDetails" class="grid-action">Add Vendor Quot.</a>
      <a fromza-grid-action="AnalyseQuotation" class="grid-action">Analyse Quotation</a>
             `;
        return template;
      }
      else {
        let template = `<a fromza-grid-action="View" class="grid-action">RFQ Details</a>
      <a fromza-grid-action="AttachQuotationDocuments" class="grid-action">Attach Files</a>
      <a fromza-grid-action="AddQuotationDetails" class="grid-action">Update Vendor Quot.</a>
      <a fromza-grid-action="AnalyseQuotation" class="grid-action">Analyse Quotation</a>
             `;
        return template;
      }
      ...
      let template = `<a fromza-grid-action="View" class="grid-action">RFQ Details</a>
          ...
          <a fromza-grid-action="SelectedQuotation" class="grid-action"> Order Letter </a>
          <a fromza-grid-action="addPO" class="grid-action" style="display:none"> Add PO </a>`;
      return template;
    }
  }
  ...
  static ShowActionForPODraftList(params) {
    ...
    if (params.data.Status !== "Discarded") {
      template += `</a>
                <a fromza-grid-action="review" class="grid-action">
                Review/Edit
             </a>&nbsp;
             `;
    }
    ...
    if (params.data.Status !== "Discarded") {
      ...
      template += `<a fromza-grid-action="CopyToPo" class="grid-action">
                Copy To PO
             </a> &nbsp;
             `;
      template += `<a fromza-grid-action="DiscardDraft" class="grid-action">
                Discard Draft
             </a> &nbsp;
             `;
    }
