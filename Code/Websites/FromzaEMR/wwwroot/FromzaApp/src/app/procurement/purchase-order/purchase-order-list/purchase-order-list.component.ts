...
export class PurchaseOrderListComponent implements OnInit {
  ...
  showPurchaseOrderDraftAddEditPage: boolean = false;
  showPurchaseOrderDraftListPage: boolean = false;
  constructor(
    ...
    this.procBLService.GetPurchaseOrderList(this.fromDate, this.toDate, Status)
      .subscribe((res: FromzaHTTPResponse) => {
        if (res.Status === ENUM_FromzaHTTPResponses.OK) {
          ...
          this.messageBoxService.showMessage(ENUM_MessageBox_Status.Failed, ['failed to get Purchase Order.. please check log for details.']);
  ...
  ClosePurchaseOrderDraftAddEditPage(event) {
    this.showPurchaseOrderDraftAddEditPage = false;
    if (event) {
      this.showPurchaseOrderDraftListPage = true;
    }
  }
