...
export class ProcurementComponent implements OnInit {
  ...
  constructor(public securityService: SecurityService, public msgBox: MessageboxService, public securityBlService: SecurityBLService, public inventoryService: InventoryService, public inventoryBLService: InventoryBLService, private _activateInventoryService: ActivateInventoryService, public router: Router) {
    FromzaCache.GetData(MasterType.AllMasters, null);
    ...
    this.inventoryBLService.GetVendorList()
      .subscribe((res: FromzaHTTPResponse) => {
        if (res.Status == "OK") {
          ...
          this.msgBox.showMessage('Failed', ["Couldn't load vendor list. (inventory-main)"]);
  ...
  UnsetGlobalInventory() {
    ...
    this._activateInventoryService.DeactivateInventory().subscribe(res => {
      if (res.Status == "OK") {
        this.router.navigate(['/ActivateInventory']);
      }
      else {
        this.msgBox.showMessage("notice", ["Failed to logout inventory."])
      }
    })
  }
  ShowInfo() {
