...
export class ReturnToVendorAddComponent implements OnInit {
    ...
    if (typeof Item === "object" && !Array.isArray(Item) && Item !== null) {
      ...
      for (var a = 0; a < this.itemsToReturn.length; a++) {
        ...
        if (a == index) {
          ...
          this.itemsToReturn[index].GoodReceiptNo = Item.BatchDetails[0].GoodReceiptNo;
        }
      }
    }
  }
    ...
    if (GoodsReceiptId != null) {
      ...
      if (checkBatchNoPresent == false) {
        ...
        if (batchList) {
          ...
          this.itemsToReturn[index].BatchNo = batchList.BatchNo;
  ...
  CalculateAll(index) {
    for (let i = 0; i <= this.itemsToReturn.length; i++) {
      this.itemsToReturn.forEach(itm => {
        itm.DiscountAmount == null ? itm.DiscountAmount = 0 : itm.DiscountAmount;
        itm.VATAmount == null ? itm.DiscountAmount = 0 : itm.VATAmount;
        itm.CCAmount == null ? itm.CCAmount = 0 : itm.CCAmount;
        itm.TotalAmount == null ? itm.TotalAmount = 0 : itm.TotalAmount;

      })

      if (this.itemsToReturn[index].Quantity > 0 && this.itemsToReturn[index].ReturnItemRate > 0) {
        this.itemsToReturn[index].SubTotal = this.itemsToReturn[index].Quantity * this.itemsToReturn[index].ReturnItemRate;

        if (this.itemsToReturn[index].DiscountAmount > 0 && this.itemsToReturn[index].VATAmount > 0) {
          this.itemsToReturn[index].TotalAmount = this.itemsToReturn[index].SubTotal - this.itemsToReturn[index].DiscountAmount + this.itemsToReturn[index].VATAmount;
        }
        if (this.itemsToReturn[index].DiscountAmount > 0 && this.itemsToReturn[index].VATAmount == 0) {
          this.itemsToReturn[index].TotalAmount = this.itemsToReturn[index].SubTotal - this.itemsToReturn[index].DiscountAmount;
        }
        if (this.itemsToReturn[index].DiscountAmount == 0 && this.itemsToReturn[index].VATAmount > 0) {
          this.itemsToReturn[index].TotalAmount = this.itemsToReturn[index].SubTotal + this.itemsToReturn[index].VATAmount;
        }
        if (this.itemsToReturn[index].DiscountAmount == 0 && this.itemsToReturn[index].VATAmount == 0) {
          this.itemsToReturn[index].TotalAmount = this.itemsToReturn[index].SubTotal;
        }
        if (this.itemsToReturn[index].CCAmount > 0) {
          this.itemsToReturn[index].TotalAmount = this.itemsToReturn[index].TotalAmount + this.itemsToReturn[index].CCAmount;
        }

        this.itemsToReturn[index].ReturnCostPrice = this.itemsToReturn[index].TotalAmount / this.itemsToReturn[index].Quantity;
      }
      else {
        this.itemsToReturn[index].SubTotal = 0;
        this.itemsToReturn[index].DiscountAmount = 0;
        this.itemsToReturn[index].VATAmount = 0;
        this.itemsToReturn[index].CCAmount = 0;
        this.itemsToReturn[index].TotalAmount = 0;
      }
  ...
  MainLevelCalculation() {
    this.itemsToReturn.forEach(itm => {
      ...
      this.returnToVendor.DiscountAmount = DiscountAmount;
      this.returnToVendor.VATTotal = VATAmount;
      this.returnToVendor.CCAmount = CCAmount;
