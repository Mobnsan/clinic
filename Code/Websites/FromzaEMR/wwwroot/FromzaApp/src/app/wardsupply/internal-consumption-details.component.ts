...
export class InternalConsumptionDetailsComponent {
        ...
        this.wardSupplyBLService.GetInternalConsumptionDetails(this.consumptionId)
            .subscribe((res: FromzaHTTPResponse) => {
                if (res.Status === ENUM_FromzaHTTPResponses.OK) {
                    ...
                    this.msgBoxService.showMessage(ENUM_MessageBox_Status.Failed, ["Something went wrong..."]);
                }
            })
    }
    ...
    public editPatientDataConsumption() {
        ...
        if (isEditForbidden) {
            this.msgBoxService.showMessage(ENUM_MessageBox_Status.Failed, ["Invoice already Generated. Further editing is forbidden."]);
        }
        else {
    ...
    editDataConsumption() {
        ...
        if (isEditForbidden) {
            ...
            this.msgBoxService.showMessage(ENUM_MessageBox_Status.Failed, ["Invoice is already Generated. Further editing is forbidden."]);
        }
    }
        ...
        this.wardSupplyBLService.GetConsumptionItemList(patientId, wardId, this.CurrentStoreId)
            .subscribe(res => {
                if (res.Status === ENUM_FromzaHTTPResponses.OK) {
                    ...
                    this.msgBoxService.showMessage(ENUM_MessageBox_Status.Failed, ['Failed to get List.' + res.ErrorMessage]);
                }
            },
                err => {
                    this.msgBoxService.showMessage(ENUM_MessageBox_Status.Failed, ['Failed to get List.' + err.ErrorMessage]);
                }
            )
    }
    ...
    GetInventoryBillingHeaderParameter() {
        ...
        if (paramValue && paramValue.ParameterValue)
            ...
            this.msgBoxService.showMessage(ENUM_MessageBox_Status.Error, ["Please enter parameter values for BillingHeader"]);
    }

    CalculateInternalTotalAmount() {
        let totalAmount = 0;
        this.consumptionLists.forEach(a => {
            totalAmount += a.Quantity * a.SalePrice;
        });
        ...
        if (check) {
            this.wardSupplyBLService.PutInternalConsumptionData(this.consumptionLists)
                ...
                .subscribe(res => {
                    ...
                    err => {
                        this.msgBoxService.showMessage(ENUM_MessageBox_Status.Failed, [err.error.ErrorMessage]);
                        this.consumptionLists = JSON.parse(JSON.stringify(this.consumptionListCopy));
                    });
        }
        else {
            this.loading = false;
            this.msgBoxService.showMessage(ENUM_MessageBox_Status.Failed, ["No changes found"]);
            this.openEditBox = false;
            this.showReceipt = true;
        }
    }
        ...
        if (check) {
            this.wardSupplyBLService.PutConsumptionData(this.consumptionLists)
                ...
                .subscribe(res => {
                    ...
                    err => {
                        this.msgBoxService.showMessage(ENUM_MessageBox_Status.Error, [err.error.ErrorMessage]);
                        this.consumptionLists = JSON.parse(JSON.stringify(this.consumptionListCopy));
                    });
        }
        else {
