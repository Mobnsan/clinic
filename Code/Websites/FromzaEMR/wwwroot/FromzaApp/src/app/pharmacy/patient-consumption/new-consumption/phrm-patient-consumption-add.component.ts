...
import { ENUM_BillingStatus, ENUM_FromzaHTTPResponseText, ENUM_FromzaHTTPResponses, ENUM_MessageBox_Status, ENUM_ServiceBillingContext } from "../../../shared/shared-enums";
...
export class PHRMPatientConsumptionAddComponent {
    ...
    GetDefaultStore() {
        if (this.WardId) {
            this.pharmacyBLService.GetWardSubStoreMapDetails(this.WardId).subscribe((res: FromzaHTTPResponse) => {
    ...
    SaveNMCNo() {
        ...
        this.pharmacyBLService.UpdateNMCNo(this.SelectedPrescriber.EmployeeId, this.MedicalCertificateNo).subscribe((res: FromzaHTTPResponse) => {
            if (res.Status === ENUM_FromzaHTTPResponses.OK) {
                this.NMCNoAddPopup = false;
