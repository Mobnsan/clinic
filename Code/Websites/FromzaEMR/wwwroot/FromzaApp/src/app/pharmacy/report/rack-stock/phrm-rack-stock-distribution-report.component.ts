...
export class PHRMRackStockDistributionReportComponent implements OnInit {
    ...
    public StoreId: number = 0; //by default, select dispensary
    public locationList = [];
    ...
    public FilterRackBasedOnLocation() {
        this.parentRackListFiltered = this.parentRackList.filter(rack => rack.StoreId == this.StoreId);
        this.childRackListFiltered = this.childRackList.filter(rack => rack.StoreId == this.StoreId);
    ...
    public LoadRackReport() {
        if (this.selectedRacks) {
            ...
            this.rackIdsString = "";
            this.selectedLocationId = this.StoreId;
