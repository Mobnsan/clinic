...
import { ENUM_FromzaHTTPResponses, ENUM_MessageBox_Status, ENUM_StoreCategory } from '../../../shared/shared-enums';
...
export class DirectDispatchComponent implements OnInit {
  ...
  DirectDispatch() {
    if (this.showSubstore && !this.DispatchNo) {
      return this.messageBoxService.showMessage(ENUM_FromzaHTTPResponses.Failed, ["Issue No is required for direct dispatch to Sub Store"]);
    }
    this.loading = true;
