...
export class PHRMPatientConsumptionListComponent {
    ...
    GetPatientConsumptionList() {
        this.pharmacyBLService.GetPatientConsumptions().subscribe((res: FromzaHTTPResponse) => {
            if (res.Status === ENUM_FromzaHTTPResponses.OK) {
                this.PatientConsumptions = res.Results;
            }
            else {
                this.messageboxService.showMessage(ENUM_FromzaHTTPResponses.Failed, ['Failed to get patient consumption list']);
            }
        },
            err => {
                this.messageboxService.showMessage(ENUM_FromzaHTTPResponses.Failed, ['Failed to get patient consumption list']);
    ...
    GetConsumptionsOfPatient(PatientId: number, PatientVisitId: number) {
        this.pharmacyBLService.GetPatientConsumptionsOfPatient(PatientId, PatientVisitId).subscribe((res: FromzaHTTPResponse) => {
            if (res.Status === ENUM_FromzaHTTPResponses.OK) {
    ...
    GetPatientConsumptionInfo(PatientConsumptionId: number) {
        this.pharmacyBLService.GetPatientConsumptionInfo(PatientConsumptionId).subscribe((res: FromzaHTTPResponse) => {
            if (res.Status === ENUM_FromzaHTTPResponses.OK) {
                this.PatientConsumptionInfo = res.Results;
            }
            else {
                this.messageboxService.showMessage(ENUM_FromzaHTTPResponses.Failed, ['Failed to get consumption information']);
            }
        },
            err => {
                this.messageboxService.showMessage(ENUM_FromzaHTTPResponses.Failed, ['Failed to get consumption information']);
