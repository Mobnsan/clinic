...
export class PHRMPatientConsumptionFinalizeComponent {
    ...
    GetPatientConsumptionFinalizeList() {
        this.pharmacyBLService.GetFinalizePatientConsumptions().subscribe((res: FromzaHTTPResponse) => {
    ...
    GetPatientConsumptionFinalizeInvoice(InvoicePrintNo: number) {
        this.pharmacyBLService.GetPatientConsumptionFinalizeInvoice(InvoicePrintNo).subscribe((res: FromzaHTTPResponse) => {
            if (res.Status === ENUM_FromzaHTTPResponses.OK) {
