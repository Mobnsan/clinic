...
import { ENUM_FromzaHTTPResponses, ENUM_InvoiceReceiptMode, ENUM_MessageBox_Status } from "../../shared/shared-enums";
...
export class PharmacyReceiptComponent {
  ...
  GetInvoiceInfo(InvoiceId) {
    if (InvoiceId > 0) {
      this.pharmacyBLService.GetInvoiceReceiptByInvoiceId(InvoiceId).subscribe((res: FromzaHTTPResponse) => {
  ...
  GetCreditNoteInfo(InvoiceIdReturnId) {
    if (InvoiceIdReturnId > 0) {
      this.pharmacyBLService.GetSaleReturnInvoiceItemsByInvoiceRetId(InvoiceIdReturnId).subscribe((res: FromzaHTTPResponse) => {
        if (res.Status === ENUM_FromzaHTTPResponses.OK) {
          this.receipt = res.Results;
          this.updateItemDisplayName(this.showGenericName, this.showItemName, this.LeadingSeparator, this.showGenNameAfterItemName);
        }
