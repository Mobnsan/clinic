...
export default class PHRMGridColumns {
    ...
    static PHRMTAXList = [
        ...
        { headerName: "Description", field: "Description", width: 100 },
        { headerName: "Action", field: "", width: 150, template: '<a fromza-grid-action="edit" class="grid-action">Edit</a>' },
    ]
    static PHRMGoodsReceiptList = [
        ...
        { headerName: "Action", field: "", width: 100, template: '<a fromza-grid-action="view" class="grid-action">View</a>' },
    ]
    ...
    static PHRMACCTGoodsReceiptListAction(params) {
        ...
        if (count != null) {
            return `<a fromza-grid-action="view" class="grid-action">View</a>`
        }
    }
    ...
    static PHRMReturnToSupplier = [
        ...
        { headerName: "VAT Amount", field: "VATAmount", width: 100 },
        { headerName: "Total Amount", field: "TotalAmount", width: 100 },
        { headerName: "Action", field: "", width: 100, template: '<a fromza-grid-action="return" class="grid-action">Return</a> &nbsp;<a fromza-grid-action="preview" class="grid-action">Preview</a>' },
    ]
    ...
    static PHRMNArcoticsStockDetailsList = [
        ...
        //   field: "", width: 150,
        //   template:
        //     ` <a  fromza-grid-action="manage-stock" class="grid-action">
        //                     Manage Stock </a>
        //       <a  fromza-grid-action="transfer-store" class="grid-action">
        //                     Store Transfer </a>
        //                  `
        // },
        //<a fromza- grid - action="view" class="grid-action" >
    ...
    static StockTxnItems = [
        ...
        //{ headerName: "Exp Date", field: "ExpiryDate", width: 100, cellRenderer: PHRMGridColumns.StockTxnItemExpDateRender },        
        {
            ...
            template:
                `</a>
                        <a fromza-grid-action="update-mrp" class="grid-action">
                       Update SalePrice
                     </a>                      
                     `
        },
        ...
        if (params.data.IsActive == true) {
            let template =
                `
                    ...
                    <a fromza-grid-action="activateDeactivateIsActive" class="grid-action">Deactivate</a>            
                `
            return template
        }
        else {
            let template =
                `
                    <a fromza-grid-action="activateDeactivateIsActive" class="grid-action">Activate</a>
                `
            return template;
        }
    }
        ...
        if (params.data.IsActive == true) {
            let template =
                `
                    <a fromza-grid-action="edit" class="grid-action">Edit</a>                    
                    <a fromza-grid-action="activateDeactivateIsActive" class="grid-action">Deactivate</a>             
                    <a fromza-grid-action="showPaymentModes" class="grid-action">Payment Modes</a>             
                `
            return template
        }
        else {
    ...
    static ShowActionforPHRMSettingItemManage(params) {
        if (params.data.IsActive == true) {
            let template =
                `  
                    <a fromza-grid-action="add-rack" class="grid-action">Add to Rack</a>
                    <a fromza-grid-action="edit" class="grid-action">Edit</a>                    
                    <a fromza-grid-action="activateDeactivateIsActive" class="grid-action">Deactivate</a>             
                `
            return template
    ...
    static PHRMPatientList = [
        ...
        { headerName: "Patient Type", field: "IsOutdoorPat", width: 100, cellRenderer: PHRMGridColumns.IsOutdoorPatientText },
        {
            ...
            width: 320,
            template:
                `<a fromza-grid-action="sale" class="grid-action">
                    Sale
                </a>
                &nbsp;
    ...
    static ShowActionForPHRMSaleList(params) {
        ...
        //                <a  fromza-grid-action="saleCredit" class="grid-action">
        //                 Make Payment </a>
        //              `
        //     return template
        //            }
    ...
    static GetStoreStockActions(params) {
        let template = ``;
        template += '<a fromza-grid-action="print-barcode" class="grid-action phrm-stock-action fa fa-barcode" title = "Print Barcode" > </a>'
        if (PHRMGridColumns.securityService.HasPermission('store-update-mrp-button'))
            template += `<a fromza-grid-action="update-mrp" class="grid-action">Update SalePrice</a>`
        if (PHRMGridColumns.securityService.HasPermission('store-update-expirydate-batchno-button'))
            template += `<a fromza-grid-action="update-expirydate-batchno" class="grid-action">Update Exp&Batch</a>`
        if (PHRMGridColumns.securityService.HasPermission('show-pharmacy-manage-stock-item-button'))
            template += `<a fromza-grid-action="manage-stock" class="grid-action">Manage Stock</a >`;
        return template;

    }
    GetDispensaryStockActions() {
        let template = `<a  fromza-grid-action="transfer-store" class="grid-action">Store Transfer</a>`;
    ...
    static SupplierLedgerViewList = [
        ...
        { headerName: "Total Amount", field: "TotalAmount", width: 100 },
        {
            ...
            width: 200,
            template:
                `<a fromza-grid-action="pay" class="grid-action">
           Payment
        </a>
       `
        }
