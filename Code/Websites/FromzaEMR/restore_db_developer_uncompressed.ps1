...
Write-Host "1/4. Disabling Windows NTFS Compression on the target folder..." -ForegroundColor Cyan
# This fixes the 'file is compressed' SQL Server error!
& compact.exe /u /s /i $tempDir | Out-Null

Write-Host "2/4. Moving files to C:\Temp to bypass Windows Permissions..." -ForegroundColor Cyan
...
Write-Host "3/4. Killing active connections (if any)..." -ForegroundColor Cyan
$cmd.CommandText = $killSql
try { $cmd.ExecuteNonQuery() | Out-Null } catch {}

Write-Host "4/4. Restoring database from backup. This can take 1-3 minutes..." -ForegroundColor Yellow
