...
namespace FromzaEMR.Services
{
    public interface IInventoryGoodReceiptService
    {
        ...
        InventoryStockIds AddtoInventoryStock(GoodsReceiptItemsModel grItem, string GRCategory, int StoreId, RbacUser currentUser, DateTime currentDate, int currentFiscalYearId);
        bool ReceiveGoodsReceipt(int GRId, RbacUser currentUser, string ReceiveRemarks = "");
        List<InventoryChargesMasterModel> GetINVOtherChargesDetails();
    }
}
