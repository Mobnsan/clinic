...
namespace FromzaEMR.Services
{
    public class InventoryReceiptNumberService : IInventoryReceiptNumberService
    {
        ...
        {
            int dispatchId = (from dis in db.Dispatch
                              where dis.FiscalYearId == fiscalYearId && dis.ReqDisGroupId == ReqDisGroupId
                              select dis.DispatchId).DefaultIfEmpty(0).Max();
            return ++dispatchId;
        }
        ...
        public int GeneratePurchaseOrderDraftNumber(int? fiscalYearId, int? PODGroupId)
        {
            int poDraftNumber = (from purchaseorderno in db.PurchaseOrderDrafts
                            where purchaseorderno.FiscalYearId == fiscalYearId && purchaseorderno.PODGroupId == PODGroupId
                            select purchaseorderno.DraftPurchaseOrderNo).DefaultIfEmpty(0).Max();
            return ++poDraftNumber;
        }
    }
}
