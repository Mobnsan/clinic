...
namespace FromzaEMR.Services
{
    public class PharmacyStockBarcodeService
    {
        ...
        public int AddStockBarcode(PHRMStockMaster stock, int createdBy)
        {
            ...
            // Check if barcode is already available for the stock
            var stockBarcodeEntity = db.StockBarcodes.Where(b => b.ItemId == stock.ItemId && b.BatchNo == stock.BatchNo && b.ExpiryDate == stock.ExpiryDate && b.SalePrice == stock.SalePrice).FirstOrDefault();
