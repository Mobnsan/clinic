...
namespace FromzaEMR.Services.Maternity
{
    public class MaternityService : IMaternityService
    {
        ...
        public DataSet GetMaternityAllowanceReportList(DateTime FromDate, DateTime ToDate , int userId)
        {
        ...
        public MaternityPayment AddMaternityPatientPayment(MaternityPayment payment)
        {
            using (var transaction = maternityDbContext.Database.BeginTransaction())
            {
                try
                {
                    ...
                    maternityDbContext.SaveChanges();
                    MasterDbContext masterDbContext = new MasterDbContext(connStr);
                    var paymentModeObj = masterDbContext.PaymentModes.FirstOrDefault(a => a.PaymentSubCategoryName.ToLower() == payment.PaymentMode);
                    //emp transaction
                    ...
                    empCashTransaction.PaymentModeSubCategoryId = paymentModeObj.PaymentSubCategoryId;
                    empCashTransaction.ModuleName = "Maternity";
                    empCashTransaction.PatientId = payment.PatientId;
