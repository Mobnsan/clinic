...
using FromzaEMR.ServerModel.Vaccination;
...
namespace FromzaEMR.Services.Vaccination
{
    public class VaccinationService : IVaccinationService
    {
        ...
        public PatientModel AddUpdateVaccinationPatient(PatientModel patient)
        {
            ...
            {
                vaccRegdNum = vaccinationDbContext.Patients.Where(p => (p.IsVaccinationPatient == true) && p.VaccinationRegNo.HasValue
            ...
            else
            {
                ...
                var patWithRegNumber = vaccinationDbContext.Patients.Where(p => ((patientId > 0) ? (p.PatientId != patientId) : true)
            ...
            if (patient.PatientId == 0)
            {
                //var membership = vaccinationDbContext.Schemes.Where(i => i.SchemeName == "General").FirstOrDefault();
                ...
                patient.IsActive = true;
                patient.IsVaccinationPatient = true;
                patient.IsVaccinationActive = true;
                patient.VaccinationFiscalYearId = fiscalYearId;
                tempPat = CreatePatientAndSave(vaccinationDbContext, patient);  //Krishna,18th,Jul'22 , This function will register a patient(handling duplictae PatientNo i.e. It will be recursive until the unique PatientNo is received.)
                ...
                GenerateUniqueVisitCodeAndSaveChanges(vaccinationDbContext, immunVisiObj, connStr); //Krishna, 18th,Jul'22, this function will work in the recursive manner if the visitcode is getting duplicate.
                //vaccinationDbContext.SaveChanges();
            }
            //update case
            else
            {
                ...
                //{
                //    var membership = vaccinationDbContext.Schemes.Where(i => i.SchemeName == "General").FirstOrDefault();
                //    selectedPat.MembershipTypeId = membership.SchemeId;
                //}
            ...
            catch (Exception ex)
            {
                if (ex is DbUpdateException dbUpdateEx)
                {
                    if (dbUpdateEx.InnerException?.InnerException is SqlException sqlException)
                    {
                        ...
                        {
                            CreatePatientAndSave(vaccinationDbContext,patient);
                        }
                        else
                        {
                            throw;
        ...
        public int GetLatestVaccRegNumber(int fiscalYearId)
        {
            ...
            var vaccRegdNum = vaccinationDbContext.Patients.Where(p =>(p.IsVaccinationPatient == true) && p.VaccinationRegNo.HasValue
                                && (p.VaccinationFiscalYearId == fiscalYearId)).Max(m => m.VaccinationRegNo) ?? 0;
            vaccRegdNum++;
            return vaccRegdNum;
        }
        ...
        public void UpdatePatienVaccRegNumber(int patId, int regNum, int selectedFiscalYearId)
        {
            if (selectedFiscalYearId > 0)
            {
                ...
                var patWithRegNumber = vaccinationDbContext.Patients.Where(p => (p.PatientId != patId)
        ...
        public VaccPatientWithVisitInfoVM PostFollowupVisit(VisitModel vis, string connString, RbacUser currentUser)
        {
            ...
            vaccinationDbContext.SaveChanges();
            vis.QueueNo = CreateNewPatientQueueNo(vis.PatientVisitId, connString);
        ...
        public static int CreateNewPatientQueueNo(int visitId, string con)
        {
            int QueueNo;
