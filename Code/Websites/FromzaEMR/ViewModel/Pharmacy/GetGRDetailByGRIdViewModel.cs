...
namespace FromzaEMR.ViewModel.Pharmacy
{
    ...
    public static class GetGRDetailsByGRIdFunc
    {
        ...
        {
            int StoreId = db.PHRMStore.Where(s => s.SubCategory == ENUM_StoreCategory.Pharmacy).Select(s => s.StoreId).FirstOrDefault();
            ...
            goodreceipt.GoodsReceiptItems = (from gri in db.PHRMGoodsReceiptItems
                                             ...
                                             select new GetGRDetailsByGRIdItemDTO
                                             {
                                                 ...
                                                 MRP= gri.MRP,
                                                 RackNo = (from rackItem in db.PHRMRackItem.Where(ri => ri.ItemId == gri.ItemId && ri.StoreId == StoreId)
                                                               join rack in db.PHRMRack on rackItem.RackId equals rack.RackId
